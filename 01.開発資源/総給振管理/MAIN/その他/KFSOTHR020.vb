...
Public Class KFSOTHR020
    Inherits System.Windows.Forms.Form

    Private CAST As New CASTCommon.Events
    Private CASTx01 As New CASTCommon.Events("0-9", CASTCommon.Events.KeyMode.GOOD)

    Private MainLOG As New CASTCommon.BatchLOG("KFSOTHR020", "振込発信リエンタ作成取消画面")
    Private Const msgtitle As String = "振込発信リエンタ作成取消画面(KFSOTHR020)"
    ...
    '画面LOAD時
    Private Sub KFSOTHR020_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            If MessageBox.Show(MSG0015I.Replace("{0}", "振込発信リエンタ作成取消"), msgtitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.Cancel Then
                Exit Sub
            End If
            ...
            '振込発信マスタ削除処理
            '------------------------------------------------
            If fn_delete_HASSINMAST(strTIME_STAMP_W, lngHASSIN_CNT, MainDB) = False Then
                MessageBox.Show(MSG0002E.Replace("{0}", "更新"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                MainDB.Rollback()
                Return
            End If
            ...
            MessageBox.Show(MSG0016I.Replace("{0}", "振込発信リエンタ作成取消"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
...
#Region "KFSOTHR020用関数"
    ...
    Private Function fn_update_SCHMAST(ByVal strTIME_STAMP_W As String, ByRef lngSCH_CNT As Long, ByVal rDB As CASTCommon.MyOracle) As Boolean
        ...
        Try
            ...
            SQL.AppendLine(" HASSIN_DATE_S = '00000000'")
            SQL.AppendLine(",HASSIN_TIME_STAMP_S = '00000000000000'")
            SQL.AppendLine(",HASSIN_FLG_S = '0'")
            SQL.AppendLine(" WHERE HASSIN_TIME_STAMP_S = '" & strTIME_STAMP_W & "'")
            SQL.AppendLine(" AND HASSIN_FLG_S = '1'")
            SQL.AppendLine(" AND EXISTS")
            SQL.AppendLine(" (SELECT * FROM HASSINMAST")
            SQL.AppendLine(" WHERE HASSIN_TIME_STAMP_S = TIME_STAMP_FH)")
    ...
    Private Function fn_delete_HASSINMAST(ByVal strTIME_STAMP_W As String, ByRef lngCNT As Long, ByVal rDB As CASTCommon.MyOracle) As Boolean
        '============================================================================
        'NAME           :fn_delete_KESSAIMAST
        'Parameter      :
        'Description    :振込発信マスタ削除
        'Return         :True=OK,False=NG
        'Create         :
        'Update         :
        '============================================================================
        ...
        Try
            ...
            '振込発信マスタ削除
            '------------------------------------------------
            SQL = "DELETE FROM HASSINMAST"
            SQL &= " WHERE TIME_STAMP_FH = '" & strTIME_STAMP_W & "'"

            Dim nRet As Integer = rDB.ExecuteNonQuery(SQL)
            If nRet = 0 Then
                lngCNT = nRet
            ElseIf nRet < 0 Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(発信マスタ削除)", "失敗", "予期せぬエラー")
                Return False
            End If

        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(発信マスタ削除)", "失敗", ex.Message)
            Return False
        Finally
        End Try
