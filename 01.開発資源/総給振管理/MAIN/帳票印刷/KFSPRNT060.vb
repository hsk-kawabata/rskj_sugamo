...
Public Class KFSPRNT060

    Private MainLOG As New CASTCommon.BatchLOG("KFSPRNT060", "為替振込明細表再印刷画面")
    Private Const msgTitle As String = "為替振込明細表再印刷画面(KFSPRNT060)"
...
#Region "ロード"
    Private Sub KFSPRNT060_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'ログの書込に必要な情報の取得
        ...
        Try
            ...
            '=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
            Dim Msg As String
            Select Case GCom.SetComboBox(cmbPrintKbn, "KFSPRNT060_対象帳票.TXT", True)
                Case 1  'ファイルなし
                    MessageBox.Show(String.Format(MSG0025E, "対象帳票", "KFSPRNT060_対象帳票.TXT"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Msg = "対象帳票設定ファイルなし。ファイル:KFSPRNT060_対象帳票.TXT"
    ...
    Private Function Get_PrintKbn() As Boolean
        ...
        Try
            Select Case GCom.GetComboBox(cmbPrintKbn)
                Case 0
                    PrintID = "KFSP010"
                    PrintName = "為替振込明細表(本支店為替)"
                Case 1
                    PrintID = "KFSP011"
                    PrintName = "為替振込明細表(他行為替)"
                Case 2
                    PrintID = "KFSP012"
                    PrintName = "為替振込明細表(自振ロギング登録)"
                Case Else
                    Return False
            End Select
            Return True
