...
Public Class KFSPRNT050

    Private MainLOG As New CASTCommon.BatchLOG("KFSPRNT050", "処理結果確認表再印刷画面")
    Private Const msgTitle As String = "処理結果確認表再印刷画面(KFSPRNT050)"
...
#Region "ロード"
    Private Sub KFSPRNT050_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'ログの書込に必要な情報の取得
        ...
        Try
            ...
            '=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
            Dim Msg As String
            Select Case GCom.SetComboBox(cmbPrintKbn, "KFSPRNT050_対象帳票.TXT", True)
                Case 1  'ファイルなし
                    MessageBox.Show(String.Format(MSG0025E, "対象帳票", "KFSPRNT050_対象帳票.TXT"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Msg = "対象帳票設定ファイルなし。ファイル:KFSPRNT050_対象帳票.TXT"
    ...
    Private Function Get_PrintKbn() As Boolean
        ...
        Try
            Select Case GCom.GetComboBox(cmbPrintKbn)
                ...
                Case 1
                    PrintID = "KFSP005"
                    PrintName = "処理結果確認表(資金確保)"
                Case 2
                    PrintID = "KFSP007"
                    PrintName = "処理結果確認表(資金確保結果)"
                Case 3
                    PrintID = "KFSP008"
                    PrintName = "処理結果確認表(振込発信)"
                Case 4
                    PrintID = "KFSP014"
                    PrintName = "処理結果確認表(振込発信結果)"
                Case 5
                    PrintID = "KFSP015"
                    PrintName = "処理結果確認表(為替請求)"
                Case 6
                    PrintID = "KFSP016"
                    PrintName = "処理結果確認表(為替請求結果)"
                Case 7
                    PrintID = "KFSP020"
