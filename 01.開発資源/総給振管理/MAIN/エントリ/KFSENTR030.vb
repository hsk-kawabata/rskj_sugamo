...
Public Class KFSENTR030
    Private CAST As New CASTCommon.Events
    Private CASTx01 As New CASTCommon.Events("0-9", CASTCommon.Events.KeyMode.GOOD)

    Private strBAITAI_CODE As String

    Private MainLOG As New CASTCommon.BatchLOG("KFSENTR030", "依頼書用データ入力画面")
    Private Const msgTitle As String = "依頼書用データ入力画面(KFSENTR030)"
...
#Region " ロード"
    Private Sub KFSENTR030_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            'ログの書込に必要な情報の取得
...
#Region " 実行"
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            Dim KFSENTR031 As KFSENTR031 = Nothing
            Dim KFSENTR032 As KFSENTR032 = Nothing
            'Dim KFSENTR031 As New KFSENTR031
            ...
            '2018/02/09 saitou 広島信金(RSV2標準) UPD エントリ20明細対応 -------------------- START
            Dim ShowFormFlg As String = CASTCommon.GetRSKJIni("RSV2_V1.0.0", "KFSENTR030_SHOWFORM")
            If ShowFormFlg = "1" Then
                KFSENTR032 = New KFSENTR032
                '必要項目を変数にセットする
                KFSENTR032.strTORIS_CODE = strKEY1
                KFSENTR032.strTORIF_CODE = strKEY2
                KFSENTR032.strFURI_DATE = strKEY3
                If txtSyokiti.Text = "" Then
                    KFSENTR032.strSYOKITI = ""
                Else
                    KFSENTR032.strSYOKITI = txtSyokiti.Text.Trim
                End If
                ...
                If txtKaisiPage.Text.Trim = "" Then                                  '開始ﾍﾟｰｼﾞ
                    KFSENTR032.lngPAGE = 1                                                '(未入力時は1をｾｯﾄ)
                Else
                    KFSENTR032.lngPAGE = GCom.NzLong(txtKaisiPage.Text)
                End If
                KFSENTR032.BERYFAI = BERYFAI
            Else
                KFSENTR031 = New KFSENTR031
                '必要項目を変数にセットする
                KFSENTR031.strTORIS_CODE = strKEY1
                KFSENTR031.strTORIF_CODE = strKEY2
                KFSENTR031.strFURI_DATE = strKEY3
                If txtSyokiti.Text = "" Then
                    KFSENTR031.strSYOKITI = ""
                Else
                    KFSENTR031.strSYOKITI = txtSyokiti.Text.Trim
                End If
                If rdbSei.Checked Then
                    KFSENTR031.intCHK = 0
                Else
                ...
                End If
                If txtKaisiPage.Text.Trim = "" Then                                  '開始ﾍﾟｰｼﾞ
                    KFSENTR031.lngPAGE = 1                                                '(未入力時は1をｾｯﾄ)
                Else
                    KFSENTR031.lngPAGE = GCom.NzLong(txtKaisiPage.Text)
                End If
                KFSENTR031.BERYFAI = BERYFAI
            End If
            ''必要項目を変数にセットする
            ...
            '2018/02/09 saitou 広島信金(RSV2標準) UPD エントリ20明細対応 -------------------- START
            If ShowFormFlg = "1" Then
                Call CASTCommon.ShowFORM(GCom.GetUserID, CType(KFSENTR032, Form), Me)
            Else
                Call CASTCommon.ShowFORM(GCom.GetUserID, CType(KFSENTR031, Form), Me)
            End If
            'Call CASTCommon.ShowFORM(gstrUSER_ID, CType(KFSENTR031, Form), Me)
            '2018/02/09 saitou 広島信金(RSV2標準) UPD --------------------------------------- END
        Catch ex As Exception
...
#Region " 振込日変更"
    ...
    '    'Parameter      :
    '    'Description    :振込日変更ボタン
    '    'Return         :
    '    'Create         :2010/02/05
    '    'Update         :
    '    '=====================================================================================
    '    Dim KFSMAST031 As KFSMAST031
    '    Try
    '        MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(振込日変更)開始", "成功", "")
    '        KFSMAST031 = New KFSMAST031
    '        If KFSMAST031.fn_check_text(txtTorisCode, txtTorifCode, txtFuriDateY, txtFuriDateM, txtFuriDateD) = False Then
    '            Return
    '        End If
    '        If KFSMAST031.fn_check_Table(txtTorisCode, txtTorifCode, txtFuriDateY, txtFuriDateM, txtFuriDateD, True) = False Then
    '            Return
    '        End If
    ...
    '    Catch ex As Exception
    '        MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(振込日変更)例外", "失敗", ex.Message)
    '    Finally
    '        MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(振込日変更)終了", "成功", "")
    '    End Try
    'End Sub
#End Region
End Class