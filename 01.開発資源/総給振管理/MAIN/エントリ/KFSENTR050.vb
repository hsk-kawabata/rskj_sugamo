...
Public Class KFSENTR050
    Private CAST As New CASTCommon.Events
    Private CASTx01 As New CASTCommon.Events("0-9", CASTCommon.Events.KeyMode.GOOD)

    Private strBAITAI_CODE As String

    Private MainLOG As New CASTCommon.BatchLOG("KFSENTR050", "総合振込入力チェックリスト印刷画面")
    Private Const msgTitle As String = "総合振込入力チェックリスト印刷画面(KFSENTR050)"
...
#Region " ロード"
    Private Sub KFSENTR050_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            'ログの書込に必要な情報の取得
...
#Region " 印刷"
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        Try
            ...
            '印刷前確認メッセージ
            If MessageBox.Show(String.Format(MSG0013I, "総合振込入力チェックリスト"), _
            ...
            'パラメータ設定：ログイン名、取引先主副コード,振込日,媒体コード,ソート順
            ...
            nRet = ExeRepo.ExecReport("KFSP023.EXE", param)

            '戻り値に対応したメッセージを表示する
            Select Case nRet
                Case 0
                    '印刷後確認メッセージ
                    MessageBox.Show(String.Format(MSG0014I, "総合振込入力チェックリスト"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Case -1
                    '印刷対象なしメッセージ
                    MessageBox.Show(String.Format(MSG0106W, "総合振込入力チェックリスト"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Case Else
                    '印刷失敗メッセージ
                    MessageBox.Show(String.Format(MSG0004E, "総合振込入力チェックリスト"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Select
...
#Region " 関数"
    ...
    Function fn_check_Denpyo() As Boolean
        ...
        Try
            ...
            SQL = New StringBuilder(128)
            SQL.Append("SELECT COUNT(*) COUNTER ")
            SQL.Append(" FROM S_TORIMAST,S_DENPYOMAST")
            SQL.Append(" WHERE FSYORI_KBN_E ='3'")
            If txtTorisCode.Text + txtTorifCode.Text <> "222222222222" Then
                SQL.Append(" AND TORIS_CODE_T = " & SQ(Trim(txtTorisCode.Text)))
                SQL.Append(" AND TORIF_CODE_T = " & SQ(Trim(txtTorifCode.Text)))
