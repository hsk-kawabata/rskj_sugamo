...
Public Class KFSENTR020
    Private CAST As New CASTCommon.Events
    Private CASTx01 As New CASTCommon.Events("0-9", CASTCommon.Events.KeyMode.GOOD)

    Private strBAITAI_CODE As String

    Private MainLOG As New CASTCommon.BatchLOG("KFSENTR020", "総合振込依頼書印刷画面")
    Private Const msgTitle As String = "総合振込依頼書印刷画面(KFSENTR020)"
...
#Region " ロード"
    Private Sub KFSENTR020_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            'ログの書込に必要な情報の取得
            ...
            '=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
            Dim Msg As String
            Select Case GCom.SetComboBox(cmbKbn, "KFSENTR020_出力区分.TXT", True)
                Case 1  'ファイルなし
                    Msg = "出力区分の設定に失敗しました。" & vbCrLf & _
                          "テキストファイル:KFSENTR020_出力区分.TXTが存在しません。"
                    MessageBox.Show(Msg, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Msg = "出力区分設定ファイルなし。ファイル:KFSENTR020_出力区分.TXT"
...
#Region " 印刷"
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        Try
            ...
            '印刷前確認メッセージ
            If MessageBox.Show(String.Format(MSG0013I, "総合振込依頼書"), _
            ...
            'パラメータ設定：ログイン名、取引先主副コード、振込日、依頼書種別
            param = GCom.GetUserID & "," & strTORIS_CODE & strTORIF_CODE & "," & strFuriDate & "," & GCom.GetComboBox(cmbKbn)
            nRet = ExeRepo.ExecReport("KFSP022.EXE", param)

            '戻り値に対応したメッセージを表示する
            Select Case nRet
                Case 0
                    '印刷後確認メッセージ
                    MessageBox.Show(String.Format(MSG0014I, "総合振込依頼書"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Case -1
                    '印刷対象なしメッセージ
                    MessageBox.Show(String.Format(MSG0106W, "総合振込依頼書"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Case Else
                    '印刷失敗メッセージ
                    MessageBox.Show(String.Format(MSG0004E, "総合振込依頼書"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Select
