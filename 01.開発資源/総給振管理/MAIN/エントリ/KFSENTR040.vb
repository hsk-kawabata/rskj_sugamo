...
Public Class KFSENTR040
    Private CAST As New CASTCommon.Events
    Private CASTx01 As New CASTCommon.Events("0-9", CASTCommon.Events.KeyMode.GOOD)

    Private strBAITAI_CODE As String
    Private Jikinko As String = ""  '自金庫コード
    Private MainLOG As New CASTCommon.BatchLOG("KFSENTR040", "伝票用データ入力画面")
    Private Const msgTitle As String = "伝票用データ入力画面(KFSENTR040)"
...
#Region " ロード"
    Private Sub KFSENTR040_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            'ログの書込に必要な情報の取得
...
#Region " 実行"
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            Dim KFSENTR041 As KFSENTR041 = Nothing
            Dim KFSENTR042 As KFSENTR042 = Nothing
            'Dim KFSENTR041 As New KFSENTR041
            ...
            '2018/02/09 saitou 広島信金(RSV2標準) UPD エントリ20明細対応 -------------------- START
            Dim ShowFormFlg As String = CASTCommon.GetRSKJIni("RSV2_V1.0.0", "KFSENTR040_SHOWFORM")
            If ShowFormFlg = "1" Then
                KFSENTR042 = New KFSENTR042
                '必要項目を変数にセットする
                KFSENTR042.strTORIS_CODE = strKEY1
                KFSENTR042.strTORIF_CODE = strKEY2
                KFSENTR042.strFURI_DATE = strKEY3
                KFSENTR042.strSYOKITI_KIN = txtKinSyokiti.Text.Trim
                KFSENTR042.strSYOKITI_SIT = txtSitSyokiti.Text.Trim
                If txtSyokiti.Text = "" Then
                    KFSENTR042.strSYOKITI = "0"
                Else
                    KFSENTR042.strSYOKITI = fn_DEL_KANMA(txtSyokiti.Text.Trim).ToString
                End If
                ...
                If txtKaisiPage.Text.Trim = "" Then                                  '開始ﾍﾟｰｼﾞ
                    KFSENTR042.lngPAGE = 1                                                '(未入力時は1をｾｯﾄ)
                Else
                    KFSENTR042.lngPAGE = GCom.NzLong(txtKaisiPage.Text)
                End If
                KFSENTR042.BERYFAI = BERYFAI
            Else
                KFSENTR041 = New KFSENTR041
                '必要項目を変数にセットする
                KFSENTR041.strTORIS_CODE = strKEY1
                KFSENTR041.strTORIF_CODE = strKEY2
                KFSENTR041.strFURI_DATE = strKEY3
                KFSENTR041.strSYOKITI_KIN = txtKinSyokiti.Text.Trim
                KFSENTR041.strSYOKITI_SIT = txtSitSyokiti.Text.Trim
                If txtSyokiti.Text = "" Then
                    KFSENTR041.strSYOKITI = "0"
                Else
                    KFSENTR041.strSYOKITI = fn_DEL_KANMA(txtSyokiti.Text.Trim).ToString
                End If
                If rdbSei.Checked Then
                    KFSENTR041.intCHK = 0
                Else
                ...
                End If
                If txtKaisiPage.Text.Trim = "" Then                                  '開始ﾍﾟｰｼﾞ
                    KFSENTR041.lngPAGE = 1                                                '(未入力時は1をｾｯﾄ)
                Else
                    KFSENTR041.lngPAGE = GCom.NzLong(txtKaisiPage.Text)
                End If
                KFSENTR041.BERYFAI = BERYFAI
            End If
            ''必要項目を変数にセットする
            ...
            '    gstrSYOKITI = "0"
            'Else
            '    gstrSYOKITI = fn_DEL_KANMA(txtSyokiti.Text.Trim).ToString
            'End If
            'If rdbSei.Checked Then
            '    gintCHK = 0
            'Else
            ...
            '2018/02/09 saitou 広島信金(RSV2標準) UPD エントリ20明細対応 -------------------- START
            If ShowFormFlg = "1" Then
                Call CASTCommon.ShowFORM(GCom.GetUserID, CType(KFSENTR042, Form), Me)
            Else
                Call CASTCommon.ShowFORM(GCom.GetUserID, CType(KFSENTR041, Form), Me)
            End If
            'Call CASTCommon.ShowFORM(gstrUSER_ID, CType(KFSENTR041, Form), Me)
            '2018/02/09 saitou 広島信金(RSV2標準) UPD --------------------------------------- END
        Catch ex As Exception
    ...
    Function fn_Delete_DenMast() As Boolean
        ...
        Try
            ...
            If rdbSei.Checked Then  '正(伝票マスタ)
                SQL.Append(" FROM S_DENPYOMAST")
            Else                    '副(伝票副マスタ)
                SQL.Append(" FROM S_FUKU_DENPYOMAST")
            End If
            SQL.Append(" WHERE FSYORI_KBN_E ='3'")
            SQL.Append(" AND TORIS_CODE_E = " & SQ(strKEY1))
            SQL.Append(" AND TORIF_CODE_E = " & SQ(strKEY2))
            ...
            SQL = New StringBuilder(128)
            If rdbSei.Checked Then  '正(伝票マスタ)
                SQL.Append("DELETE  FROM S_DENPYOMAST")
            Else                    '副(伝票副マスタ)
                SQL.Append("DELETE  FROM S_FUKU_DENPYOMAST")
            End If
