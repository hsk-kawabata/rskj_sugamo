...
Public Class KFSMAIN070

#Region " 宣言 "

    Private MainLog As New CASTCommon.BatchLOG("KFSMAIN070", "依頼データ落込（一括）画面")
    Private Const msgTitle As String = "依頼データ落込（一括）画面(KFSMAIN070)"
...
#Region " ロード "
    ...
    Private Sub KFSMAIN070_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            ' コンボボックス設定
            '------------------------------------------
            Select Case GCom.SetComboBox(cmbBaitai, "KFSMAIN070_媒体.TXT", True)
                Case 1  'ファイルなし
                    MessageBox.Show(String.Format(MSG0025E, "媒体", "KFSMAIN070_媒体.TXT"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    MainLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "ロード", "失敗", "ファイルなし。ファイル:KFSMAIN070_媒体.TXT")
                Case 2  '異常
                    ...
                    If SubFileName(1) = SelectBaitaiKCode Then
                        ...
                        If GetMastInfo(MainDB, GyomuCode, SubFileName(2), SubFileName(3), SubFileName(5), _
                            ...
                            Dim TourokuParam As String = TorisCode & TorifCode & "," & _
                                                         ...
                                                         SelectBaitaiCode & "," & _
                                                         LabelKbn & ",," & i
                            If SetJobMast(MainDB, "S010", TourokuParam) = False Then
    ...
    Private Function GetMastInfo(ByVal MainDB As CASTCommon.MyOracle, ByVal GyomuCode As String, ByVal MultiKbn As String, ByVal SearchCode As String, ByVal FuriDate As String, _
        ...
        Try
            ...
            If OraReader.DataReader(SQL) = False Then
                ...
                MainLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "取引先、スケジュール取得", "失敗", "スケジュールマスタ検索失敗　代表委託者コード:" & SearchCode.Substring(0, 10) & " 振込日:" & FuriDate)
                Return False
            Else
                TorisCode = OraReader.GetString("TORIS_CODE_T")
                TorifCode = OraReader.GetString("TORIF_CODE_T")
