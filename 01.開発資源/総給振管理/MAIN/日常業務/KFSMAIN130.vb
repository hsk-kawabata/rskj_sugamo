...
Public Class KFSMAIN130
    ...
    Private Const msgTitle As String = "未照合状況画面(KFSMAIN130)"
    Private MainLOG As New CASTCommon.BatchLOG("KFSMAIN130", "未照合状況画面")
    ...
    '画面起動イベント処理
    Private Sub KFSMAIN130_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '--------------------------------------------------
            'ログの書込に必要な情報の取得
            '--------------------------------------------------
            ...
            Call GetCodeName(BAITAI_CODE_INF, "Common_総振_媒体コード.TXT")
            Call GetCodeName(SYUBETU_CODE_INF, "KFSMAST010_種別.TXT")
            Call GetCodeName(CHECK_KBN_INF, "KFSMAST011_照合要否区分.TXT")
            Call GetCodeName(CHECK_FLG_INF, "KFSMAIN130_照合結果.TXT")
            ...
            With ListView
                ...
                Select Case ListView.Name.ToUpper
                    Case "ListView1".ToUpper
                        ...
                        .Columns.Add("振込日", 80, HorizontalAlignment.Center)
                    ...
                    Case "ListView2".ToUpper
                        ...
                        .Columns.Add("落とし込み日", 0, HorizontalAlignment.Center)
    ...
    Private Sub CmdUkeUpdate_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdUkeUpdate.Click
        Try
            ...
            '2014/08/05 saitou 標準版 照合対応 UPD -------------------------------------------------->>>>
            '大規模版と仕様を変える
            '大規模版は別画面に遷移するようになっていたが、RSは送付状登録情報変更のグループボックスを出す
            'Dim onForm As New KFJUKT0210G
            'With onForm
    ...
    'スケジュール変更ボタン処理
    Private Sub CmdSchduleUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSchduleUpdate.Click
        'Try
        '    Dim MSG As String
        '    If GCom.NzInt(GCom.SelectedItem(ListView2, 16), 0) = 3 Then

        '        MSG = "振込のデータは当該画面からスケジュール変更画面へ直接遷移できません。" & Space(5)
        '        Dim DRet As DialogResult = MessageBox.Show(MSG, GCom.GLog.Job2, _
        '                MessageBoxButtons.OK, MessageBoxIcon.Information)
        '        Return
        '    End If

        '    MSG = "スケジュール変更画面へ遷移します。よろしいですか？"
        '    Dim StrData(8) As String
        '    If CheckMessage(MSG, StrData) = DialogResult.OK Then

        '        GCom.GLog.Job2 = "スケジュールマスタ登録"
        '        Dim onForm As New KFJMAST0300G
        '        With onForm
        '            .Show()
        '            .ITAKU_CODE_S.Text = StrData(3)
        '            .TORIS_CODE_S.Text = StrData(4)
        '            .TORIF_CODE_S.Text = StrData(5)
        '            .ITAKU_NNAME_T.Text = StrData(0)
        '            .FURI_DATE_S.Text = StrData(6).Replace("."c, "").Substring(0, 4)
        '            .FURI_DATE_S1.Text = StrData(6).Replace("."c, "").Substring(4, 2)
        '            .FURI_DATE_S2.Text = StrData(6).Replace("."c, "").Substring(6)
    ...
    '依頼データ補正ボタン処理
    Private Sub CmdIraiDataUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdIraiDataUpdate.Click
        'Try
        '    Dim ExeName As String = GCom.SET_PATH(GCom.GetDLLFolder) & "Tflin32.exe"

        '    If Not File.Exists(ExeName) Then
        '        Throw New Exception(String.Format("SIMPLIA/TF-LINDA実行ファイルがありません" _
        '                                    & ControlChars.Cr & "'{0}'", ExeName))
        '    End If

        '    '処理ファイルのパス取得  
        '    Dim sLindaArg As String = "D:\FSKJ\TEST-DATA\0000000108.20080108"

        '    If Not File.Exists(sLindaArg) Then
        '        Throw New Exception(String.Format("補正対象ファイルがありません" _
        '                                    & ControlChars.Cr & "'{0}'", sLindaArg))
        '    End If

        '    Dim ExecProc As Process = Process.Start(ExeName, sLindaArg)    ' LINDA起動
        '    If ExecProc.Id <> 0 Then
        '    Else
        '        Throw New Exception(String.Format("LINDAの起動に失敗しました" _
        '                                    & ControlChars.Cr & "'{0}'", ExeName))
        '    End If
        '    ExecProc.WaitForExit()    ' LINDA終了まで待機

        'Catch ex As Exception
        '    With GCom.GLog
        '        .Job2 = "依頼データ補正ボタン処理"
        '        .Result = GCom.NG
        '        .Discription = ex.Message.Replace(ControlChars.Cr, "")
        '    End With
        '    Call GCom.FN_LOG_WRITE(ThisModuleName, New StackTrace(True))
        '    Dim DRet As DialogResult = MessageBox.Show(ex.Message, _
        '        GCom.GLog.Job2, MessageBoxButtons.OK, MessageBoxIcon.Warning)
        'End Try
    End Sub

    '依頼データ再登録ボタン処理
    Private Sub CmdIraiDataEntry_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdIraiDataEntry.Click
        'Dim REC As OracleDataReader
        'Try
        '    Dim MSG As String = "依頼データを再登録します。よろしいですか？"
        '    Dim StrData(8) As String
        '    If CheckMessage(MSG, StrData) = DialogResult.OK Then

        '        Dim TORI_CODE As String = GCom.NzDec(GCom.SelectedItem(ListView2, 6), "")
        '        TORI_CODE &= GCom.NzDec(GCom.SelectedItem(ListView2, 7), "")

        '        Dim FURI_DATE As String = GCom.NzDec(GCom.SelectedItem(ListView2, 10), "")
        '        Dim FMT_KBN As String = GCom.NzDec(GCom.SelectedItem(ListView2, 20), "")
        '        Dim BAITAI_CODE As String = GCom.NzDec(GCom.SelectedItem(ListView2, 21), "")
        '        Dim CODE_KBN As String = GCom.NzDec(GCom.SelectedItem(ListView2, 22), "")

        '        Dim CmdLine As String = TORI_CODE
        '        CmdLine &= "," & FURI_DATE
        '        CmdLine &= "," & CODE_KBN
        '        CmdLine &= "," & FMT_KBN
        '        CmdLine &= "," & BAITAI_CODE
        ...
        '        '登録済みチェック
        '        Dim SQL As String = "SELECT * FROM JOBMAST"
        '        SQL &= " WHERE TOUROKU_DATE_J = TO_CHAR(SYSDATE, 'YYYYMMDD')"
        '        SQL &= " AND JOBID_J = 'T101'"
        '        SQL &= " AND STATUS_J IN ('0', '1')"
        '        SQL &= " AND PARAMETA_J = '" & CmdLine & "'"
        ...
        '        End If
        '        If Counter > 0 Then
        '            Throw New Exception(String.Format("バッチジョブ管理マスタに登録済です" _
        '                                    & ControlChars.Cr & "'{0}'", CmdLine))
        '        End If
        ...
        '        SQL &= " (TUUBAN_J"             '必須(NUMBER)
        '        SQL &= ", TOUROKU_DATE_J"       '必須(以下CHAR)
        '        SQL &= ", TOUROKU_TIME_J"
        '        SQL &= ", STA_DATE_J"
        '        SQL &= ", STA_TIME_J"
        '        SQL &= ", END_DATE_J"
        ...
        '        SQL &= ", ERRLEBEL_J"
        '        SQL &= ", ERRMSG_J"
        '        SQL &= ", DENSO_CNT_CODE_J"
        '        SQL &= ", ZENGIN_F_NAME_J"
        '        SQL &= ", DENSO_F_NAME_J"
        '        SQL &= ", DENSO_TIME_STAMP_J"
        '        SQL &= ")"
        '        SQL &= " VALUES"
        '        SQL &= " ((SELECT NVL(MAX(TUUBAN_J), 0) + 1"
        '        SQL &= " FROM JOBMAST"
        '        SQL &= " WHERE TOUROKU_DATE_J = TO_CHAR(SYSDATE, 'YYYYMMDD'))"  'TUUBAN_J
        '        SQL &= ", TO_CHAR(SYSDATE, 'YYYYMMDD')"                         'TOUROKU_DATE_J
        '        SQL &= ", TO_CHAR(SYSDATE, 'HH24MISS')"                         'TOUROKU_TIME_J
        '        SQL &= ", ' '"                                                  'STA_DATE_J
        '        SQL &= ", ' '"                                                  'STA_TIME_J
        '        SQL &= ", ' '"                                                  'END_DATE_J
        '        SQL &= ", ' '"                                                  'END_TIME_J
        '        SQL &= ", 'T101'"                                               'JOBID_J
        '        SQL &= ", '0'"                                                  'STATUS_J
        '        SQL &= ", '" & GCom.GetUserID & "'"                             'USERID_J
        '        SQL &= ", '" & CmdLine & "'"                                    'PARAMETA_J
        '        SQL &= ", ' '"                                                  'ERRLEBEL_J
        '        SQL &= ", ' '"                                                  'ERRMSG_J
        '        SQL &= ", ' '"                                                  'DENSO_CNT_CODE_J
        '        SQL &= ", ' '"                                                  'ZENGIN_F_NAME_J
        ...
        '            With GCom.GLog
        '                .ToriCode = TORI_CODE.Substring(0, 7) & "-" & TORI_CODE.Substring(7)
        '                .FuriDate = FURI_DATE
        '                .Job2 = "依頼データ再登録ボタン処理"
        '                .Result = GCom.OK
        '                .Discription = ""
        '            End With
        '            Call GCom.FN_LOG_WRITE(ThisModuleName, New StackTrace(True))
        '            MSG = "起動パラメタを登録しました。" & Space(5)
        '            Dim DRet As DialogResult = MessageBox.Show(MSG, _
        '                        GCom.GLog.Job2, MessageBoxButtons.OK, MessageBoxIcon.Information)
        '        Else
        '            Throw New Exception(String.Format("バッチジョブ管理マスタ登録失敗" _
        '                                                        & ControlChars.Cr & "'{0}'", CmdLine))
        '        End If
        '    End If
        'Catch ex As Exception
    ...
    'Private Sub KFJOTHER0910MG_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
    ...
    Private Sub CmdUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdUpdate.Click
        Try
            ...
            Try
                ...
                'If Ret = 1 AndAlso Not NEW_FURI_DATE = DT.FURI_DATE Then
                '    '振替日が変更された場合には入庫実績TBLも更新する。

                '    SQL = "UPDATE MEDIA_IN_TBL"
                '    SQL &= " SET FURI_DATE = '" & NEW_FURI_DATE & "'"
                '    SQL &= ", UPDATE_DATE = TO_CHAR(SYSDATE, 'yyyymmdd')"
                '    With DT
                '        SQL &= " WHERE BAITAI_KANRI_CODE = '" & .BAITAI_KANRI_CODE & "'"
                '        SQL &= " AND IN_DATE = '" & .IN_DATE & "'"
                '        SQL &= " AND STATION_IN_NO = '" & .STATION_IN_NO & "'"
                '        SQL &= " AND IN_COUNTER = '" & .IN_COUNTER & "'"
                '    End With

                '    Ret = GCom.DBExecuteProcess(GCom.enDB.DB_Execute, SQL, SQLCode)
                'End If
                ...
                Select Case SQLCode
                    Case 0
                        Try
                            ...
                            'Call GCom.SelectedItem(ListView, 14, NEW_SYUBETU_CODE)      '種目コード
                            'Call GCom.SelectedItem(ListView, 15, Me.CmbShurui.Text)     '種目名称
