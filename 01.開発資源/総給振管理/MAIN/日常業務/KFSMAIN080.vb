...
#Region " 定数/変数 "
    ...
    ' LOG関連項目
    '--------------------------------
    Private MainLOG As New CASTCommon.BatchLOG("KFSMAIN080", "振込発信リエンタ書込画面")
    Private Const msgtitle As String = "振込発信リエンタ書込画面(KFSMAIN080)"
    ...
    Friend Structure INI_INFO
        ...
        Dim KAWASE_RIENTANAME As String              ' リエンタファイル名
    End Structure
...
#Region " ロード "
    ...
    Private Sub KFSMAIN080_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region " ボタン "
    ...
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            If MessageBox.Show(String.Format(MSG0077I, "振込発信リエンタ", "媒体書込処理"), msgtitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.Cancel Then
                Exit Try
            End If
            ...
            FileName = "RNT_FH_" & TimeStamp_Date & "_" & TimeStamp_Time & "_"
            ...
            For i As Integer = 0 To FileList.Length - 1 Step 1
                ...
                If SetFileName.Length = 5 Then
                    If SetFileName(0) = "RNT" And _
                       SetFileName(1) = "FH" And _
                       SetFileName(2) = TimeStamp_Date And _
                       SetFileName(3) = TimeStamp_Time Then
                        FileCount += 1
                    End If
                End If
            Next
            ...
            If FileCount > 1 Then
                MessageBox.Show("振込発信リエンタファイルが " & FileCount & " ファイル存在します。" & vbCrLf & _
                                "媒体の準備をお願いします。" _
            ...
            For i As Integer = 1 To FileCount Step 1
                '複数枚の場合はメッセージ表示
                If i > 1 Then
                    MessageBox.Show(String.Format(MSG0079I, "媒体(" & i.ToString & "本目)", "ドライブ(" & Path.GetPathRoot(IniInfo.COMMON_RIENTADR) & ")"), _
                                    msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
                ...
                Do
                    ...
                    Catch ex As Exception
                        ...
                        If FileCount = 1 Then
                            Message = String.Format(MSG0079I, "媒体", "ドライブ(" & Path.GetPathRoot(IniInfo.COMMON_RIENTADR) & ")")
                        Else
                            Message = String.Format(MSG0079I, "媒体(" & i.ToString & "本目)", "ドライブ(" & Path.GetPathRoot(IniInfo.COMMON_RIENTADR) & ")")
                        End If
                        If MessageBox.Show(Message, msgtitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Information) <> DialogResult.OK Then
                ...
                Select Case iRet
                    Case 0          ' データ格納成功
                        If File.Exists(Path.Combine(IniInfo.COMMON_RIENTADR, IniInfo.KAWASE_RIENTANAME)) Then
                        ...
                        If iRet = 0 Then
                            File.Copy(Path.Combine(IniInfo.COMMON_BAITAIWRITE, FileName & i.ToString), Path.Combine(IniInfo.COMMON_RIENTADR, IniInfo.KAWASE_RIENTANAME), True)
                        End If
                End Select
            Next
            ...
            '------------------------------------------------
            If iRet = 0 Then
                MessageBox.Show(String.Format(MSG0078I, "振込発信リエンタの媒体書込"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "実行", "成功", "出力ファイル:" & IniInfo.COMMON_BAITAIWRITE & FileName)
            Else
                MessageBox.Show(String.Format(MSG0080I, "振込発信リエンタの媒体書込", "中断"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
...
#Region " 関数(Function)"
    ...
    Private Function SetIniFIle() As Boolean
        ...
        Try
            ...
            IniInfo.KAWASE_RIENTANAME = CASTCommon.GetFSKJIni("KAWASE", "RIENTANAME")
            If IniInfo.KAWASE_RIENTANAME.ToUpper = "ERR" OrElse IniInfo.KAWASE_RIENTANAME = Nothing Then
                MessageBox.Show(String.Format(MSG0001E, "リエンタファイル名", "KAWASE", "RIENTANAME"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "INI情報取得", "失敗", "項目名:リエンタファイル名 分類:KAWASE 項目:RIENTANAME")
                Return False
            End If
