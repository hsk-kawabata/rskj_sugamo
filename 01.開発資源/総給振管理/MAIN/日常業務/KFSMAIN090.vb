...
#Region " 変数宣言"
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFSMAIN090", "振込発信データ作成（通常定例）画面")
    Private Const msgTitle As String = "振込発信データ作成（通常定例）画面(KFSMAIN090)"
    ...
    Private UKETUKE_DATE As String              '受付日
...
#End Region
    ...
    '画面LOAD時
    Private Sub KFSMAIN090_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            dRet = MessageBox.Show(MSG0023I.Replace("{0}", "振込発信データ作成"), msgTitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question)
            ...
            para = UKETUKE_DATE & ",1"                 ' "受付日","1(通常定例)" をパラメタとして設定
            ...
            MessageBox.Show(MSG0021I.Replace("{0}", "振込発信データ作成(通常定例)"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub btnRef_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRef.Click
        ...
        Try
            ...
            UKETUKE_DATE = txtKijyunDateY.Text & txtKijyunDateM.Text & txtKijyunDateD.Text

            '１／２営業日後
            GCom.CheckDateModule(UKETUKE_DATE, EIGYOUBI_1_AFTER, 1, 0)
            GCom.CheckDateModule(UKETUKE_DATE, EIGYOUBI_2_AFTER, 2, 0)
    ...
    Private Sub btnClear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClear.Click
        ...
        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(クリア)", "失敗", ex.ToString)
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            mRet = MessageBox.Show(MSG0013I.Replace("{0}", "振込発信データ対象一覧"), msgTitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question)
            ...
            For Each item As ListViewItem In nItems
                CreateCSV.OutputCsvData(UKETUKE_DATE)                                   '受付日
            ...
            'パラメータ変更　ログイン名、ＣＳＶファイル名、"4(通常定例)"
            param = GCom.GetUserID & "," & strCSVFileName & ",4"
            ...
            Select Case iRet
                Case 0
                    '印刷後確認メッセージ
                    MessageBox.Show(String.Format(MSG0014I, "振込発信データ対象一覧"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Case -1
                    '印刷対象なしメッセージ
                    MessageBox.Show(MSG0106W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "振込発信データ対象一覧印刷", "失敗", "印刷対象なし")
                Case Else
                    '印刷失敗メッセージ
                    MessageBox.Show(String.Format(MSG0004E, "振込発信データ対象一覧"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "振込発信データ対象一覧印刷", "失敗")
            End Select

        Catch ex As Exception
            Call MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(印刷)例外エラー", "失敗", ex.ToString)
        Finally
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(印刷)開始", "成功", "")
        End Try
...
#Region " 関数"
    ...
    Private Function HassinDataList() As Integer
        ...
        Try
            ...
            SQL.AppendLine("     FSYORI_KBN_S      = '3'")
            SQL.AppendLine(" AND UKETUKE_DATE_S   <= '" & UKETUKE_DATE & "'")
            SQL.AppendLine(" AND FURI_DATE_S       > '" & UKETUKE_DATE & "'")
            SQL.AppendLine(" AND TOUROKU_FLG_S     = '1'")
            SQL.AppendLine(" AND HASSIN_FLG_S      = '0'")
            ...
            If OraReader.DataReader(SQL) = True Then
                ...
                Do While OraReader.EOF = False
                    ...
                    '契約種別
                    Dim KYUUYO_KBN As String = OraReader.GetString("KYUUYO_KBN_T")
                    Select Case OraReader.GetString("SYUBETU_S")
                        Case "11"
                            If koukin Then
                                Select Case KYUUYO_KBN
                                    Case "1"
                                        Data(4) = "摘要給与(公金)"
                                    Case "2"
                                        Data(4) = "種目給与(公金)"
                    ...
                    Select Case furidate
                        Case EIGYOUBI_1_AFTER
                            Data(5) = "振込"
                        Case EIGYOUBI_2_AFTER
                            Select Case KYUUYO_KBN
                                Case "1", "3"
                                    Data(5) = "振込"
                                Case Else
        ...
        Catch ex As Exception
            ...
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(データ検索)", "失敗", ex.ToString)
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return -1
