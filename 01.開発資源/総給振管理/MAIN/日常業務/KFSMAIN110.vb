...
''' <summary>
''' MT代行発信データ作成画面　メインクラス
''' </summary>
''' <remarks>
''' 2016/10/12 saitou added for RSV2対応(信組)
''' 笠岡信組ベースでRSV2に標準組込み
''' </remarks>
Public Class KFSMAIN110
...
#Region "クラス定数"
    ...
    'メッセージ
    Private Const msgTitle As String = "MT代行発信データ作成画面(KFSMAIN110)"
...
#Region "クラス変数"
    ...
    'イベントログ
    Private MainLOG As New CASTCommon.BatchLOG("KFSMAIN110", "MT代行発信データ作成画面")
...
#Region "イベントハンドラ"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub KFSMAIN110_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            '--------------------------------------------------
            'ログ書き込み変数の初期化
            '--------------------------------------------------
            ...
            bRet = GCom.CheckDateModule(strSysDate, strGetDate, 1, 0)
            Me.txtKijyunDateY.Text = strGetDate.Substring(0, 4)
            Me.txtKijyunDateM.Text = strGetDate.Substring(4, 2)
            Me.txtKijyunDateD.Text = strGetDate.Substring(6, 2)

            '給振／総振範囲
            Me.KYUFURI_HANI = GCom.NzInt(CASTCommon.GetFSKJIni("KAWASE", "KYUFURI"), 0)
            Me.SOUFURI_HANI = GCom.NzInt(CASTCommon.GetFSKJIni("KAWASE", "SOUFURI"), 0)
    ...
    Private Sub btnRef_Click(sender As Object, e As EventArgs) Handles btnRef.Click
        Try
            ...
            '１／２営業日後
            GCom.CheckDateModule(Me.HASSIN_DATE, Me.EIGYOUBI_1_AFTER, 1, 0)
            GCom.CheckDateModule(Me.HASSIN_DATE, Me.EIGYOUBI_2_AFTER, 2, 0)

            '総給振範囲取得
            GCom.CheckDateModule(Me.HASSIN_DATE, Me.KYUFURI_END_DATE, Me.KYUFURI_HANI, 0)
            GCom.CheckDateModule(Me.HASSIN_DATE, Me.SOUFURI_END_DATE, Me.SOUFURI_HANI, 0)
    ...
    Private Sub btnAction_Click(sender As Object, e As EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            If MessageBox.Show(String.Format(MSG0023I, "MT代行発信データ作成"), msgTitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) <> Windows.Forms.DialogResult.OK Then
                Return
            End If
            ...
            jobid = "S110"
            para = Me.HASSIN_DATE                 '発信日をパラメタとして設定
            para &= "," & Me.KYUFURI_END_DATE
            para &= "," & Me.SOUFURI_END_DATE
            ...
            MessageBox.Show(String.Format(MSG0021I, "MT代行発信データ作成"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            If MessageBox.Show(String.Format(MSG0013I, "MT代行発信データ対象一覧"), msgTitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) <> Windows.Forms.DialogResult.OK Then
                Return
            End If
            ...
            'パラメータ設定：ユーザーID,CSVファイル名,2(MT代行)
            param = GCom.GetUserID & "," & strCSVFileName & ",2"
            iRet = ExeRepo.ExecReport("KFSP009.EXE", param)

            Select Case iRet
                Case 0
                    '印刷後確認メッセージ
                    MessageBox.Show(String.Format(MSG0014I, "MT代行発信データ対象一覧"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Case -1
                    '印刷対象なしメッセージ
                    MessageBox.Show(MSG0106W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "MT代行発信データ対象一覧印刷", "失敗", "印刷対象なし")
                Case Else
                    '印刷失敗メッセージ
                    MessageBox.Show(String.Format(MSG0004E, "MT代行発信データ対象一覧"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "MT代行発信データ対象一覧印刷", "失敗")
            End Select
    ...
    Private Function HassinDataList() As Integer
        ...
        Try
            With SQL
                ...
                .Append(" and FURI_DATE_S >= '" & Me.HASSIN_DATE & "'")
                .Append(" and TYUUDAN_FLG_S = '0'")
                .Append(" and SOUSIN_KBN_S = '2'")          'CSVリエンタ
                .Append(" and (")
                .Append(" (FURI_DATE_S <= '" & Me.KYUFURI_END_DATE & "' and SYUBETU_S in ('11', '12'))")
                .Append("  or (FURI_DATE_S <= '" & Me.SOUFURI_END_DATE & "' and SYUBETU_S = '21')")
                .Append(" )")
                .Append(" order by")
                .Append(" FURI_DATE_S")
                .Append(",SOUSIN_KBN_S")
