...
#Region "変数宣言"
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFSMAST042", "スケジュール管理画面")
    Private Const msgTitle As String = "スケジュール管理画面(KFSMAST042)"
...
#End Region
    ...
    '画面起動イベント処理
    Private Sub KFSMAST042_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            SORT_KEY = GCom.GetObjectParam("KFSMAST041", "SORT", "0")
            ...
            With ListView1
                ...
                .Columns.Add("確保", 40, HorizontalAlignment.Center)                '9
                .Columns.Add("発信", 40, HorizontalAlignment.Center)                '10
                .Columns.Add("決済", 40, HorizontalAlignment.Center)                '11
                .Columns.Add("種別", 60, HorizontalAlignment.Left)                  '12
                .Columns.Add("依頼件数", 70, HorizontalAlignment.Right)             '13
                .Columns.Add("依頼金額", 120, HorizontalAlignment.Right)            '14
                .Columns.Add("媒体コード", 0, HorizontalAlignment.Right)            '15
                .Columns.Add("コード区分", 0, HorizontalAlignment.Right)            '16
                .Columns.Add("フォーマット区分", 0, HorizontalAlignment.Right)      '17
                .Columns.Add("ラベル区分", 0, HorizontalAlignment.Right)            '18
                .Columns.Add("持込SEQ", 0, HorizontalAlignment.Right)               '19
                .Columns.Add("ファイルSEQ", 0, HorizontalAlignment.Right)           '20
                .Columns.Add("代表委託者コード", 0, HorizontalAlignment.Right)      '21
                .Columns.Add("受付", 0, HorizontalAlignment.Right)                  '22
                .Columns.Add("受付日時", 140, HorizontalAlignment.Center)           '23
                .Columns.Add("", 0, HorizontalAlignment.Right)                      '24
            End With
    ...
    '画面終了時処理
    Private Sub KFSMAST042_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
        Try
    ...
    Private Sub CmdSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSelect.Click
        ...
        Try
            ...
            SQL.AppendLine(", SYOUGOU_FLG_S")
            SQL.AppendLine(", SYOUGOU_KBN_T")
            SQL.AppendLine(", UKETUKE_TIME_STAMP_S")

            SQL.AppendLine("  FROM S_SCHMAST, S_SCHMAST_SUB, S_TORIMAST_VIEW")
            ...
            SQL.AppendLine(" AND FSYORI_KBN_S = FSYORI_KBN_SSUB")
            SQL.AppendLine(" AND TORIS_CODE_S = TORIS_CODE_SSUB")
            SQL.AppendLine(" AND TORIF_CODE_S = TORIF_CODE_SSUB")
            SQL.AppendLine(" AND FURI_DATE_S  = FURI_DATE_SSUB")
            SQL.AppendLine(" AND MOTIKOMI_SEQ_S = MOTIKOMI_SEQ_SSUB")
            ...
            If GCom.SetDynaset(SQL.ToString, REC) Then
                ...
                Do While REC.Read
                    ...
                    '確保
                    ' 2017/03/29 タスク）綾部 CHG 【ME】(飯田信金では確保結果更新不要のためハイフン表示) -------------------- START
                    ...
                    'End Select
                    Data(9) = "-"
                    ' 2017/03/29 タスク）綾部 CHG 【ME】(飯田信金では確保結果更新不要のためハイフン表示) -------------------- END
                    ...
                    '決済
                    ' 2017/03/29 タスク）綾部 CHG 【ME】(飯田信金では決済不要のためハイフン表示) -------------------- START
                    ...
                    'End Select
                    Data(11) = "-"
                    ' 2017/03/29 タスク）綾部 CHG 【ME】(飯田信金では決済不要のためハイフン表示) -------------------- END

                    '種別
                    If SYUBETU_LIST.ContainsKey(SYUBETU) Then
                        Data(12) = SYUBETU_LIST.Item(SYUBETU)
                    Else
                        Data(12) = "？？"
                    End If

                    Data(13) = CLng(REC.Item("SYORI_KEN_S")).ToString("#,0")          '依頼件数
                    Data(14) = CLng(REC.Item("SYORI_KIN_S")).ToString("#,0")          '依頼金額
                    Data(15) = BAITAI_CODE                                            '媒体コード
                    Data(16) = GCom.NzDec(REC.Item("CODE_KBN_T"), "")                 'コード区分
                    Data(17) = GCom.NzDec(REC.Item("FMT_KBN_T"), "")                  'フォーマット区分
                    Data(18) = GCom.NzDec(REC.Item("LABEL_KBN_T"), "")                'ラベル区分
                    Data(19) = GCom.NzDec(REC.Item("MOTIKOMI_SEQ_S"), "")             '持込SEQ
                    Data(20) = GCom.NzDec(REC.Item("FILE_SEQ_S"), "")                 'ファイルＳＥＱ
                    Data(21) = GCom.NzDec(REC.Item("ITAKU_KANRI_CODE_T"), "")         '代表委託者コード
                    Data(22) = GCom.NzDec(REC.Item("UKETUKE_FLG_S"), "")              '受付フラグ
                    '受付タイムスタンプ
                    With GCom.NzDec(REC.Item("UKETUKE_TIME_STAMP_S"), "")
                        Data(23) = String.Format("{0}/{1}/{2} {3}:{4}:{5}", .Substring(0, 4), .Substring(4, 2), .Substring(6, 2), .Substring(8, 2), .Substring(10, 2), .Substring(12, 2))
                    End With
                    Data(24) = BAITAI_CODE                                              '媒体コード
                    Data(25) = SYUBETU                                                  '種別コード
                    Data(26) = GCom.NzDec(REC.Item("SYOUGOU_KBN_T"), "")                '照合区分
    ...
    Private Sub CmdSchPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSchPrint.Click
        ...
        Try
            ...
            For Each item As ListViewItem In nItems
                ...
                CreateCSV.OutputCsvData(item.SubItems(9).Text.Replace("/", ""), True)                   '確保
                CreateCSV.OutputCsvData(item.SubItems(10).Text.Replace("/", ""), True)                  '発信
                CreateCSV.OutputCsvData(item.SubItems(11).Text.Replace("/", ""), True)                  '決済
                CreateCSV.OutputCsvData(item.SubItems(13).Text.Replace(",", ""))                        '処理件数
                CreateCSV.OutputCsvData(item.SubItems(14).Text.Replace(",", ""), False, True)           '処理金額
    ...
    Private Sub CmdUkeDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdUkeDelete.Click
        ...
        Try
            ...
            Dim memITAKU_KANRI_CODE As String = GCom.NzDec(GCom.SelectedItem(ListView1, 21), "")
            '種別を条件に追加
            Dim memSyubetu As String = GCom.SelectedItem(ListView1, 12).ToString
            ...
            For Each Item As ListViewItem In ListView1.Items
                ...
                Dim ITAKU_KANRI_CODE As String = GCom.NzDec(Item.SubItems(21), "")
                Dim Syubetu As String = GCom.NzStr(Item.SubItems(12).Text)
                ...
                Dim BAITAI_CODE As String = GCom.NzDec(Item.SubItems(24), "")
                Dim SYUBETU_CODE As String = GCom.NzDec(Item.SubItems(25), "")
                Dim KEN As String = GCom.NzDec(Item.SubItems(13), "")
                Dim KIN As String = GCom.NzDec(Item.SubItems(14), "")
                ...
                If FURI_DATE = memFURI_DATE AndAlso MOTIKOMI_SEQ = memMOTIKOMI_SEQ AndAlso _
                    ...
                    'Dim SYUBETU_CODE As String = GCom.NzStr(Item.SubItems(12).Text)
                    ...
                    CreateCSV.OutputCsvData(Item.SubItems(13).Text.Replace(",", ""))              '処理件数
                    CreateCSV.OutputCsvData(Item.SubItems(14).Text.Replace(",", ""))              '処理金額
                    CreateCSV.OutputCsvData(MOTIKOMI_SEQ, True)                                   '持込ＳＥＱ
                    CreateCSV.OutputCsvData(Syubetu, True, True)                                  '種別
                    ...
                    If Ret = 1 AndAlso SQLCode = 0 Then
                        ...
                        SQL.Length = 0
                        SQL.AppendLine("UPDATE S_SCHMAST_SUB")
                        SQL.AppendLine(" SET SYOUGOU_FLG_S = '0'")
                        SQL.AppendLine(", SYOUGOU_DATE_S = '" & New String("0"c, 8) & "'")
                        SQL.AppendLine(", UKETUKE_TIME_STAMP_S = '" & New String("0"c, 14) & "'")
                        SQL.AppendLine(" WHERE TORIS_CODE_SSUB = '" & TORIS_CODE & "'")
                        SQL.AppendLine(" AND TORIF_CODE_SSUB = '" & TORIF_CODE & "'")
                        SQL.AppendLine(" AND FURI_DATE_SSUB = '" & FURI_DATE & "'")
                        SQL.AppendLine(" AND MOTIKOMI_SEQ_SSUB = " & MOTIKOMI_SEQ)
                        ...
                        If Ret = 1 AndAlso SQLCode = 0 Then
                            ...
                            If SQLCode = 0 Then
                                ...
                                SQL.Length = 0
                                SQL.AppendLine("UPDATE MEDIA_ENTRY_TBL")
                                SQL.AppendLine(" SET CHECK_FLG_ME = '0'")
                                SQL.AppendLine(", UPDATE_OP_ME = '" & GCom.GetUserID & "'")
                                SQL.AppendLine(", UPDATE_DATE_ME = TO_CHAR(SYSDATE, 'yyyymmddHH24MIss')")
                                SQL.AppendLine(" WHERE FSYORI_KBN_ME = '3'")
                                SQL.AppendLine(" AND TORIS_CODE_ME = '" & TORIS_CODE & "'")
                                SQL.AppendLine(" AND TORIF_CODE_ME = '" & TORIF_CODE & "'")
                                SQL.AppendLine(" AND FURI_DATE_ME = '" & FURI_DATE & "'")
                                SQL.AppendLine(" AND CYCLE_NO_ME = " & MOTIKOMI_SEQ)
                                SQL.AppendLine(" AND SYORI_KEN_ME = " & KEN)
                                SQL.AppendLine(" AND SYORI_KIN_ME = " & KIN)
                                SQL.AppendLine(" AND SYUBETU_CODE_ME = '" & SYUBETU_CODE & "'")
                                SQL.AppendLine(" AND BAITAI_CODE_ME = '" & BAITAI_CODE & "'")

                                Ret = GCom.DBExecuteProcess(MenteCommon.clsCommon.enDB.DB_Execute, SQL.ToString, SQLCode)
                                If SQLCode = 0 Then

                                    SQL.Length = 0
                                    SQL.AppendLine("DELETE FROM MATCHING_ERR_TBL")
                                    SQL.AppendLine(" WHERE FSYORI_KBN_SE = '3'")
                                    SQL.AppendLine(" AND TORIS_CODE_SE = '" & TORIS_CODE & "'")
                                    SQL.AppendLine(" AND TORIF_CODE_SE = '" & TORIF_CODE & "'")
                                    SQL.AppendLine(" AND FURI_DATE_SE = '" & FURI_DATE & "'")
                                    SQL.AppendLine(" AND CYCLE_NO_SE = " & MOTIKOMI_SEQ)

                                    Ret = GCom.DBExecuteProcess(MenteCommon.clsCommon.enDB.DB_Execute, SQL.ToString, SQLCode)
                                    If SQLCode = 0 Then
                                        Item.SubItems(7).Text = "×"
                                        Item.SubItems(13).Text = "0"
                                        Item.SubItems(14).Text = "0"

                                        If Item.SubItems(26).Text = "1" Then
       ...
       Optional ByVal button As MessageBoxDefaultButton = MessageBoxDefaultButton.Button1) As DialogResult
        ...
        Try
            ...
            Dim Data() As String = {"3", _
                        ...
                        String.Format("{0:#,##0}", GCom.NzDec(GCom.SelectedItem(ListView1, 14), 0)), _
        ...
        Try
            For Each Item As ListViewItem In ListView1.Items
                ...
                If CurrFURI_DATE = FURI_DATE AndAlso CurrMOTIKOMI_SEQ = MOTIKOMI_SEQ AndAlso CurrTORIS_CODE = TORIS_CODE AndAlso CurrTORIF_CODE = TORIF_CODE Then
                    ...
                    If Item.SubItems(10).Text.Trim = "○" Then
                        Exit Try
                    End If
                End If
            Next Item
    ...
    Private Sub cmdOtosi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdOtosi.Click
        ...
        Try
            ...
            Dim paraCodeKbn As String = GCom.NzDec(GCom.SelectedItem(ListView1, 16), "")
            Dim ParaFmtKbn As String = GCom.NzDec(GCom.SelectedItem(ListView1, 17), "")
            Dim ParaBaitaiCode As String = GCom.NzDec(GCom.SelectedItem(ListView1, 15), "")
            Dim paraLabelKbn As String = GCom.NzDec(GCom.SelectedItem(ListView1, 18), "")
            ...
            Dim UKETUKE As String = GCom.NzDec(GCom.SelectedItem(ListView1, 22), "")
    ...
    Private Sub CmdInputRePrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdInputRePrint.Click
        ...
        Try
            ...
            Dim ToriCode As String = GCom.NzDec(GCom.SelectedItem(ListView1, 2).ToString.Replace("-", ""), "")
            Dim FuriDate As String = GCom.NzDec(GCom.SelectedItem(ListView1, 5).ToString.Replace("/", ""), "")
            Dim MotikomiSeq As String = CInt(GCom.NzDec(GCom.SelectedItem(ListView1, 6), "").Trim).ToString("0000")

            '---------------------------------------------------
            ' 受付明細表再印刷
            '---------------------------------------------------
            PrintID = "KFSP002"
            PrintRPD = "KFSP002_受付明細表(プリンタ用).rpd"
            ...
            '---------------------------------------------------
            PrintID = "KFSP035"
            PrintRPD = "KFSP035_インプットエラーリスト.rpd"
            PrintFileName = PrintID & "_" & ToriCode & FuriDate & MotikomiSeq & ".CSV"
            If System.IO.File.Exists(System.IO.Path.Combine(PrintFileDir, PrintFileName)) = True Then
