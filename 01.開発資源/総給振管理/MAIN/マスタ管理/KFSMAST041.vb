...
#Region "変数宣言"
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFSMAST041", "スケジュール管理画面")
    Private Const msgTitle As String = "スケジュール管理画面(KFSMAST041)"
...
#End Region
    ...
    '画面起動イベント処理
    Private Sub KFSMAST041_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            With ListView1
                ...
                '2017/02/28 タスク）西野 ADD 飯田信金 カスタマイズ対応(受付タイムスタンプ表示対応) -------------------- START
                .Columns.Add("受付日時", 140, HorizontalAlignment.Center)              '22
                '2017/02/28 タスク）西野 ADD 飯田信金 カスタマイズ対応(受付タイムスタンプ表示対応) -------------------- END
            End With
    ...
    '画面終了時処理
    Private Sub KFSMAST041_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
        Try
    ...
    Private Sub CmdSelect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSelect.Click
        ...
        Try
            ...
            '2017/02/28 タスク）西野 ADD 飯田信金 カスタマイズ対応(受付時間表示対応) -------------------- START
            SQL.AppendLine(", UKETUKE_TIME_STAMP_S")
            '2017/02/28 タスク）西野 ADD 飯田信金 カスタマイズ対応(受付時間表示対応) -------------------- END

            '2017/02/28 タスク）西野 CHG 飯田信金 カスタマイズ対応(受付タイムスタンプ表示対応) -------------------- START
            SQL.AppendLine("  FROM S_SCHMAST, S_SCHMAST_SUB, S_TORIMAST")
            'SQL.AppendLine("  FROM S_SCHMAST, S_TORIMAST")
            '2017/02/28 タスク）西野 CHG 飯田信金 カスタマイズ対応(受付タイムスタンプ表示対応) -------------------- END
            ...
            If GCom.SetDynaset(SQL.ToString, REC) Then
                ...
                Do While REC.Read
                    ...
                    With GCom.NzDec(REC.Item("UKETUKE_TIME_STAMP_S"), "")
                        Data(22) = String.Format("{0}/{1}/{2} {3}:{4}:{5}", .Substring(0, 4), .Substring(4, 2), .Substring(6, 2), .Substring(8, 2), .Substring(10, 2), .Substring(12, 2))
                    End With
                    '2017/02/28 タスク）西野 ADD 飯田信金 カスタマイズ対応(受付タイムスタンプ表示対応) -------------------- END
