...
Module ModMain
    ' ログ処理クラス
    Private MainLOG As New CASTCommon.BatchLOG("KFJ050_EDITION2", "日報集計処理バッチ")
    Private Const msgTitle As String = "日報集計処理バッチ(KFJ050_EDITION2)"
    ' パブリックＤＢ
    ...
    Private KEKKA_HENKYAKU_KBN As String = ""
    ...
    Public Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            '#####################################
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報集計", "開始")
        ...
        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報集計", "失敗", ex.Message)
            Return 999
        Finally
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報集計", "終了")
        End Try
    ...
    Private Function NippouInit(ByVal CmdArgs() As String) As Boolean
        ...
        Try
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報初期処理", "開始", "")

            '------------------------------------------------
            ' パラメータ有無チェック
            '------------------------------------------------
            If CmdArgs.Length = 0 Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報初期処理", "失敗", "コマンドライン引数異常")
                ReturnCode = 10
                Return False
            End If
            ...
            If param.Length = 3 Then
                ...
                If FileKbn <> "1" AndAlso FileKbn <> "2" Then
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報初期処理", "失敗", "ファイル区分異常：" & CmdArgs(0))
                    ReturnCode = 11
                    Return False
                End If
            ...
            Else
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報初期処理", "失敗", "コマンドライン引数異常：" & CmdArgs(0))
                ReturnCode = 12
                Return False
            End If

            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報初期処理", "成功", "")
            Return True

        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報初期処理", "", ex.Message)
            ReturnCode = 99
            Return False
        Finally
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報初期処理", "終了", "")
        End Try
    ...
    Private Function NippouMain() As Boolean
        ...
        Try
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "開始", "")
            ...
            If File.Exists(InFile) = False Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "日報ファイル取得失敗 日報ファイル名:" & InFile)
                ReturnCode = 11
                Return False
            End If
            ...
            Else
                ...
                Select Case ret
                    Case 0
                        MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "成功", "コード変換")
                    Case Else
                        Select Case ret
                            Case 100
                                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "コード変換失敗　ファイルパス:" & InFile)
                            Case 200
                                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "コード区分異常(JIS改行あり)　ファイルパス:" & InFile)
                            Case 300
                                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "コード区分異常(JIS改行なし)　ファイルパス:" & InFile)
                            Case 400
                                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "出力ファイル作成失敗　ファイルパス:" & OutFile)
                        End Select
                        ReturnCode = 12
                        Return False
                End Select
            ...
            If clsFUSION.fn_POWER_SORT(intDisposalNumber, intFieldDefinition, strKeyCmdStr, strInputFiles, _
                ...
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "ファイルソート")
                ReturnCode = 13
                Return False
            End If
            ...
            If LIST_SCH_ERR.Count > 0 Then
                ...
                If ret <> 0 Then
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "スケジュールマスタエラーリスト印刷　リターンコード:" & ret)
                    ReturnCode = 15
                    Return False
                Else
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "成功", "スケジュールマスタエラーリスト印刷")
                End If
            End If
            ...
            If LIST_TORI_ERR.Count > 0 Then
                ...
                If ret <> 0 Then
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", "取引先マスタエラーリスト印刷　リターンコード:" & ret)
                    ReturnCode = 16
                    Return False
                Else
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "成功", "取引先マスタエラーリスト印刷")
                End If
            End If
            ...
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "成功", "入力ファイル正常フォルダへ移動 " & InFile & " -> " & ToFile)
            ...
            Try
                ...
                If iRet = 1 Then    'ジョブ登録済
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "手数料計算登録", "失敗", "ジョブ登録済み")
                    ReturnCode = 1
                ElseIf iRet = -1 Then 'ジョブ検索失敗
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "手数料計算登録", "失敗", "")
                    ReturnCode = 2
                Else
                    'job登録
            ...
            Catch ex As Exception
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "手数料計算登録", "失敗", ex.Message)
                ReturnCode = 90
                Return False
            End Try
            ...
            If TESUU_RECALC_KBN = "1" Then
                ...
                Try
                    ...
                    iRet = MainLOG.SearchJOBMAST(jobid, para, MainDB)
                    If iRet = 1 Then    'ジョブ登録済
                        MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "手数料再計算登録", "失敗", "ジョブ登録済み")
                        ReturnCode = 1
                    ElseIf iRet = -1 Then 'ジョブ検索失敗
                        MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "手数料再計算登録", "失敗", "")
                        ReturnCode = 2
                    Else
                        'job登録
                        If MainLOG.InsertJOBMAST(jobid, MainLOG.UserID, para, MainDB) = False Then
                            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "手数料再計算登録", "失敗", "ジョブ登録失敗 パラメータ:" & para)
                            ReturnCode = 3
                        End If
                    End If
                ...
                Catch ex As Exception
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "手数料再計算登録", "失敗", ex.Message)
                    ReturnCode = 90
                    Return False
                End Try
            End If
            ...
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "成功", "")
            Return True

        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "失敗", ex.Message)
            ReturnCode = 99
            Return False
        Finally
            MainDB.Rollback()
            ...
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報主処理", "終了", "")
        End Try
    ...
    Private Function IniRead() As Boolean
        ...
        Try
            ...
            If DEN_DIR = "err" OrElse DEN_DIR = "" Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "設定ファイル取得", "失敗", "設定ファイル取得失敗 項目名:DENフォルダ 分類:COMMON 項目:DEN")
                Return False
            End If
            ...
            If DENBK_DIR = "err" OrElse DENBK_DIR = "" Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "設定ファイル取得", "失敗", "設定ファイル取得失敗 項目名:DENBKフォルダ 分類:COMMON 項目:DENBK")
                Return False
            End If
            ...
            If DATBK_DIR = "err" OrElse DATBK_DIR = "" Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "設定ファイル取得", "失敗", "設定ファイル取得失敗 項目名:DATBKフォルダ 分類:COMMON 項目:DATBK")
                Return False
            End If
            ...
            If NTC = "err" OrElse NTC = "" Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "設定ファイル取得", "失敗", "設定ファイル取得失敗 項目名:NTC接続方法 分類:NIPPOU 項目:NTC")
                Return False
            End If
            ...
            If NIPPOU_HONBU = "err" Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "設定ファイル取得", "失敗", "設定ファイル取得失敗 項目名:日報本部コード 分類:NIPPOU 項目:HONBUCD")
                Return False
            End If
            ...
            If CSV_DIR = "err" OrElse CSV_DIR = "" Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "設定ファイル取得", "失敗", "設定ファイル取得失敗 項目名:CSVフォルダ 分類:COMMON 項目:CSV")
                Return False
            End If
        ...
        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "設定ファイル取得", "", ex.Message)
            Return False
        Finally
    ...
    Private Function ReadFile(ByVal FileName As String) As Boolean
        ...
        Try
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報ファイル読込(開始)", "成功")

            'ファイルオープン
            Select Case Nippou.FirstRead(FileName)
                Case 0
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "ファイル読込", "失敗", "ファイルパス:" & FileName)
                    Return False
                Case 1
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "ファイル読込", "成功", "ファイルパス:" & FileName)
            End Select
        ...
        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報ファイル読込", "失敗", ex.Message)
            Return False
        Finally
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報ファイル読込(終了)", "成功")
        End Try
        ...
        Try
            ...
            If OraReader.DataReader(SQL) = True Then
                While OraReader.EOF = False
                    ...
                    KEKKA_HENKYAKU_KBN = OraReader.GetString("KEKKA_HENKYAKU_KBN_T")
            ...
            If intCOUNT_SCH = 0 Then
                ...
                SQL.AppendLine(" AND NS_KBN_T = '" & NS_KBN & "' ")
                SQL.AppendLine("ORDER BY TORIS_CODE_T, TORIF_CODE_T")
    ...
    Private Function MergeNIPPOUMAST() As Boolean
        ...
        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "日報マスタ更新", "失敗", ex.Message)
            Return False
        End Try
    ...
    Private Function InsertSchmast(ByVal TorisCode As String, ByVal TorifCode As String, ByVal FuriDate As String) As Boolean
        ...
        Catch ex As Exception
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "スケジュールマスタ登録", "失敗", ex.Message)
            Return False
        End Try
    End Function
