...
'***** 2008/08/18 資金決済帳票カスタマイズ対応 kakiwaki ****************
Imports System.IO
...
Class ClsPrnKessaiSyorikekkaKakuninhyo
    ...
    Public Overrides Function CreateCsvFile() As String
        ...
        CSVObject.Output("作成区分", False, True)
        'CSVObject.Output("メッセージ", False, True)
        '2017/03/16 タスク）西野 CHG 標準版修正（資金決済性能改善）------------------------------------ END
        ' 2016/06/13 タスク）綾部 CHG 【PG】UI-03-12(RSV2対応<小浜信金>) -------------------- END
        '====================
    ...
    Public Function OutputCSVKekka(ByVal ary As ArrayList, _
        ...
        'Public Function OutputCSVKekka(ByVal ary As ArrayList, ByVal jikinko As String, ByVal strDate As String, ByVal strTime As String, ByVal strKessaiDate As String, ByVal db As CASTCommon.MyOracle, ByVal aryMSG As ArrayList) As Integer
        '2017/03/16 タスク）西野 CHG 標準版修正（資金決済性能改善）------------------------------------ END


        Dim KData As CAstFormKes.ClsFormKes.KessaiData = Nothing
        ' 2016/06/13 タスク）綾部 ADD 【PG】UI-03-12(RSV2対応<小浜信金>) -------------------- START
        Dim YData As ClsKessaiDataCreate.msgDATA = Nothing
        ' 2016/06/13 タスク）綾部 ADD 【PG】UI-03-12(RSV2対応<小浜信金>) -------------------- END
        ...
        Try
            ...
            For i As Integer = 0 To cnt
                ...
                ' 2016/06/13 タスク）綾部 ADD 【PG】UI-03-12(RSV2対応<小浜信金>) -------------------- START
                YData.Init()
                YData = CType(aryMSG.Item(i), ClsKessaiDataCreate.msgDATA)
                ' 2016/06/13 タスク）綾部 ADD 【PG】UI-03-12(RSV2対応<小浜信金>) -------------------- END

                newkey = KData.TorisCode & KData.TorifCode & KData.FuriDate

                ' キー設定
                'Call Key.SetOraDataCSV(OraReader)
