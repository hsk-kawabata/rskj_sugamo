...
Public Class ClsTouroku
    ...
    Private Function TourokuMain() As Boolean
        ...
        Try
            ...
            Select Case mKoParam.CP.BAITAI_CODE
                ...
                Case "00", "01", "05", "06", "04", "09", "07", "10", "11", "12", "13", "14", "15", "SA", _
                    ...
                    Case Else       'その他(伝送 etc)
                        ...
                        Else
                            ...
                            Select Case INI_RSV2_EDITION
                                ...
                                Case Else
                                    '2018/01/23 FJH）向井 ADD 青梅信金 伝送用ファイル名変更-------------------- START
                                    If mKoParam.CP.RENKEI_FILENAME = "" Then
                                        Dim FileSearchName As String = String.Empty
                                        Dim FileList() As String = System.IO.Directory.GetFiles(CASTCommon.GetFSKJIni("COMMON", "DEN"))

                                        If mArgumentData.INFOToriMast.MULTI_KBN_T = "0" Then
                                            FileSearchName = "D" & .TORI_CODE
                                        Else
                                            FileSearchName = "D" & mArgumentData.INFOToriMast.ITAKU_KANRI_CODE_T
                                        End If

                                        If FileList.Length = 0 Then
                                            mKoParam.CP.RENKEI_FILENAME = ""
                                        Else
                                            For i As Integer = 0 To FileList.Length - 1 Step 1
                                                Dim CheckFileName() As String = System.IO.Path.GetFileName(FileList(i)).Split(CChar("_"))
                                                If CheckFileName(0) = FileSearchName Then
                                                    mKoParam.CP.RENKEI_FILENAME = Path.Combine(GetFSKJIni("COMMON", "DEN"), CheckFileName(0) & "_" & CheckFileName(1) & "_" & CheckFileName(2))
                                                    Exit For
                                                Else
                                                    mKoParam.CP.RENKEI_FILENAME = ""
                                                End If
                                    ...
                                    'End If
                                    '' 2017/04/12 タスク）綾部 CHG 【ME】(RSV2標準機能対応) -------------------- END
                                    '2018/01/23 FJH）向井 ADD 青梅信金 伝送用ファイル名変更-------------------- END
                            End Select
                            ' 2016/01/11 タスク）綾部 CHG 【PG】UI_B-14-01(RSV2対応) -------------------- END
    ...
    Private Function MakingMeiMast(ByVal aReadFmt As CAstFormat.CFormat) As Boolean
        ...
        '2018/04/13 FJH) 向井 ADD 青梅信金　FAX基本手数料契解約チェック処理追加 --------------------------------------------- START
        Dim OraReader As CASTCommon.MyOracleReader = Nothing
        '2018/04/13 FJH) 向井 ADD 青梅信金　FAX基本手数料契解約チェック処理追加 --------------------------------------------- END
        ...
        Try
            ...
            '2018/04/13 FJH) 向井 ADD 青梅信金　FAX基本手数料契解約チェック処理追加 --------------------------------------------- START
            Dim INI_FaxKihonTorisCode As String = CASTCommon.GetRSKJIni("CUSTOMIZE_1358", "FAX_KIHONTESUU_TORIS_CODE")
            Dim INI_FaxKihonTorifCode As String = CASTCommon.GetRSKJIni("CUSTOMIZE_1358", "FAX_KIHONTESUU_TORIF_CODE")
            '2018/04/13 FJH) 向井 ADD 青梅信金　FAX基本手数料契解約チェック処理追加 --------------------------------------------- END
            ...
            Try
                ...
                Do Until aReadFmt.EOF
                    ...
                    If aReadFmt.IsDataRecord = True Then
                        If aReadFmt.ToriData.INFOToriMast.TORIS_CODE_T = INI_FaxKihonTorisCode And _
                            aReadFmt.ToriData.INFOToriMast.TORIF_CODE_T = INI_FaxKihonTorifCode Then
                            If Not CheckKeiKaiyaku(aReadFmt) Then
                                aReadFmt.InfoMeisaiMast.FURIKETU_CODE = 6
                                If sCheckRet <> "ERR" Then
                                    sCheckRet = "IJO"
                                End If
                            End If
                        End If
                            ...
                            If aReadFmt.InfoMeisaiMast.FURIKETU_CODE <> 6 Then
                                ...
                                If Not PrnMeisai Is Nothing Then
                                    ...
                                    Select Case aReadFmt.ToriData.INFOToriMast.UMEISAI_KBN_T
                                        Case "0"
                                        Case Else
                                            Dim UketukeErrOut As String = CASTCommon.GetText_CodeToName(Path.Combine(CASTCommon.GetFSKJIni("COMMON", "TXT"), "KFJMAST010_受付明細表出力区分.TXT"), _
                    ...
                    If sCheckRet <> "99" And sCheckRet <> "1A" Then
                        '2018/04/13 FJH) 向井 ADD 青梅信金　FAX基本手数料契解約チェック処理追加 --------------------------------------------- START
                        '振替結果コードが6(契約・解約チェックで徴求対象外)の場合明細マスタにINSERTしない
                        If aReadFmt.InfoMeisaiMast.FURIKETU_CODE <> 6 Then
                            ' 明細マスタ登録
                    ...
                    If aReadFmt.IsTrailerRecord() = True Or aReadFmt.EOF = True Then
                        ...
                        If PrnFlag = True Then
                            ...
                            If aReadFmt.ToriData.INFOToriMast.TORIS_CODE_T = INI_FaxKihonTorisCode And _
                                ...
                                SQL.Length = 0
                                SQL.Append("SELECT")
                                SQL.Append(" COUNT(*) AS IRAI_KEN")
                                SQL.Append(",SUM(FURIKIN_K) AS IRAI_KIN")
                                SQL.Append(",SUM(DECODE(FURIKETU_CODE_K,0,1,0)) AS SYORI_KEN")
                                SQL.Append(",SUM(DECODE(FURIKETU_CODE_K,0,FURIKIN_K,0)) AS SYORI_KIN")
                                SQL.Append(" FROM ")
                                SQL.Append(" MEIMAST ")
                                SQL.Append(" WHERE ")
                                SQL.Append(" TORIS_CODE_K = " & aReadFmt.ToriData.INFOToriMast.TORIS_CODE_T)
                                SQL.Append(" AND TORIF_CODE_K = " & aReadFmt.ToriData.INFOToriMast.TORIF_CODE_T)
                                SQL.Append(" AND FURI_DATE_K = " & aReadFmt.InfoMeisaiMast.FURIKAE_DATE)
                                SQL.Append(" AND DATA_KBN_K = 2")

                                OraReader = New CASTCommon.MyOracleReader(MainDB)
                                If OraReader.DataReader(SQL) = True Then
                                    aReadFmt.InfoMeisaiMast.TOTAL_KEN = OraReader.GetInt("IRAI_KEN")
                                    aReadFmt.InfoMeisaiMast.TOTAL_KIN = OraReader.GetInt("IRAI_KIN")
                                    aReadFmt.InfoMeisaiMast.TOTAL_IRAI_KEN = OraReader.GetInt("SYORI_KEN")
                                    aReadFmt.InfoMeisaiMast.TOTAL_IRAI_KIN = OraReader.GetInt("SYORI_KIN")
                                Else
                                    BatchLog.Write("FAX基本手数料", "処理対象レコードなし")
                                    BatchLog.UpdateJOBMASTbyErr("FAX振込基本手数料　処理対象レコードなし")
                                    mErrMessage = "FAX振込基本手数料　処理対象レコードなし"
                                    bRet = False
                                    Exit Do
                                End If
                            End If
    ...
    Private Function CheckKeiKaiyaku(ByVal aReadFmt As CAstFormat.CFormat) As Boolean

        Try
            Dim GCOM As New MenteCommon.clsCommon
            Dim CHECK_KeiKaiyakuFlg As String = aReadFmt.RecordDataNoChange.Substring(38, 1)                                '契解約フラグ
            Dim CHECK_KeiKaiyakuDate As String = aReadFmt.RecordDataNoChange.Substring(112, 8)                              '契解約日
            Dim CHECK_FuriDate As String = aReadFmt.InfoMeisaiMast.FURIKAE_DATE                                             '振替日
            Dim CHECK_FuriDate_Before1Month As String = GCOM.SET_DATE(CHECK_FuriDate).AddMonths(-1).ToString("yyyyMMdd")    '振替日の1ヶ月前
            Dim CHECK_FuriDate_FirstDay As String = CHECK_FuriDate.Substring(0, 6) & "01"                                   '振替月の1日

            '数値チェック
            If IsNumeric(CHECK_KeiKaiyakuDate) = False Then
                BatchLog.Write("契解約日数値チェック", "徴求対象")
                Return True
            End If

            Select Case CHECK_KeiKaiyakuFlg
                Case "1"                '契約の場合、契約日をチェック
                    '契約月が振替月の１ヶ月前以降の場合、徴求対象外
                    If CInt(CHECK_KeiKaiyakuDate.Substring(0, 6)) >= CInt(CHECK_FuriDate_Before1Month.Substring(0, 6)) Then
                        BatchLog.Write("契約日範囲チェック", "徴求対象外", "契約日：" & CHECK_KeiKaiyakuDate & "　基準月：" & CHECK_FuriDate_Before1Month.Substring(0, 6))
                        Return False
                    Else
                        BatchLog.Write("契約日範囲チェック", "徴求対象", "契約日：" & CHECK_KeiKaiyakuDate & "　基準月：" & CHECK_FuriDate_Before1Month.Substring(0, 6))
                        Return True
                    End If
                Case "2"                '解約の場合、解約日をチェック
                    '解約日が振替月の１日以前の場合、徴求対象外
                    If CInt(CHECK_KeiKaiyakuDate) <= CInt(CHECK_FuriDate_FirstDay) Then
                        BatchLog.Write("解約日範囲チェック", "徴求対象外", "解約日：" & CHECK_KeiKaiyakuDate & "　基準日：" & CHECK_FuriDate_FirstDay)
                        Return False
                    Else
                        BatchLog.Write("解約日範囲チェック", "徴求対象", "解約日：" & CHECK_KeiKaiyakuDate & "　基準日：" & CHECK_FuriDate_FirstDay)
                        Return True
                    End If
                Case Else               'それ以外の場合
                    BatchLog.Write("契解約フラグチェック", "徴求対象", "契解約フラグ：" & CHECK_KeiKaiyakuFlg)
                    Return True
            End Select

        Catch ex As Exception
            BatchLog.Write("契解約チェック", "徴求対象外", "失敗", ex.Message)
            Return False
        End Try
