...
Public Class KFSMAST010C
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFSMAST010C", "取引先マスタメンテナンス画面")
    Private Const msgTitle As String = "取引先マスタメンテナンス画面(KFSMAST010C)"
    ...
    Private INI_KIHONBAITAI As String
    ...
    '2014/01/14 saitou 標準版 印紙税対応 ADD --------------------------------------------------<<<<
#Region " ロード"
    Private Sub KFSMAST010C_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            For Index = 0 To MaxColumns Step 1
                Select Case Index
                    ...
                    Case Is = 139
                        ...
                        sTORI(Index, 5) = ""
                    Case Is = 140
                        oTORI(Index) = YOBI10_T
                        nTORI(Index, 0) = 7
                        nTORI(Index, 1) = 1
                        sTORI(Index, 0) = "基本料金徴求口座"
                            ...
                            Select Case .Name.ToUpper
                                ...
                                Case "SOUSIN_KBN_T"
                                    ...
                                    If Center = "0" Then
                                        ...
                                        '2016/11/15 saitou RSV2(近畿産業信組) MODIFY 初期値設定 ------------------------------------- START
                                        '信組の場合、送信区分=2(CSVリエンタ)が選択されている状態にする
                                        Ret = GCom.SetComboBox(CMB, "KFSMAST010_送信区分(信組).TXT", 2)
                                ...
                                Case "YOBI7_T"
                                    CmbName = "基本料金区分"
                                    TxtFileName = "KFSMAST010C_基本料金区分.TXT"
                                    Ret = GCom.SetComboBox(CMB, "KFSMAST010C_基本料金区分.TXT", True)
                                Case "YOBI9_T"
                                    CmbName = "基本料金徴求科目"
                                    TxtFileName = "KFSMAST010C_基本料金徴求科目.TXT"
                                    Ret = GCom.SetComboBox(CMB, "KFSMAST010C_基本料金徴求科目.TXT", True)
...
#Region " 帳票印刷"
    Public Function fn_CreateCSV_INSERT() As Boolean
        ...
        'Parameter      :
        'Description    :KFSP016(取引先マスタメンテ(登録))印刷用ＣＳＶファイル作成
        'Return         :
        'Create         :2009/09/16
        'Update         :
        '============================================================================
        ...
        Try
            ...
            'ＣＳＶファイル作成
            '------------------------------------------------
            Dim CreateCSV As New KFSPxxx("KFSP017")
            ...
            If GCom.SetDynaset(SQL.ToString, REC) AndAlso REC.Read Then
                ...
                With CreateCSV
                    ...
                    '2014/01/14 saitou 標準版 印紙税対応 UPD -------------------------------------------------->>>>
                    '印紙税の区分を追加
                    '.OutputCsvData(GCom.NzStr(REC.Item("KIGYO_CODE_T")), True, True)                '企業コード
                    ...
                    '2014/01/14 saitou 標準版 印紙税対応 UPD --------------------------------------------------<<<<
                    '2017/09/11 タスク）日比野 ADD 青梅信金(UI.5-2<PG>) ---------------------------------------- START
                    .OutputCsvData(GetComboBoxValue(2, GetIndex("YOBI7_T"), 1), True)               '基本料金区分
                    SITEN = GCom.NzStr(REC.Item("YOBI8_T"))
                    .OutputCsvData(SITEN, True)                                                     '基本料金徴求支店コード
                    .OutputCsvData(GCom.GetBKBRName(Jikinko, SITEN, 30), True)                      '基本料金徴求支店名
                    .OutputCsvData(GetComboBoxValue(2, GetIndex("YOBI9_T"), 1), True)               '基本料金徴求科目
                    .OutputCsvData(GCom.NzStr(REC.Item("YOBI10_T")), True)                          '基本料金徴求口座
                    .OutputCsvData("", True, True)                                                  'ダミー(改行)
    ...
    Public Function fn_Print(ByVal PrintNo As Integer) As Boolean
        ...
        Try
            ...
            Select Case PrintNo
                Case 1
                    nRet = ExeRepo.ExecReport("KFSP017.EXE", Param)
                Case 2
...
#Region " 関数"
    ...
    Private Function InsertModule() As Boolean
        ...
        Try
            ...
            '登録時には更新日を設定しない
            SQL.AppendLine(", '00000000000000')")                       '更新日
    ...
    Private Function UpdateModule() As Boolean
        ...
        '2016/11/15 saitou RSV2(近畿産業信組) ADD 標準版修正 ---------------------------------------- START
        '更新すると作成日がクリアされるのを回避
        sTORI(GetIndex("SAKUSEI_DATE_T"), 4) = sTORI(GetIndex("SAKUSEI_DATE_T"), 3)
        '2016/11/15 saitou RSV2(近畿産業信組) ADD --------------------------------------------------- END
        ...
        Try
            ...
            'スケジュールマスタの項目更新判定
            '媒体コード・種別・委託者コード・取扱金融機関コード・取扱支店コード・送信区分
            'のいずれかに変更がある場合は更新する
...
#Region " チェック"
    ...
    Private Function CheckMutualRelation_005(ByRef MSG As String) As Integer
        Try
            ...
            '    End If
            'Next Cnt
            Dim FieldName() As String = {"TESUUTYO_SIT_T", "TORIMATOME_SIT_T", "YOBI8_T"}

            For Cnt As Integer = 0 To FieldName.Length - 1 Step 1
...
#Region " イベント"
    ...
    ''' <param name="e"></param>
    ''' <remarks>2017/04/28 saitou RSV2 added for 標準機能追加(種別によって表示する手数料変更)</remarks>
