'2017/09/08 タスク）綾部 ADD 【PG】青梅信金 カスタマイズ対応(UI_5-3) -------------------- START
Option Strict On
Option Explicit On
...
Public Class KFSMAIN210C

#Region "変数宣言"

    Private MainLOG As New CASTCommon.BatchLOG("KFSMAIN210C", "後取手数料データ作成画面")
    Private Const msgTitle As String = "後取手数料データ作成画面(KFSMAIN210C)"
    ...
    Private Structure IniInfo
        ...
        Dim TesuuTorisCode As String    ' 後取手数料徴求取引先(主コード)
        Dim TesuuTorifCode As String    ' 後取手数料徴求取引先(副コード)
        Dim TesuuItakuCode As String    ' 後取手数料データ委託者コード
        Dim TesuuItakuName As String    ' 後取手数料データ委託者名
        Dim TesuuShitenCode As String   ' 後取手数料データ支店コード
        Dim TesuuShitenName As String   ' 後取手数料データ支店名
        Dim TesuuFileName As String     ' 後取手数料データ出力ファイル名
...
#Region "画面制御"
    ...
    '画面ロード
    Private Sub KFSMAIN210C_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region "ボタン"
    ...
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            Dim KijunFuriDate_Obj As String = txtFuriDateY.Text & "/" & txtFuriDateM.Text & "/" & txtFuriDateD.Text
            Dim KijunMonth As String = CDate(KijunFuriDate_Obj).AddMonths(-1).ToString("yyyyMM")
            If CheckMaster(KijunMonth, KijunFuriDate) = False Then
            ...
            If File.Exists(Path.Combine(Ini.TesuuFileName)) = True Then
                ...
                If MessageBox.Show(String.Format(CUST_MSG0003I, "作成済みの後取手数料データ"), msgTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    File.Delete(Path.Combine(Ini.TesuuFileName))
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "実行", "成功", "既存ファイル削除")
                Else
                    MessageBox.Show(String.Format(CUST_MSG0004I, "後取手数料データ作成"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
            ...
            ' 処理開始前確認
            '------------------------------------------
            If MessageBox.Show(String.Format(CUST_MSG0002I, "後取手数料データ作成"), msgTitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Cancel Then
                MessageBox.Show(String.Format(CUST_MSG0004I, "後取手数料データ作成"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
            ...
            '------------------------------------------
            MainDB.BeginTrans()
            If MakeZenginData(KijunMonth, KijunFuriDate) = False Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "実行", "中断", "後取手数料データ作成失敗")
                Exit Sub
            End If
...
#Region "関数"
    ...
    Private Function CheckMaster(ByVal KijunMonth As String, ByVal KijunDate As String) As Boolean
        ...
        Try
            ...
            SQL.Append("   , S_I_RENKEIMAST")
            SQL.Append(" WHERE")
            SQL.Append("     KESSAI_PATN_T              = '0'")
            SQL.Append(" AND TESUUTYO_KBN_T             = '1'")
            SQL.Append(" AND TORIS_CODE_T               = TORIS_CODE_SR")
            SQL.Append(" AND TORIF_CODE_T               = TORIF_CODE_SR")
            SQL.Append(" AND SUBSTR(FURI_DATE_SR, 1, 6) = " & SQ(KijunMonth))
            SQL.Append(" AND HASSIN_TESUU_KIN_KEI_SR    > 0")

            If OraReader.DataReader(SQL) = False Then
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "マスタチェック", "成功", "後取手数料データ作成先０件")
                MessageBox.Show("後取手数料データ作成" & MSG0112W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return False
            End If
            ...
            Else
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "マスタチェック", "失敗", "後取手数料徴求取引先未登録")
    ...
    Private Function MakeZenginData(ByVal KijunMonth As String, ByVal KijunDate As String) As Boolean
        ...
        Try
            ...
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "後取手数料データ作成", "開始", "")
            ...
            If OraReader.DataReader(SQL) = True Then
                ...
                '------------------------------------------
                SQL.Length = 0
                '2018/04/18 FJH）向井 CHG 青梅信金 同一委託者手数料合算対応 -------------------- START
                SQL.Length = 0
                SQL.Append(" SELECT ")
                ...
                SQL.Append("   , SUM(HASSIN_TESUU_KIN_KEI_SR) AS SUM_HASSIN_TESUU_KIN_KEI_SR")
                SQL.Append(" FROM")
                SQL.Append("     S_TORIMAST")
                SQL.Append("   , S_I_RENKEIMAST")
                SQL.Append(" WHERE")
                ...
                'SQL.Append("   , TUKEKAMOKU_T")
                'SQL.Append("   , TUKEKOUZA_T")
                '2018/04/18 FJH）向井 CHG 青梅信金 同一委託者手数料合算対応 -------------------- END
                ...
                If MeiReader.DataReader(SQL) = True Then
                    While MeiReader.EOF = False
                        With gZENGIN_REC2
                            ...
                            .ZG4 = MeiReader.GetString("TUKESIT_NO_T").PadRight(3)
                            ...
                            End If
                            .ZG6 = Space(4)
                            .ZG7 = ConvertKamoku2TO1(MeiReader.GetString("TUKEKAMOKU_T").PadLeft(2, "0"c))
                            .ZG8 = MeiReader.GetString("TUKEKOUZA_T").PadLeft(7, "0"c)
                            .ZG9 = MeiReader.GetString("ITAKU_KNAME_T").PadRight(30).Substring(0, 30)

                            '2018/04/18 FJH）向井 CHG 青梅信金 同一委託者手数料合算対応 -------------------- START
                            Dim FuriKin As Long = CLng(MeiReader.GetString("SUM_HASSIN_TESUU_KIN_KEI_SR"))
                            .ZG10 = Format(FuriKin, "0000000000")
                            .ZG11 = "0"
                            .ZG12 = MeiReader.GetString("ITAKU_CODE_T")
                            .ZG13 = Space(10)
                            'Dim FuriKin As Long = CLng(MeiReader.GetString("HASSIN_TESUU_KIN_KEI_SR"))
                            '.ZG10 = Format(FuriKin, "0000000000")
                            '.ZG11 = "0"
            ...
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "後取手数料データ作成", "成功", "")
            Return True

        Catch ex As Exception
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "後取手数料データ作成", "失敗", ex.Message)
            Return False
        Finally
            ...
            If Not FileWrite Is Nothing Then
                ...
                FileWrite = Nothing
            End If
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "後取手数料データ作成", "終了", "")
        End Try
...
End Class
'2017/09/08 タスク）綾部 ADD 【PG】青梅信金 カスタマイズ対応(UI_5-3) -------------------- END
