        ...
        Public Function GetTourokuFileName() As String
            ...
            Select Case CASTCommon.GetRSKJIni("RSV2_V1.0.0", "EDITION")
                ...
                Case Else
                    ...
                    Else
                        'RetFileName &= DaihyoItakuCode & "00"
                        RetFileName &= DaihyoItakuCode
                    End If
                    '2018/01/23 FJH）向井 ADD 青梅信金 ファイル名変更-------------------- START
                    'タイムスタンプ
                    RetFileName &= "_" & Now.ToString("yyyyMMddHHmmssfff")
                    ...
                    Dim ProcessID As Integer = System.Diagnostics.Process.GetCurrentProcess.Id
                    RetFileName &= "_" & ProcessID.ToString("00000000").Substring(8 - 4)
                    '2018/01/23 FJH）向井 ADD 青梅信金 ファイル名変更-------------------- END
            End Select
    ...
    Public Function RevMain() As Boolean
        ...
        Try
            ...
            If FileInfo_FAX.Length <= 360 Then
                ...
                MainLOG.Write("伝送連携受信個別メイン処理", "成功", "処理対象無しのため処理中断:" & " " & " -> " & BackupFile)
                ' 2017/12/18 FJH）向井 ADD 青梅信金 0件の場合正常終了でJOB登録を行う-------------------- START
                MainDB = New CASTCommon.MyOracle
                MainDB.BeginTrans()
                JobMessage = "FAX振込データ0件"
                If Not MainLOG.InsertJOBMAST(MainLOG.ModuleName.Substring(2, 4), "admin", "FileNameFullPath", MainDB, "2", JobMessage) Then
                ...
                MainDB.Close()
                MainDB = Nothing
                ' 2017/12/18 FJH）向井 ADD 青梅信金 0件の場合正常終了でJOB登録を行う-------------------- END
                ret = True
                Exit Try
            End If
            ...
            For i As Integer = 0 To SplitFileName.Count - 1 Step 1
                ...
                '2017/12/04 タスク）日比野 ADD 【ST】青梅信金 カスタマイズ対応(UI_5-10) -------------------- START
                'ファイルコピー時に同名ファイルの存在確認を行う。一定回数経過しても存在する場合、処理中止。（FAX振込で）
                Dim iSleep As Integer = CASTCommon.GetRSKJIniNum("CUSTOMIZE_1358", "SLEEP")
                Dim iLoopMax As Integer = CASTCommon.GetRSKJIniNum("CUSTOMIZE_1358", "LOOPMAX")
                If iSleep <= 0 Then
                    iSleep = 1000           'デフォルト：1秒間
                End If
                If iLoopMax <= 0 Then
                    iLoopMax = 600          'デフォルト：600回
                End If
                Do
                    If File.Exists(JobInfo.DestFileName) = False Then
                        '同名ファイルが存在しないので、JOB監視へ登録可能
                        Exit Do
                    End If
                    '同名ファイルが存在する為、落し込み処理が終わるのを待つ
                    System.Threading.Thread.Sleep(iSleep)

                    iLoopMax -= 1
                Loop While (0 < iLoopMax)
                If iLoopMax = 0 Then
                    '同名ファイルが存在し続けるため、処理中断
                    JobMessage = "ファイルが残っている為処理できません:" & JobInfo.DestFileName
                    Exit Try
                End If
                File.Copy(SplitFileNameFullPath, JobInfo.DestFileName, True)
                '2017/12/04 タスク）日比野 ADD 【ST】青梅信金 カスタマイズ対応(UI_5-10) -------------------- END
            ...
            ' 2017/12/19 FJH）向井 ADD 青梅信金 正常終了でJOB登録を行う-------------------- START
            MainDB = New CASTCommon.MyOracle
            MainDB.BeginTrans()
            JobMessage = ""
            If Not MainLOG.InsertJOBMAST(MainLOG.ModuleName.Substring(2, 4), "admin", FileNameFullPath, MainDB, "2", JobMessage) Then
            ...
            MainDB.Close()
            MainDB = Nothing
            ' 2017/12/19 FJH）向井 ADD 青梅信金 正常終了でJOB登録を行う-------------------- END
            ret = True
        ...
        Finally
            ...
            Else
                ...
                If Not ret Then
                    ...
                    '2017/11/06 タスク）日比野 ADD 【IT】青梅信金 カスタマイズ対応(UI_5-10) -------------------- START
                    If IsNothing(MainDB) = True Then
                        MainDB = New CASTCommon.MyOracle
                        MainDB.BeginTrans()
                    End If
                    '2017/11/06 タスク）日比野 ADD 【IT】青梅信金 カスタマイズ対応(UI_5-10) -------------------- END
