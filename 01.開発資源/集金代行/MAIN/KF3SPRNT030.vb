...
''' </summary>
''' <remarks></remarks>
Public Class KF3SPRNT030

#Region "クラス定数"
    Private MainLOG As New CASTCommon.BatchLOG("KF3SPRNT030", "銀行別振替結果合計表印刷画面")
    Private Const msgTitle As String = "銀行別振替結果合計表印刷画面(KF3SPRNT030)"
...
#Region "イベントハンドラ"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub KF3SPRNT030_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '------------------------------------------------
            'ログの書込に必要な情報の取得
            '------------------------------------------------
    ...
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        Try
            ...
            '印刷前確認メッセージ
            If MessageBox.Show(String.Format(MSG0013I, "銀行別振替結果合計表"), _
            ...
            nRet = ExeRepo.ExecReport("KF3SP003.EXE", param)

            '戻り値に対応したメッセージを表示する
            Select Case nRet
                Case 0
                    '印刷後確認メッセージ
                    MessageBox.Show(String.Format(MSG0014I, "銀行別振替結果合計表"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Case -1
                    '印刷対象なしメッセージ
                    MessageBox.Show(MSG0106W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Case Else
                    '印刷失敗メッセージ
                    MessageBox.Show(String.Format(MSG0004E, "銀行別振替結果合計表"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Select
