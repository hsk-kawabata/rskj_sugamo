...
Module ModMain
    ...
    Public BatchLog As New CASTCommon.BatchLOG("KFJP032", "取引先マスタチェックリスト")
    ...
    Public Toris_Kaisi As String = ""     '取引先主コード(開始)
    Public Toris_Syuryo As String = ""     '取引先主コード(終了)
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            Toris_Kaisi = Cmd(1)      '取引先コード(開始)
            Toris_Syuryo = Cmd(2)     '取引先コード(終了)
            BatchLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "パラメータ取得", "成功", "コマンドライン引数：" & CmdArgs(0))
            '---------------------------------------------------------
            ' 取引先マスタチェックリスト印刷処理
            '---------------------------------------------------------
            Dim CheckList As New KFJP032()
            CheckList.CreateCsvFile()
            If CheckList.MakeRecord = False Then
            ...
            '印刷処理実行
            If CheckList.ReportExecute(PrinterName) = True Then
                '印刷成功
            Else
                '印刷失敗
