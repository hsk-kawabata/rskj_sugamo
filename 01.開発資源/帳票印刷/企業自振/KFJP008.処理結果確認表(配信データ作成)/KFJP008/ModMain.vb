...
Module ModMain
    ...
    Private BatchLog As New CASTCommon.BatchLOG("KFJP008", "処理結果確認表(配信データ作成)")
    
    '
    ' 機能　 ：処理結果確認表(配信データ作成)印刷 メイン処理
    '
    ' 引数　 ：ARG1 - 起動パラメータ
    '
    ' 戻り値 ： 0    － 正常
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            BatchLog.Write("", Toris_Code & "-" & Torif_Code, "00000000", "(印刷処理)開始", "成功", "")
            ...
            If Cmd.Length <> 2 Then
                'パラメータ間違い
                BatchLog.Write(LoginID, Toris_Code & "-" & Torif_Code, "00000000", "パラメータチェック", "失敗", "コマンドライン引数異常：" & CmdArgs.ToString)
                Return -100
            End If
            ...
            End If
            '---------------------------------------------------------
            ' 処理結果確認表(配信データ作成)印刷処理
            '---------------------------------------------------------
            Dim HAISIN As New KFJP008(CSVFileName)

            '印刷処理実行
            If HAISIN.ReportExecute(PrinterName) = True Then
                '印刷成功
            Else
                '印刷失敗
                BatchLog.Write(LoginID, "0000000000-00", "00000000", "RepoAgent印刷", "失敗", HAISIN.ReportMessage)
