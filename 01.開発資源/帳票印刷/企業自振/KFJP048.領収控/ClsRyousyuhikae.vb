    ...
    Private Function PrnRyousyuhikae(ByVal TorisCode As String, ByVal TorifCode As String, ByVal FuriDate As String, ByVal PrinterName As String) As Boolean
        ...
        Try
            ...
            If JikinkoName.ToUpper = "ERR" OrElse JikinkoName = "" Then
                MainLOG.Write("設定ファイル取得", "失敗", "項目名:自金庫名 分類:KOKUZEI 項目:KINKONAME")
                Return -300
            End If
            ...
            If Ryousyu2.ToUpper = "ERR" Then
                MainLOG.Write("設定ファイル取得", "失敗", "項目名:領収欄２ 分類:KOKUZEI 項目:RYOUSYU2")
                Return -300
            End If
                ...
                Select Case OraReader.GetItem("DATA_KBN_K").Trim
                    ...
                    Case "3" '個別明細レコード（Ｃ）
                        ...
                        Select Case KFmt.KOKUZEI_REC1.KZ7.Trim
                            Case "1"
                                ClsPrnt.OutputCsvData("") '納期区分カナ文字
                        ...
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC2.KZ3.Trim) '税務署コード(局署番号)    '※グループ化条件
                        ...
                        ClsPrnt.OutputCsvData(ZumiCnt) 'レコード番号

                        '*** FJH KAKI 2010/3/5 ******************************************
                        '*** 4行MAXだったらの仕様が微妙なので保留************************
                        '*** とりあえず詰めます******************************************
                        ...
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC3.KZ30.Trim & Space(1) & KFmt.KOKUZEI_REC3.KZ31.Trim)  '納税者名１、２
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC3.KZ13.Trim.Substring(0, 4)) '整理番号１
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC3.KZ13.Trim.Substring(4)) '整理番号２
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC3.KZ9.Trim)  '納付税額
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC3.KZ10.Trim)  '内利子税
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC3.KZ18.Trim.Substring(0, 4)) '振替金融機関コード
                        ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC3.KZ18.Trim.Substring(4, 3)) '振替支店コード

                        '***金融機関整理欄***
                        '取り合えず空白
                        ClsPrnt.OutputCsvData(" ") '金融機関整理欄
                        '***金融機関整理欄***

                        ClsPrnt.OutputCsvData(FuriDate)  '振替日
                        ClsPrnt.OutputCsvData(Ryousyu1)                '領収欄１
                        ClsPrnt.OutputCsvData(Ryousyu2)                 '領収欄２
                        'ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC2.KZ27.Trim)  '領収欄３
                        'ClsPrnt.OutputCsvData(KFmt.KOKUZEI_REC2.KZ28.Trim, False, True)  '領収欄４
                        ClsPrnt.OutputCsvData(Ryousyu3)  '領収欄３
                        ClsPrnt.OutputCsvData(Ryousyu4, False, True)  '領収欄４
            ...
            If ClsPrnt.ReportExecute(PrinterName) = True Then
                MainLOG.Write("領収控", "成功")
            Else
                MainLOG.Write("領収控", "失敗", ClsPrnt.ReportMessage)
                Return False
            End If
