...
Module ModMain
    ...
    Private BatchLog As New CASTCommon.BatchLOG("KFJP044", "処理結果確認表(自振契約結果)")
    
    '
    ' 機能　 ：処理結果確認表(自振契約結果)印刷 メイン処理
    '
    ' 引数　 ：ARG1 - 起動パラメータ
    '
    ' 戻り値 ： 0    － 正常
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            ' 処理結果確認表(自振契約結果)印刷処理
            '---------------------------------------------------------
            Dim JifKekka As New KFJP044(CSVFileName)

            '印刷処理実行
            If JifKekka.ReportExecute(PrinterName) = True Then
                '印刷成功
            Else
                '印刷失敗
                BatchLog.Write(LoginID, "0000000000-00", "00000000", "RepoAgent印刷", "失敗", JifKekka.ReportMessage)
