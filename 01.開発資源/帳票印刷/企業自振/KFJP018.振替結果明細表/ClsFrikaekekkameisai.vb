...
' 
Public Class ClsFrikaekekkameisai
    ...
    Private INI_RSV2_J_KEKKA As String = ""
    ...
    Function Main() As Integer
        ...
        Try
            ...
            ' 振替結果明細表印刷処理
            bRet = PrintKekkameisai()

        Catch ex As Exception
            MainLOG.Write("振替結果明細表", "失敗", ex.Message & ":" & ex.StackTrace)
    ...
    ' 備考   ： 
    '
    Private Function PrintKekkameisai() As Boolean
        ...
        ' 2015/12/18 タスク）綾部 ADD 【PG】UI_B-14-05(RSV2対応) -------------------- START
        INI_RSV2_J_KEKKA = CASTCommon.GetRSKJIni("RSV2_V1.0.0", "J_KEKKA")
        ...
        Dim PrnKekkameisai As New ClsPrnFrikaeKekkameisai    ' 振替結果明細表
        ...
        If bSQL = True Then
            ...
            name = PrnKekkameisai.CreateCsvFile()
            ...
            Do
                ...
                PrnKekkameisai.OutputCsvData(Jikinko_NAME)      '自金庫名
                PrnKekkameisai.OutputCsvData(OraReader.GetString("TORIS_CODE_T") & OraReader.GetString("TORIF_CODE_T"))
                PrnKekkameisai.OutputCsvData(OraReader.GetString("ITAKU_NNAME_T"))      ' 委託者名漢字
                PrnKekkameisai.OutputCsvData(OraReader.GetString("ITAKU_CODE_T"))       ' 委託者コード
                PrnKekkameisai.OutputCsvData(CASTCommon.ConvertDate(OraReader.GetItem("FURI_DATE_K"), "yyyyMMdd"))  '振替日
                PrnKekkameisai.OutputCsvData(OraReader.GetString("TORIMATOME_SIT_T"))   ' とりまとめ店コード
                PrnKekkameisai.OutputCsvData(strTORIMATOME_SIT_NAME)        ' とりまとめ店名漢字
                PrnKekkameisai.OutputCsvData(OraReader.GetString("KEIYAKU_KIN_K"))      ' 金融機関コード
                PrnKekkameisai.OutputCsvData(OraReader.GetString("KEIYAKU_SIT_K"))      ' 支店
                PrnKekkameisai.OutputCsvData(OraReader.GetString("KEIYAKU_KNAME_K"), True)    ' 口座名義
                If OraReader.GetString("KEIYAKU_KAMOKU_K") = "01" Then
                    PrnKekkameisai.OutputCsvData("当座")
                ElseIf OraReader.GetString("KEIYAKU_KAMOKU_K") = "02" Then
                    PrnKekkameisai.OutputCsvData("普通")
                ElseIf OraReader.GetString("KEIYAKU_KAMOKU_K") = "05" Then
                    PrnKekkameisai.OutputCsvData("納税")
                ElseIf OraReader.GetString("KEIYAKU_KAMOKU_K") = "37" Then
                    PrnKekkameisai.OutputCsvData("職員")
                ElseIf OraReader.GetString("KEIYAKU_KAMOKU_K") = "09" Then
                    PrnKekkameisai.OutputCsvData("その他")
                Else
                    PrnKekkameisai.OutputCsvData("")
                End If

                PrnKekkameisai.OutputCsvData(OraReader.GetString("KEIYAKU_KOUZA_K"))        ' 口座番号（契約者口座名）
                PrnKekkameisai.OutputCsvData(OraReader.GetInt64("FURIKIN_K").ToString)      ' 振替依頼金額（振替金額K）
                If OraReader.GetInt64("FURIKETU_CODE_K") = 0 Then
                    PrnKekkameisai.OutputCsvData("")
                ElseIf OraReader.GetInt64("FURIKETU_CODE_K") = 1 Then
                    PrnKekkameisai.OutputCsvData("1:資金不足")
                ElseIf OraReader.GetInt64("FURIKETU_CODE_K") = 2 Then
                    PrnKekkameisai.OutputCsvData("2:取引なし")
                ElseIf OraReader.GetInt64("FURIKETU_CODE_K") = 3 Then
                    PrnKekkameisai.OutputCsvData("3:預金者都合")
                ElseIf OraReader.GetInt64("FURIKETU_CODE_K") = 4 Then
                    PrnKekkameisai.OutputCsvData("4:依頼書なし")
                ElseIf OraReader.GetInt64("FURIKETU_CODE_K") = 8 Then
                    PrnKekkameisai.OutputCsvData("8:委託者都合")
                ElseIf OraReader.GetInt64("FURIKETU_CODE_K") = 9 Then
                    PrnKekkameisai.OutputCsvData("9:その他")
                Else
                    PrnKekkameisai.OutputCsvData("9:その他")
                End If
                PrnKekkameisai.OutputCsvData(OraReader.GetString("JYUYOUKA_NO_K"))          ' 備考　
                PrnKekkameisai.OutputCsvData(OraReader.GetInt64("FURI_KEN_S").ToString)     ' 振替済件数
                PrnKekkameisai.OutputCsvData(OraReader.GetInt64("FURI_KIN_S").ToString)     ' 振替済金額
                PrnKekkameisai.OutputCsvData(OraReader.GetInt64("FUNOU_KEN_S").ToString)    ' 振替不能件数
                PrnKekkameisai.OutputCsvData(OraReader.GetInt64("FUNOU_KIN_S").ToString)    ' 振替不能金額
                PrnKekkameisai.OutputCsvData(OraReader.GetInt64("SYORI_KEN_S").ToString)    ' 振替依頼件数
                PrnKekkameisai.OutputCsvData(OraReader.GetInt64("SYORI_KIN_S").ToString)    ' 振替依頼金額
                '*** 修正 mitsu 2009/06/11 合計件数に対する手数料考慮 ***
                ...
                If OraReader.GetString("TESUUTYO_KBN_T") <> "2" AndAlso OraReader.GetString("TESUUMAT_PATN_T") = "1" Then
                    ...
                    '=========================
                    'マスタの値ではなく算出した値を出力
                    PrnKekkameisai.OutputCsvData((TesuuKin1 + TesuuKin2 + TesuuKin3).ToString)      ' 手数料消費税込みS（手数料S）
                Else
                    PrnKekkameisai.OutputCsvData(OraReader.GetInt64("TESUU_KIN_S").ToString)        ' 手数料消費税込みS（手数料S）
                End If
                '********************************************************

                PrnKekkameisai.OutputCsvData(mMatchingDate)
                PrnKekkameisai.OutputCsvData(OraReader.GetString("KIN_NNAME_N"))
                PrnKekkameisai.OutputCsvData(OraReader.GetString("SIT_NNAME_N"))
                ...
                PrnKekkameisai.OutputCsvData(strFormatKubunName)                                'フォーマット区分
                PrnKekkameisai.OutputCsvData(OraReader.GetString("FURI_CODE_T"))                '振替コード
                PrnKekkameisai.OutputCsvData(OraReader.GetString("KIGYO_CODE_T"), False, True)  '企業コード
            ...
            'If PrnKekkameisai.ReportExecute(PRINTERNAME) = True Then
            '    MainLOG.Write("振替結果明細表出力", "成功")
            'Else
            '    MainLOG.Write("振替結果明細表出力", "失敗", PrnKekkameisai.ReportMessage)
            '    Return False
            'End If

            '2012/06/30 標準版　WEB伝送対応
            If User_ID <> "" Then
                If PrnKekkameisai.ReportExecute(WEB_PRINTERNAME, User_ID, ITAKU_CODE) = True Then
                    MainLOG.Write("振替結果明細表出力", "成功")
                Else
                    MainLOG.Write("振替結果明細表出力", "失敗", PrnKekkameisai.ReportMessage)
                    Return False
            ...
            Else
                ...
                Select Case INI_RSV2_J_KEKKA
                    Case "1"
                        ...
                        End If
                    Case "2"
                        If PrnKekkameisai.ReportExecute(INI_COMMON_PRINT_NAME) = True Then
                            MainLOG.Write("振替結果明細表出力(ListWorks)", "成功")
                        Else
                            MainLOG.Write("振替結果明細表出力(ListWorks)", "失敗", PrnKekkameisai.ReportMessage)
                            Return False
                        End If
                    Case "3"
                        If PrnKekkameisai.ReportExecute(PRINTERNAME) = True Then
            ...
            If Not PrnKekkameisai.HostCsvName Is Nothing AndAlso PrnKekkameisai.HostCsvName <> "" Then
                Try
                    Dim DestName As String = CASTCommon.GetFSKJIni("COMMON", "HOST-PRT")
                    DestName &= PrnKekkameisai.HostCsvName
                    File.Copy(PrnKekkameisai.FileName, DestName, True)
                Catch ex As Exception
