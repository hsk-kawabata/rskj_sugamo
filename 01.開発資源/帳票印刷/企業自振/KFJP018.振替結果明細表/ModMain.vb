...
Module ModMain
    ...
    ' ログ処理クラス
    Public MainLOG As New CASTCommon.BatchLOG("KFJP018", "振替結果明細表")
    ...
    Function Main(ByVal arg() As String) As Integer
        ...
        Try
            ...
            ' 振替結果明細表出力処理
            Dim Frikaekekkameisai As New ClsFrikaekekkameisai
            ...
            Try
                ' 起動パラメータ格納
                Frikaekekkameisai.TORIS_CODE = Cmd(0).PadRight(10).Substring(0, 10)
                Frikaekekkameisai.TORIF_CODE = Cmd(0).PadRight(2).Substring(10)
                Frikaekekkameisai.FURI_DATE = Cmd(1)
                Frikaekekkameisai.FUNO_FLG = Cmd(2)
                If Cmd.Length = 4 Then
                    Frikaekekkameisai.PRINTERNAME = Cmd(3)
                End If
                '2012/06/30 標準版　WEB伝送対応------------->
                If Cmd.Length = 5 Then
                    Frikaekekkameisai.INVOKE_KBN = Cmd(4)
                End If
                '-------------------------------------------<
