...
Public Class KFSP011
    ...
    Public Sub New(ByVal CSVFileName As String, SousinKbn As String)
        Try
            ...
            If SousinKbn = "2" Then
                ReportBaseName = System.IO.Path.Combine(strLSTPath, "KFSP011_為替振込明細表(他行為替)(MT代行).rpd")
            Else
                ReportBaseName = System.IO.Path.Combine(strLSTPath, "KFSP011_為替振込明細表(他行為替).rpd")
            End If
    ...
    '        ReportBaseName = strLSTPass & "KFSP011_為替振込明細表(他行為替).rpd"
    '        'ＣＳＶファイル名セット(フルパス)
    '        MyBase.CsvName = CSVFileName
    '    Catch ex As Exception
    '    End Try
