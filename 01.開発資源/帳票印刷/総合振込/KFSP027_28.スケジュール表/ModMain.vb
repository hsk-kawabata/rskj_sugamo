...
Module ModMain
    ...
    Public BatchLog As New CASTCommon.BatchLOG("KFSP027", "スケジュール表")
    ...
    Public FuriDate As String        '指定日
    Public PrintNo As Integer        '印刷帳票(1:月間 2:振込日指定)
    Public PrintKbn As Integer       '印刷区分
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            FuriDate = Cmd(1)         '対象年月/振込日
            PrintNo = CInt(Cmd(2))    '印刷帳票
            PrintKbn = CInt(Cmd(3))   '印刷区分
            BatchLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "パラメータ取得", "成功", "コマンドライン引数：" & CmdArgs(0))
            '---------------------------------------------------------
            ' スケジュール表印刷処理
            '---------------------------------------------------------
            Dim SCH As New KFSP027(PrintNo)
            SCH.CreateCsvFile()
            If SCH.MakeRecord = False Then
            ...
            '印刷処理実行
            If SCH.ReportExecute(PrinterName) = True Then
                '印刷成功
            Else
                '印刷失敗
                BatchLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "RepoAgent印刷", "失敗", SCH.ReportMessage)
