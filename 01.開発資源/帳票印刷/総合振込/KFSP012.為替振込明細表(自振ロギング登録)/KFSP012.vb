...
Public Class KFSP012
    ...
    Public Sub New(ByVal CSVFileName As String, SousinKbn As String)
        Try
            ...
            If SousinKbn = "2" Then
                ReportBaseName = System.IO.Path.Combine(strLSTPath, "KFSP012_為替振込明細表(自振ロギング登録)(MT代行).rpd")
            Else
                ReportBaseName = System.IO.Path.Combine(strLSTPath, "KFSP012_為替振込明細表(自振ロギング登録).rpd")
            End If
    ...
    '        ReportBaseName = strLSTPass & "KFSP012_為替振込明細表(自振ロギング登録).rpd"
    '        'ＣＳＶファイル名セット(フルパス)
    '        MyBase.CsvName = CSVFileName
    '    Catch ex As Exception
    '    End Try
