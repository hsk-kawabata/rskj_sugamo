...
Public Class KFSP023
    ...
    Private TSIT_NO As String       '取扱支店コード
    ...
    Sub New(ByVal Baitai As String, ByVal Sort As String)
        If Sort = "1" Then
            ' CSVファイルセット
            InfoReport.ReportName = "KFSP023"
            ' 定義体名セット
            ReportBaseName = "KFSP023_総合振込入力チェックリスト.rpd"
        Else
            ' CSVファイルセット
            InfoReport.ReportName = "KFSP024"
            ' 定義体名セット
            ReportBaseName = "KFSP024_総合振込入力チェックリスト(店番ソート).rpd"
        End If
    End Sub
    ...
    Private Function SetENTMAST() As Boolean
        ...
        Try
            ...
            SQL.Append(" FROM S_ENTMAST,S_TORIMAST,TENMAST,S_SCHMAST")
            ...
            'Select Case IRAISYO_SORT
            '    Case "1"
            '        SQL.Append("SINKI_CODE_E,KOKYAKU_NO_E,KEIYAKU_KNAME_E,KEIYAKU_NO_E")
            '    Case "2"
            '        SQL.Append("SINKI_CODE_E,KOKYAKU_NO_E,JYUYOUKA_NO_E,KEIYAKU_NO_E")
            '    Case Else
            '        SQL.Append("SINKI_CODE_E,KOKYAKU_NO_E,KEIYAKU_NO_E")
            'End Select
            ...
            If oraReader.DataReader(SQL) = True Then
                While oraReader.EOF = False
                    ...
                    OutputCsvData("依頼書   総振", True)                                            'サブタイトル
                    ...
                    Select Case GCOM.NzStr(oraReader.GetString("SYUBETU_T", True))                  '種別
                        Case "11"
                    ...
                    OutputCsvData(ConvertKamoku2TO1_K(oraReader.GetString("KAMOKU_E")), True)         '科目(1桁)
                    ...
                    OutputCsvData(GCOM.NzStr(oraReader.GetInt64("FURIKIN_E")), True)                '振込金額
    ...
    Public Function MakeRecord_DENPYO() As Boolean
        ...
        Try
            ...
            SQL.Append(",SYUBETU_T")
            SQL.Append(" FROM S_DENPYOMAST,S_TORIMAST,TENMAST,S_SCHMAST")
            SQL.Append(" WHERE TORIS_CODE_T = TORIS_CODE_E")
            SQL.Append(" AND   TORIF_CODE_T = TORIF_CODE_E")
            SQL.Append(" AND   TORIS_CODE_T = TORIS_CODE_S")
            SQL.Append(" AND   TORIF_CODE_T = TORIF_CODE_S")
            ...
            If oraReader.DataReader(SQL) = True Then
                While oraReader.EOF = False
                    ...
                    OutputCsvData(mMatchingDate, True)                                                    '処理日
                    OutputCsvData(mMatchingTime, True)                                                    'タイムスタンプ
                    OutputCsvData("伝票   総振", True)                                                    'サブタイトル
