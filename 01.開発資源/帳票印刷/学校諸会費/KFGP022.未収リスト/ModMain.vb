...
Module ModMain
    ...
    Public BatchLog As New CASTCommon.BatchLOG("KFGP022", "未収リスト")
    ...
    Public PrintSort_Seito As String    'ソート(生徒番号)
    Public PrintSort_Tuuban As String   'ソート(年度・通番)
    Public PrintSort_Waon As String     'ソート(あいうえお)
    Public PrintSort_Seibetu As String  'ソート(性別)
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            FURI_DATE = Cmd(2)          '振替日
            PrintSort_Seito = Cmd(3)    '帳票ソート順1
            PrintSort_Tuuban = Cmd(4)   '2
            PrintSort_Waon = Cmd(5)     '3
            PrintSort_Seibetu = Cmd(6)  '4
            BatchLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "パラメータ取得", "成功", "コマンドライン引数：" & CmdArgs(0))
            '---------------------------------------------------------
            ' 未収リスト印刷処理
            '---------------------------------------------------------
            Dim List As New KFGP022()
            List.CreateCsvFile()
