...
Module ModMain
    ...
    Public BatchLog As New CASTCommon.BatchLOG("KFGP039", "生徒登録情報一覧")
    ...
    ' Iniファイル項目
    '---------------------------------------------------------
    Public HimokuPtn As String = CASTCommon.GetRSKJIni("RSV2_V1.0.0", "HIMOKU_PTN")
    Public PrintSort As String = CASTCommon.GetRSKJIni("PRINT", "KFGP039_SORT")
    Public PrintHimoku As String = CASTCommon.GetRSKJIni("PRINT", "KFGP039_HIMOKUOUT")

    '---------------------------------------------------------
    ' パラメータ項目
    '---------------------------------------------------------
    Public GakkouCode As String         '学校コード
    Public GakunenCode As String        '学年コード
    Public ClassCode As String          'クラスコード
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            GakkouCode = Cmd(1).Trim    '学校コード
            GakunenCode = Cmd(2).Trim   '学年コード
            ClassCode = Cmd(3).Trim     'クラスコード
            BatchLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "パラメータ取得", "成功", "コマンドライン引数：" & CmdArgs(0))

            '---------------------------------------------------------
            ' 生徒登録情報一覧印刷処理
            '---------------------------------------------------------
            Dim List As New KFGP039()
            List.CreateCsvFile()
