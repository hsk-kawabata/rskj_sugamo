...
Module ModMain
    ...
    Public BatchLog As New CASTCommon.BatchLOG("KFGP014", "口座振替予定一覧表(口座データ)")
    ...
    Public SFURI_DATE As String         '再振日
    Public SEIKYU_TUKI As String        '請求月
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            FURI_DATE = Cmd(2)              '振替日(再振スケジュール)
            SFURI_DATE = Cmd(3)             '再振日(初振スケジュール)
            SEIKYU_TUKI = Cmd(4)            '請求月
            PrintSort = Cmd(5)              '帳票ソート順
            FunouKen = GCOM.NzLong(Cmd(6))  '不能件数
            BatchLog.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "パラメータ取得", "成功", "コマンドライン引数：" & CmdArgs(0))
            '---------------------------------------------------------
            ' 口座振替予定一覧表(口座データ)印刷処理
            '---------------------------------------------------------
            Dim List As New KFGP014(FunouKen)
            List.CreateCsvFile()
            Select Case FunouKen
                Case Is > 0
                    ...
                    End If
                Case Else
                    If List.MakeRecord_Zero = False Then
                        If RecordCnt = -1 Then   '印刷対象なし
                            Return -1
                        Else
                            Return -300 '処理中のエラー
