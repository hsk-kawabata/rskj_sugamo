...
Class ClsPrnJifurisyori
    ...
    Sub New()
        ' CSVファイルセット
        InfoReport.ReportName = "KFKP004"

        ' 定義体名セット
        ReportBaseName = "KFKP004_自振処理企業一覧表.rpd"
    End Sub
    ...
    Public Function MakeRecord() As Boolean
        ...
        SQL.Append(",SCHMAST.KESSAI_YDATE_S")       '手数料徴求予定日
        ...
        SQL.Append(" AND SCHMAST.FURI_DATE_S = " & SQ(txtFURI_DATE))
        SQL.Append(" AND SCHMAST.TYUUDAN_FLG_S = '0'")
        SQL.Append(" AND SCHMAST.TOUROKU_FLG_S = '1'")
        ...
        Try
            ...
            If bSQL = True Then
                Do
                    ...
                    substrTESUU_DATE = OraReader.GetString("TESUU_YDATE_S")
                    If substrTESUU_DATE = "00000000" Then
                        substrTESUU_DATE = ""
                    End If
                    OutputCsvData(substrTESUU_DATE, True)                       '手数料徴求(予定)日
                    ...
                    '2017/05/16 タスク）西野 ADD 標準版修正（資金決済帳票に振替手数料単価追加）----------------- START
                    OutputCsvData(OraReader.GetString("KIHTESUU_T"), True)          '振替手数料単価追加
                    '2017/05/16 タスク）西野 ADD 標準版修正（資金決済帳票に振替手数料単価追加）----------------- END
