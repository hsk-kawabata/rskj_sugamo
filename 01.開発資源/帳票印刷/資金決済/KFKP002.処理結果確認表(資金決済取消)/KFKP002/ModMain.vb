...
Module ModMain
    ...
    Private BatchLog As New CASTCommon.BatchLOG("KFKP002", "処理結果確認表(資金決済取消)")

    '
    ' 機能　 ：処理結果確認表(資金決済取消)印刷 メイン処理
    '
    ' 引数　 ：ARG1 - 起動パラメータ
    '
    ' 戻り値 ： 0    － 正常
    ...
    Function Main(ByVal CmdArgs() As String) As Integer
        ...
        Try
            ...
            If CmdArgs.Length = 0 Then
                'パラメータ取得失敗
                BatchLog.Write("", Toris_Code & "-" & Torif_Code, "00000000", "パラメータチェック", "失敗", "コマンドライン引数なし")
                Return -100
            End If
            ...
            If Cmd.Length <> 2 Then
                'パラメータ間違い
                BatchLog.Write("", Toris_Code & "-" & Torif_Code, "00000000", "パラメータチェック", "失敗", "コマンドライン引数異常：" & CmdArgs(0))
                Return -100
            End If
            ...
            ' 処理結果確認表(資金決済取消)印刷処理
            '---------------------------------------------------------
            Dim FUNOU As New KFKP002(CSVFileName)

            '印刷処理実行
            If FUNOU.ReportExecute(PrinterName) = True Then
                '印刷成功
            Else
                '印刷失敗
                BatchLog.Write(LoginID, "0000000000-00", "00000000", "RepoAgent印刷", "失敗", FUNOU.ReportMessage)
