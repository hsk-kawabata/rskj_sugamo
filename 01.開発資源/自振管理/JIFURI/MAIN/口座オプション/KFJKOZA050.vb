...
#Region " 定数/変数 "
    ...
    ' LOG関連項目
    '--------------------------------
    Private MainLOG As New CASTCommon.BatchLOG("KFJKOZA050", "自振契約リエンタ書込画面")
    Private Const msgtitle As String = "自振契約リエンタ書込画面(KFJKOZA050)"
    ...
    Friend Structure INI_INFO
        ...
        Dim COMMON_JIKEI_RIENTAFILENAME As String    ' リエンタファイル名
    End Structure
...
#Region " ロード "
    ...
    Private Sub KFJKOZA050_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region " ボタン "
    ...
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            If MessageBox.Show(String.Format(MSG0077I, "自振契約リエンタ", "媒体書込処理"), msgtitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = DialogResult.Cancel Then
                Exit Try
            End If
            ...
            FileName = "RNT_JR_" & TimeStamp_Date & "_" & TimeStamp_Time & "_" & "1"
            ...
            Select Case iRet
                Case 0          ' データ格納成功
                    If File.Exists(Path.Combine(IniInfo.COMMON_RIENTADR, IniInfo.COMMON_JIKEI_RIENTAFILENAME)) Then
                    ...
                    If iRet = 0 Then
                        File.Copy(Path.Combine(IniInfo.COMMON_BAITAIWRITE, FileName), Path.Combine(IniInfo.COMMON_RIENTADR, IniInfo.COMMON_JIKEI_RIENTAFILENAME), True)
                    End If
            End Select
            ...
            '------------------------------------------------
            If iRet = 0 Then
                MessageBox.Show(String.Format(MSG0078I, "自振契約リエンタの媒体書込"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "実行", "成功", "出力ファイル:" & IniInfo.COMMON_BAITAIWRITE & FileName)
            Else
                MessageBox.Show(String.Format(MSG0080I, "自振契約リエンタの媒体書込", "中断"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
...
#Region " 関数(Function)"
    ...
    Private Function SetIniFIle() As Boolean
        ...
        Try
            ...
            IniInfo.COMMON_JIKEI_RIENTAFILENAME = CASTCommon.GetFSKJIni("COMMON", "JIKEI_RIENTAFILENAME")
            If IniInfo.COMMON_JIKEI_RIENTAFILENAME.ToUpper = "ERR" OrElse IniInfo.COMMON_JIKEI_RIENTAFILENAME = Nothing Then
                MessageBox.Show(String.Format(MSG0001E, "リエンタファイル名", "COMMON", "JIKEI_RIENTAFILENAME"), msgtitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "INI情報取得", "失敗", "項目名:リエンタファイル名 分類:COMMON 項目:JIKEI_RIENTAFILENAME")
                Return False
            End If
