...
#Region " 変数"
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFJENTR041", "伝票用情報入力画面")
    Private Const msgTitle As String = "伝票用情報入力画面(KFJENTR041)"
...
#End Region
#Region " ロード"
    Private Sub KFJENTR041_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '#####################################
            'ログの書込に必要な情報の取得
...
#Region "削除"
    ...
    Private Sub btnGyoSakujo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGyoSakujo.Click
        Try
            ...
            Else
                Sakujyogyo = CInt(txtSounyuKoban.Text) - CInt((page - 1) * 15)
            End If
...
start:
    ...
    Private Function fn_GYO_SAKUJO() As Boolean
        ...
        '表示項目一時保持用配列
        Dim strDEL_CHK_A(intMAXCNT, 15) As String
        Dim strTKIN_NO_A(intMAXCNT, 15) As String
        Dim strTKIN_NAME_A(intMAXCNT, 15) As String
        Dim strTSIT_NO_A(intMAXCNT, 15) As String
        Dim strTSIT_NAME_A(intMAXCNT, 15) As String
        Dim strKAMOKU_A(intMAXCNT, 15) As String
        Dim strKOUZA_A(intMAXCNT, 15) As String
        Dim strKEIYAKU_KNAME_A(intMAXCNT, 15) As String
        Dim lngFURIKIN_A(intMAXCNT, 15) As Long
        Dim strBIKOU_A(intMAXCNT, 15) As String
        Dim lngRECORD_NO_A(intMAXCNT, 15) As Long

        '背景色一時保持用配列
        Dim clrTKIN_NO_Color_A(intMAXCNT, 15) As Color
        Dim clrTKIN_NAME_Color_A(intMAXCNT, 15) As Color
        Dim clrTSIT_NO_Color_A(intMAXCNT, 15) As Color
        Dim clrTSIT_NAME_Color_A(intMAXCNT, 15) As Color
        Dim clrKAMOKU_Color_A(intMAXCNT, 15) As Color
        Dim clrKOUZA_Color_A(intMAXCNT, 15) As Color
        Dim clrKEIYAKU_NNAME_Color_A(intMAXCNT, 15) As Color
        Dim clrFURIKIN_Color_A(intMAXCNT, 15) As Color
        ...
        Try
            '※指定項番のページ・行を取得（これ以前のデータは変更しない）
            '2011/06/13 if分追加
            If CDbl(txtSounyuKoban.Text) Mod 15 = 0 Then
                SakujoPage = Math.Floor(CDbl(txtSounyuKoban.Text) / 15)  '指定項番のあるページ
                SakujoGyo = 15                '指定項番の行
            Else
                SakujoPage = Math.Floor(CDbl(txtSounyuKoban.Text) / 15) + 1 '指定項番のあるページ
                SakujoGyo = CDbl(txtSounyuKoban.Text) Mod 15                '指定項番の行
            End If
            '※現在行の情報を一時的に保持
            ...
            For P = SakujoPage To intMAXCNT
                ...
                For L = i To 15
                    ...
                    If GetPage > intMAXCNT And GetGyo = 1 Then
                        '2011/06/13最終ページの15行目の場合は、初期値セット
                        strDEL_CHK(P, L) = "0"
                        strTKIN_NO(P, L) = ""
                        strTSIT_NO(P, L) = ""
                        strTKIN_NAME(P, L) = ""
