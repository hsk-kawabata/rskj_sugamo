...
#Region " 変数"
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFJENTR042", "伝票用情報入力画面")
    Private Const msgTitle As String = "伝票用情報入力画面(KFJENTR042)"
    ...
    Private strCENTER_NM As String
    ...
    Private strTOUROKU_FLG As String
    'SQLキー項目用
    ...
    Private lngSYOUKEI2 As Long  '解約数または振替金額0円のものを引いた小計件数
    Private lngKAIYAKU_CNT As Long '全解約数
    Private lngPAGE_CNT As Long
    Private lngGYO_CNT As Long

    '画面表示項目用配列
    Public strDEL_CHK(700, 20) As String
    Public lngKOUBAN(700, 20) As Long
    Public strTKIN_NO(700, 20) As String
    Public strTKIN_NAME(700, 20) As String
    Public strTSIT_NO(700, 20) As String
    Public strTSIT_NAME(700, 20) As String
    Public strKAMOKU(700, 20) As String
    Public strKOUZA(700, 20) As String
    Public strKEIYAKU_KNAME(700, 20) As String
    Public lngFURIKIN(700, 20) As Long
    Public strBIKOU(700, 20) As String
    Public lngRECORD_NO(700, 20) As Long
    '画面の小計
    Public lngSYOUKEI_KEN(700) As Long
    Public lngGOUKEI_KEN As Long

    Private strD_TKIN_NO As String                ' 取引先マスタ 振替処理区分
    Private strD_TSIT_NO As String                ' 取引先マスタ 振替処理区分
    Private strD_KAMOKU_CD As String                ' 取引先マスタ 振替処理区分
    Private strD_KOUZA As String                ' 取引先マスタ 振替処理区分
    Private strD_KEIYAKU_NNAME As String                ' 取引先マスタ 振替処理区分
    Private strD_FURIKIN As String
    'ページの色を取得
    Private clrTKIN_NO_Color(700, 20) As Color
    Private clrTKIN_NAME_Color(700, 20) As Color
    Private clrTSIT_NO_Color(700, 20) As Color
    Private clrTSIT_NAME_Color(700, 20) As Color
    Private clrKAMOKU_Color(700, 20) As Color
    Private clrKOUZA_Color(700, 20) As Color
    Private clrKEIYAKU_NNAME_Color(700, 20) As Color
    Private clrFURIKIN_Color(700, 20) As Color

    Private strINPUT_FLG() As String
    Private strINPUT_K_SYOUKEI() As String
    Private strINPUT_T_SYOUKEI() As String

    'コントロール配列
    Dim ckbDel(20) As CheckBox
    Dim lblIndex(20) As Label
    Dim txtKinNo(20) As TextBox
    Dim txtKinName(20) As TextBox
    Dim txtSitNo(20) As TextBox
    Dim txtSitName(20) As TextBox
    Dim txtKamokuCode(20) As TextBox
    Dim lblKamoku(20) As Label
    Dim txtKouza(20) As TextBox
    Dim txtFurikin(20) As TextBox
    Dim txtKeiyakuName(20) As TextBox
    Dim lblBikou(20) As Label

    '2018/02/09 saitou 広島信金(RSV2標準) ADD エントリ20明細対応 -------------------- START
    'M_FUSIONのグローバル変数宣言をやめ、画面毎にグローバル変数を持つように変更。
    '本画面で使用しているM_FUSIONのグローバル変数を一括置換する。
    ...
    Private KOUZACHK As String
    Private CHKDEJIT As String
    '2013/10/24 saitou 標準修正 ADD --------------------------------------------------<<<<
...
#End Region
#Region " ロード"
    Private Sub KFJENTR042_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '#####################################
            'ログの書込に必要な情報の取得
            ...
            '2018/02/09 saitou 広島信金(RSV2標準) DEL エントリ20明細対応 -------------------- START
            '不要
            ''取引先名表示
            'lblToriNName.Text = gstrTORI_NAME
            '2018/02/09 saitou 広島信金(RSV2標準) DEL --------------------------------------- END
            '初期値表示
            ...
            ' 2016/09/21 タスク）西野 ADD 【PG】青梅信金 カスタマイズ対応(UI_12-7) -------------------- START
            '入力モード表示
            If intCHK = 0 Then
                lblInputMode.Text = "正データ入力"
            Else
                lblInputMode.Text = "副データ入力"
            End If
            ' 2016/09/21 タスク）西野 ADD 【PG】青梅信金 カスタマイズ対応(UI_12-7) -------------------- END
...
#Region " 口座チェック"
    Private Sub btnKOUZACHK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnKOUZACHK.Click
        Try
            ...
            '    strKDB_KNAME = txtKeiyakuName(No).Text  '契約者名退避

            '    '空白行は無視
            '    If txtKinNo(No).Text.Trim = "" AndAlso txtSitNo(No).Text.Trim = "" AndAlso txtKamokuCode(No).Text.Trim = "" _
            '       AndAlso txtKouza(No).Text.Trim = "" AndAlso txtKeiyakuName(No).Text.Trim = "" Then
            '        lblBikou(No).Text = ""
            '    ElseIf txtKinNo(No).Text.Trim = strJIKINKO_CD AndAlso gstrCHK_KOUZA = "YES" Then
            '        '口座チェックを実行
            '        Ret = GCom.KouzaChk_ENTRY(txtSitNo(No).Text, CASTCommon.ConvertKamoku1TO2(txtKamokuCode(No).Text), _
            '                                  txtKouza(No).Text, strKIGYO_CODE, _
            '                                  strFURI_CODE, strKDB_KNAME, strERR_MSG, MainDB)
            '        Select Case Ret
            '            Case -1 '口座チェック失敗
            '                lblBikou(No).Text = "チェック失敗"
            '            Case Else
            '                lblBikou(No).Text = strERR_MSG
            '                If intCHK = 0 Then       '正にチェックの場合 条件移動 
            '                    If strERR_MSG <> "" Then
            '                        lblBikou(No).Text = strERR_MSG
            '                        strERR_COLOR = Color.Yellow
            '                        txtKinNo(No).BackColor = strERR_COLOR
            '                        txtKinName(No).BackColor = strERR_COLOR
            '                        txtSitNo(No).BackColor = strERR_COLOR
            '                        txtSitName(No).BackColor = strERR_COLOR
            '                        txtKamokuCode(No).BackColor = strERR_COLOR
            '                        txtKouza(No).BackColor = strERR_COLOR
            '                        txtKeiyakuName(No).BackColor = strERR_COLOR
            '                        txtFurikin(No).BackColor = strERR_COLOR
            '                    Else
            '                        lblBikou(No).Text = ""
            ...
            '                    End If
            '                End If
            '                If (Ret = 0 OrElse Ret = 2) AndAlso txtKeiyakuName(No).Text = "" Then   '口座無し以外の場合は契約者カナ名設定
            '                    txtKeiyakuName(No).Text = GCom.NzStr(strKDB_KNAME.Trim)
            '                End If
            '        End Select
            '    Else
            '        lblBikou(No).Text = ""
            '    End If
            '    If intMAXCNT = intPAGE_CNT And No = 15 Then
            '        Exit Sub
            '    End If
            'Next
                ...
                For No As Integer = 1 To 20
                    ...
                    If strTKIN_NO(lngPAGE_LOOP, No).Trim = "" AndAlso strTSIT_NO(lngPAGE_LOOP, No).Trim = "" AndAlso strKAMOKU(lngPAGE_LOOP, No).Trim = "" _
                       AndAlso strKOUZA(lngPAGE_LOOP, No).Trim = "" AndAlso strKEIYAKU_KNAME(lngPAGE_LOOP, No).Trim = "" Then
                        ...
                        '比較する配列間違い
                    ElseIf strTKIN_NO(lngPAGE_LOOP, No).Trim = strJIKINKO_CD AndAlso KOUZACHK = "YES" Then
                        'ElseIf txtKinNo(No).Text.Trim = strJIKINKO_CD AndAlso KOUZACHK = "YES" Then
                        '2014/05/01 saitou 標準修正 MODIFY -----------------------------------------------<<<<
                        '口座チェックを実行
...
#Region " 挿入"
    ...
    Private Sub btnGyoSounyu_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGyoSounyu.Click
        Try
            ...
            If (intMAXCNT - 1) * 20 + intMAXLINE < CDbl(txtSounyuKoban.Text) Then '指定項番チェック
            ...
            Dim page As Integer
            If GCom.NzLong(txtSounyuKoban.Text.Trim) Mod 20 = 0 Then
                page = Math.Floor(GCom.NzLong(txtSounyuKoban.Text.Trim) / 20)
            Else
                page = Math.Floor(GCom.NzLong(txtSounyuKoban.Text.Trim) / 20) + 1
            End If
            ...
            Dim No As Integer
            No = GCom.NzLong(txtSounyuKoban.Text.Trim) Mod 20
            If No = 0 Then No = 20
...
#Region " 登録"
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            If intMAXCNT = intPAGE_CNT And intMAXLINE = 20 Then
                intMAXCNT += 1
                intMAXLINE = 0
...
#Region "削除"
    ...
    '指定行削除
    '*******************************************
    Private Sub btnGyoSakujo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGyoSakujo.Click
        Try
            ...
            Dim Sakujyogyo As Integer

            If page = 1 Then
                Sakujyogyo = CInt(txtSounyuKoban.Text)
            Else
                Sakujyogyo = CInt(txtSounyuKoban.Text) - CInt((page - 1) * 20)
            End If

            '対象行が空白かをチェックする
            If Trim(txtSitNo(Sakujyogyo).Text) <> "" OrElse _
               Trim(txtKamokuCode(Sakujyogyo).Text) <> "" OrElse _
               Trim(txtKouza(Sakujyogyo).Text) <> "" OrElse _
               Trim(txtKeiyakuName(Sakujyogyo).Text) <> "" OrElse _
               Trim(txtFurikin(Sakujyogyo).Text) <> "" Then
                '確認メッセージを表示する
                If MessageBox.Show(MSG0084I, msgTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.No Then
                    txtSounyuKoban.Focus()
                    Return
                End If
            End If
            ...
            If fn_GYO_SAKUJO() = False Then  '行の削除処理
                Return
            End If
        ...
        Catch ex As Exception
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(削除)", "失敗", ex.ToString)
        Finally
...
#Region " 関数"
    Function fn_select_SCHMAST() As Boolean
        ...
        'Parameter      :
        'Description    :SCHMAST・TORIMASTから取引先情報取得
    ...
    Function fn_set_INIFILE() As Boolean
        ...
        Try
            ...
            'チェックデジット
            CHKDEJIT = CASTCommon.GetFSKJIni("COMMON", "CHKDEJIT")
            If CHKDEJIT.ToUpper = "ERR" OrElse CHKDEJIT = Nothing Then
            ...
            '口座チェック判定
            KOUZACHK = CASTCommon.GetFSKJIni("COMMON", "KOUZACHK")
            If KOUZACHK.ToUpper = "ERR" OrElse KOUZACHK = Nothing Then
    ...
    Function fn_CHECK_FURIKIN(ByVal aobjKIN_CODE As TextBox, ByVal aobjKIN_NAME As TextBox, _
        ...
        '               :aobjFURIKIN：振替金額／aobjERR：エラー内容／aobjNEXT：オブジェクト
        '               :aobjKOUBAN：項番／aintPAGE：ページ番号／aintGYO：行番号
        'Description    :振替金額エンターキー押下時のチェック処理
        ...
        Try
            ...
            '-------------------------------------------------
            If intCHK = 0 Then       '正にチェックの場合
                If aobjKIN_CODE.Text = strJIKINKO_CD And CHKDEJIT = "YES" Then
                    ...
                    If GCom.ChkDejit_ENTRY(aobjKIN_CODE.Text, aobjSIT_CODE.Text, _
                       CASTCommon.ConvertKamoku1TO2(aobjKAMOKU.Text), aobjKOUZA.Text) = False Then
            ...
            '-------------------------------------------------
            If aobjKIN_CODE.Text = strJIKINKO_CD And KOUZACHK = "YES" Then
            ...
            If intCHK = 1 Then       '副にチェックの場合
                If fn_BERYFAI(aintGYO + ((aintPAGE - 1) * 20), aobjKIN_CODE, _
            ...
            '項番の表示
            '-------------------------------------------------
            aobjKOUBAN.Text = aintGYO + ((aintPAGE - 1) * 20)
    ...
    Function fn_COUNT_SYOUKEI_KEN() As Boolean
        ...
        Try
            ...
            If txtFuriKin15.Text <> "" And txtFuriKin15.Text <> "0" Then
                intSYOUKEI_KEN += 1
            End If
            If txtFuriKin16.Text <> "" And txtFuriKin16.Text <> "0" Then
                intSYOUKEI_KEN += 1
            End If
            If txtFuriKin17.Text <> "" And txtFuriKin17.Text <> "0" Then
                intSYOUKEI_KEN += 1
            End If
            If txtFuriKin18.Text <> "" And txtFuriKin18.Text <> "0" Then
                intSYOUKEI_KEN += 1
            End If
            If txtFuriKin19.Text <> "" And txtFuriKin19.Text <> "0" Then
                intSYOUKEI_KEN += 1
            End If
            If txtFuriKin20.Text <> "" And txtFuriKin20.Text <> "0" Then
                intSYOUKEI_KEN += 1
            End If
    ...
    Function fn_COUNT_SYOUKEI_KIN() As Long
        ...
        Try
            ...
            End If
            If txtFuriKin16.Text <> "" And txtFuriKin16.Text <> "0" Then
                fn_COUNT_SYOUKEI_KIN += fn_DEL_KANMA(txtFuriKin16.Text)
            End If
            If txtFuriKin17.Text <> "" And txtFuriKin17.Text <> "0" Then
                fn_COUNT_SYOUKEI_KIN += fn_DEL_KANMA(txtFuriKin17.Text)
            End If
            If txtFuriKin18.Text <> "" And txtFuriKin18.Text <> "0" Then
                fn_COUNT_SYOUKEI_KIN += fn_DEL_KANMA(txtFuriKin18.Text)
            End If
            If txtFuriKin19.Text <> "" And txtFuriKin19.Text <> "0" Then
                fn_COUNT_SYOUKEI_KIN += fn_DEL_KANMA(txtFuriKin19.Text)
            End If
            If txtFuriKin20.Text <> "" And txtFuriKin20.Text <> "0" Then
                fn_COUNT_SYOUKEI_KIN += fn_DEL_KANMA(txtFuriKin20.Text)
            End If
        Catch ex As Exception
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Function fn_BERYFAI(ByVal alngKOUBAN As Long, ByVal aobjKIN_NO As TextBox, ByVal aobjSIT_NO As TextBox, _
        ...
        Try
            ...
            '伝票マスタに登録されているか検索
            '-------------------------------------------
            SQL.Append("SELECT * FROM DENPYOMAST")
            SQL.Append(" WHERE TORIS_CODE_E = " & SQ(strTORIS_CODE))
            SQL.Append(" AND TORIF_CODE_E = " & SQ(strTORIF_CODE))
            ...
            Else
                ...
                If aobjKAMOKU.Text <> CASTCommon.ConvertKamoku2TO1(GCom.NzStr(OraReader.GetString("KAMOKU_E"))) Then
                    ...
                    Else
                        ...
                        If MessageBox.Show(MSG, msgTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                            ...
                            If fn_UPDATE_DENPYOMAST_SEI("KAMOKU_E", CASTCommon.ConvertKamoku1TO2(aobjKAMOKU.Text), alngKOUBAN) = False Then
                                MainDB.Rollback()
                                Exit Function
                            End If
                ...
                If fn_DEL_KANMA(aobjFURIKIN.Text) <> GCom.NzLong(OraReader.GetString("FURIKIN_E")) Then
                    MSG = String.Format(MSG0028I, "振替金額", Format(GCom.NzLong(OraReader.GetString("FURIKIN_E")), "#,##0"), GCom.NzLong(fn_DEL_KANMA(aobjFURIKIN.Text)).ToString("#,##0"))
                    If MessageBox.Show(MSG, msgTitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
...
start:
    ...
    Function fn_UPDATE_DENPYOMAST_SEI(ByVal astrFIELD As String, ByVal astrATAI As String, ByVal alngKOUBAN As Long) As Boolean
        ...
        Try
            ...
            SQL.Append(" UPDATE DENPYOMAST SET ")
    ...
    Function fn_OBJECT_SET() As Boolean
        ...
        Try
            ...
            lblIndex(16) = lblIndex16
            lblIndex(17) = lblIndex17
            lblIndex(18) = lblIndex18
            lblIndex(19) = lblIndex19
            lblIndex(20) = lblIndex20
            '金融機関番号
            ...
            txtKinNo(16) = txtKinNO16
            txtKinNo(17) = txtKinNO17
            txtKinNo(18) = txtKinNO18
            txtKinNo(19) = txtKinNO19
            txtKinNo(20) = txtKinNO20
            '金融機関名
            ...
            txtKinName(16) = txtKinName16
            txtKinName(17) = txtKinName17
            txtKinName(18) = txtKinName18
            txtKinName(19) = txtKinName19
            txtKinName(20) = txtKinName20
            '支店番号
            ...
            txtSitNo(16) = txtSitNo16
            txtSitNo(17) = txtSitNo17
            txtSitNo(18) = txtSitNo18
            txtSitNo(19) = txtSitNo19
            txtSitNo(20) = txtSitNo20
            '支店名
            ...
            txtSitName(16) = txtSitName16
            txtSitName(17) = txtSitName17
            txtSitName(18) = txtSitName18
            txtSitName(19) = txtSitName19
            txtSitName(20) = txtSitName20
            '科目コード
            ...
            txtKamokuCode(16) = txtKamokuCode16
            txtKamokuCode(17) = txtKamokuCode17
            txtKamokuCode(18) = txtKamokuCode18
            txtKamokuCode(19) = txtKamokuCode19
            txtKamokuCode(20) = txtKamokuCode20
            '科目名
            ...
            lblKamoku(16) = lblKamoku16
            lblKamoku(17) = lblKamoku17
            lblKamoku(18) = lblKamoku18
            lblKamoku(19) = lblKamoku19
            lblKamoku(20) = lblKamoku20
            '口座番号
            ...
            txtKouza(16) = txtKouza16
            txtKouza(17) = txtKouza17
            txtKouza(18) = txtKouza18
            txtKouza(19) = txtKouza19
            txtKouza(20) = txtKouza20
            '振替金額
            ...
            txtFurikin(16) = txtFuriKin16
            txtFurikin(17) = txtFuriKin17
            txtFurikin(18) = txtFuriKin18
            txtFurikin(19) = txtFuriKin19
            txtFurikin(20) = txtFuriKin20
            '契約者名
            ...
            txtKeiyakuName(16) = txtKeiyakuName16
            txtKeiyakuName(17) = txtKeiyakuName17
            txtKeiyakuName(18) = txtKeiyakuName18
            txtKeiyakuName(19) = txtKeiyakuName19
            txtKeiyakuName(20) = txtKeiyakuName20
            '備考
            ...
            lblBikou(16) = lblBikou16
            lblBikou(17) = lblBikou17
            lblBikou(18) = lblBikou18
            lblBikou(19) = lblBikou19
            lblBikou(20) = lblBikou20
        Catch ex As Exception
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Function fn_GYO_SOUNYU() As Boolean
        ...
        '表示項目一時保持用配列
        Dim strDEL_CHK_A(20) As String
        Dim strTKIN_NO_A(20) As String
        Dim strTKIN_NAME_A(20) As String
        Dim strTSIT_NO_A(20) As String
        Dim strTSIT_NAME_A(20) As String
        Dim strKAMOKU_A(20) As String
        Dim strKOUZA_A(20) As String
        Dim strKEIYAKU_KNAME_A(20) As String
        Dim lngFURIKIN_A(20) As Long
        Dim strBIKOU_A(20) As String
        Dim lngRECORD_NO_A(20) As Long

        '背景色一時保持用配列
        Dim clrTKIN_NO_Color_A(20) As Color
        Dim clrTKIN_NAME_Color_A(20) As Color
        Dim clrTSIT_NO_Color_A(20) As Color
        Dim clrTSIT_NAME_Color_A(20) As Color
        Dim clrKAMOKU_Color_A(20) As Color
        Dim clrKOUZA_Color_A(20) As Color
        Dim clrKEIYAKU_NNAME_Color_A(20) As Color
        Dim clrFURIKIN_Color_A(20) As Color

        '項番設定
        Dim RecordNo As Long = 0
        Try
            '※指定項番のページ・行を取得（これ以前のデータは変更しない）
            SounyuPage = Math.Floor(CDbl(txtSounyuKoban.Text) / 20) + 1 '指定項番のあるページ

            SounyuGyo = CDbl(txtSounyuKoban.Text) Mod 20                '指定項番の行
            If SounyuGyo = 0 Then
                SounyuGyo = 20
                SounyuPage -= 1
            End If

            If intMAXLINE = 20 Then
                '※登録処理を促し、挿入処理を中断する
    ...
    'NAME           :fn_GYO_SAKUJO
    'Parameter      :
    'Description    :指定された行を削除
    'Return         :True=OK,False=NG
    'Create         :2010/12/28
    'Update         :
    '============================================================================
    Private Function fn_GYO_SAKUJO() As Boolean
        Dim SakujoPage As Integer
        Dim SakujoGyo As Integer
        Dim i As Integer
        Dim GetPage As Integer
        Dim GetGyo As Integer


        '表示項目一時保持用配列
        Dim strDEL_CHK_A(intMAXCNT, 20) As String
        Dim strTKIN_NO_A(intMAXCNT, 20) As String
        Dim strTKIN_NAME_A(intMAXCNT, 20) As String
        Dim strTSIT_NO_A(intMAXCNT, 20) As String
        Dim strTSIT_NAME_A(intMAXCNT, 20) As String
        Dim strKAMOKU_A(intMAXCNT, 20) As String
        Dim strKOUZA_A(intMAXCNT, 20) As String
        Dim strKEIYAKU_KNAME_A(intMAXCNT, 20) As String
        Dim lngFURIKIN_A(intMAXCNT, 20) As Long
        Dim strBIKOU_A(intMAXCNT, 20) As String
        Dim lngRECORD_NO_A(intMAXCNT, 20) As Long

        '背景色一時保持用配列
        Dim clrTKIN_NO_Color_A(intMAXCNT, 20) As Color
        Dim clrTKIN_NAME_Color_A(intMAXCNT, 20) As Color
        Dim clrTSIT_NO_Color_A(intMAXCNT, 20) As Color
        Dim clrTSIT_NAME_Color_A(intMAXCNT, 20) As Color
        Dim clrKAMOKU_Color_A(intMAXCNT, 20) As Color
        Dim clrKOUZA_Color_A(intMAXCNT, 20) As Color
        Dim clrKEIYAKU_NNAME_Color_A(intMAXCNT, 20) As Color
        Dim clrFURIKIN_Color_A(intMAXCNT, 20) As Color
        ...
        Try
            '※指定項番のページ・行を取得（これ以前のデータは変更しない）
            '2011/06/13 if分追加
            If CDbl(txtSounyuKoban.Text) Mod 20 = 0 Then
                SakujoPage = Math.Floor(CDbl(txtSounyuKoban.Text) / 20)  '指定項番のあるページ
                SakujoGyo = 20                '指定項番の行
            Else
                SakujoPage = Math.Floor(CDbl(txtSounyuKoban.Text) / 20) + 1 '指定項番のあるページ
                SakujoGyo = CDbl(txtSounyuKoban.Text) Mod 20                '指定項番の行
            End If
            '※現在行の情報を一時的に保持
            strDEL_CHK_A = strDEL_CHK
            strTKIN_NO_A = strTKIN_NO
            strTSIT_NO_A = strTSIT_NO
            strTKIN_NAME_A = strTKIN_NAME
            strTSIT_NAME_A = strTSIT_NAME
            strKAMOKU_A = strKAMOKU
            strKOUZA_A = strKOUZA
            strKEIYAKU_KNAME_A = strKEIYAKU_KNAME
            lngFURIKIN_A = lngFURIKIN
            strBIKOU_A = strBIKOU
            lngRECORD_NO_A = lngRECORD_NO

            '※現在行の背景色情報を一時的に保持
            clrTKIN_NO_Color_A = clrTKIN_NO_Color
            clrTKIN_NAME_Color_A = clrTKIN_NAME_Color
            clrTSIT_NO_Color_A = clrTSIT_NO_Color
            clrTSIT_NAME_Color_A = clrTSIT_NAME_Color '2011/07/29
            clrKAMOKU_Color_A = clrKAMOKU_Color
            clrKOUZA_Color_A = clrKOUZA_Color
            clrKEIYAKU_NNAME_Color_A = clrKEIYAKU_NNAME_Color
            clrFURIKIN_Color_A = clrFURIKIN_Color

            If intMAXLINE = 0 Then  '最終ページの最大行が0の場合、マックスページを－１とする
                intMAXCNT -= 1
                '2012/02/24 saitou 青梅信金 MODIFY ------------------------------------->>>>
                '最終ページの最終行が0の場合、最大行を20行に設定する
                intMAXLINE = 20
                '2012/02/24 saitou 青梅信金 MODIFY -------------------------------------<<<<
            End If

            '※削除行以降は一行後の情報をセットする
            For P = SakujoPage To intMAXCNT
                If P = SakujoPage Then
                    i = SakujoGyo
                Else
                    i = 1
                End If
                ...
                For L = i To 20
                    ...
                    If P = SakujoPage And L = SakujoGyo Then
                        RecordNo = L + ((P - 1) * 20)
                    Else
                        RecordNo += 1
                    End If
                    ...
                    '※値を設定
                    If GetPage > intMAXCNT And GetGyo = 1 Then
                        '2011/06/13最終ページの20行目の場合は、初期値セット
                        strDEL_CHK(P, L) = "0"
                        strTKIN_NO(P, L) = ""
                        strTSIT_NO(P, L) = ""
                        strTKIN_NAME(P, L) = ""
                        ...
                        clrFURIKIN_Color(P, L) = Color.White
                    Else
                        strDEL_CHK(P, L) = strDEL_CHK_A(GetPage, GetGyo)
                        strTKIN_NO(P, L) = strTKIN_NO_A(GetPage, GetGyo)
                        strTSIT_NO(P, L) = strTSIT_NO_A(GetPage, GetGyo)
                        strTKIN_NAME(P, L) = strTKIN_NAME_A(GetPage, GetGyo)
                        strTSIT_NAME(P, L) = strTSIT_NAME_A(GetPage, GetGyo)
                        strKAMOKU(P, L) = strKAMOKU_A(GetPage, GetGyo)
                        strKOUZA(P, L) = strKOUZA_A(GetPage, GetGyo)
                        strKEIYAKU_KNAME(P, L) = strKEIYAKU_KNAME_A(GetPage, GetGyo)
                        lngFURIKIN(P, L) = lngFURIKIN_A(GetPage, GetGyo)
                        strBIKOU(P, L) = strBIKOU_A(GetPage, GetGyo)
                        lngRECORD_NO(P, L) = RecordNo

                        '※背景色を設定
                        clrTKIN_NO_Color(P, L) = clrTKIN_NO_Color_A(GetPage, GetGyo)
                        clrTKIN_NAME_Color(P, L) = clrTKIN_NAME_Color_A(GetPage, GetGyo)
                        clrTSIT_NO_Color(P, L) = clrTSIT_NO_Color_A(GetPage, GetGyo)
                        clrTSIT_NAME_Color(P, L) = clrTSIT_NAME_Color_A(GetPage, GetGyo)
                        clrKAMOKU_Color(P, L) = clrKAMOKU_Color_A(GetPage, GetGyo)
                        clrKOUZA_Color(P, L) = clrKOUZA_Color_A(GetPage, GetGyo)
                        clrKEIYAKU_NNAME_Color(P, L) = clrKEIYAKU_NNAME_Color_A(GetPage, GetGyo)
                        clrFURIKIN_Color(P, L) = clrFURIKIN_Color_A(GetPage, GetGyo)
                    End If

                    'If P = intMAXCNT AndAlso L = intMAXLINE Then
                    If GetPage >= intMAXCNT AndAlso GetGyo >= intMAXLINE Then  '2011/07/29
                        Exit For
                    End If
            ...
            '2011/06/13  合計件数から1ひく
            lngSYOUKEI_KEN(intMAXCNT) = lngSYOUKEI_KEN(intMAXCNT) - 1

            'If intMAXLINE = 1 Then
            If intMAXLINE = 1 Or intMAXLINE = 0 Then        '2011/07/29
                'intMAXCNT = intMAXCNT - 1
                If intMAXLINE = 1 Then      '2011/07/29
                    intMAXCNT = intMAXCNT - 1
                End If
                intMAXLINE = 20
                '2011/06/13  明細が存在しない場合、前のページに移動
                'intPAGE_CNT += -1
                If intPAGE_CNT <> 1 Then    '2011/07/29

                    '2011/08/15 削除行が最終ページでない場合は、ページ移動は行わない。*****STRAT
                    If SakujoPage = GetPage Then
                        '2011/08/15 削除行が最終ページでない場合は、ページ移動は行わない。*****END
                        intPAGE_CNT += -1
                    End If
                End If
        ...
        Catch ex As Exception
            '2011/06/13 ログ出力
            MessageBox.Show(String.Format(MSG0027E, "行", "削除"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(行削除)", "失敗", P & "ページ" & L & "レコード " & ex.ToString)
            Return False
        End Try
...
#Region " イベント"
    ...
    Handles txtKinNO1.Validating, txtKinNO2.Validating, txtKinNO3.Validating, txtKinNO4.Validating, txtKinNO5.Validating, _
            ...
            txtKinNO11.Validating, txtKinNO12.Validating, txtKinNO13.Validating, txtKinNO14.Validating, txtKinNO15.Validating, _
            txtKinNO16.Validating, txtKinNO17.Validating, txtKinNO18.Validating, txtKinNO19.Validating, txtKinNO20.Validating
        Try
            ...
            Select Case txtKinNo.Name
                ...
                Case txtKinNO16.Name
                    txtKinName = txtKinName16
                    txtSitNo = txtSitNo16
                    txtSitName = txtSitName16
                Case txtKinNO17.Name
                    txtKinName = txtKinName17
                    txtSitNo = txtSitNo17
                    txtSitName = txtSitName17
                Case txtKinNO18.Name
                    txtKinName = txtKinName18
                    txtSitNo = txtSitNo18
                    txtSitName = txtSitName18
                Case txtKinNO19.Name
                    txtKinName = txtKinName19
                    txtSitNo = txtSitNo19
                    txtSitName = txtSitName19
                Case txtKinNO20.Name
                    txtKinName = txtKinName20
                    txtSitNo = txtSitNo20
                    txtSitName = txtSitName20
            End Select
   ...
   Handles txtSitNo1.Validating, txtSitNo2.Validating, txtSitNo3.Validating, txtSitNo4.Validating, txtSitNo5.Validating, _
           ...
           txtSitNo11.Validating, txtSitNo12.Validating, txtSitNo13.Validating, txtSitNo14.Validating, txtSitNo15.Validating, _
           txtSitNo16.Validating, txtSitNo17.Validating, txtSitNo18.Validating, txtSitNo19.Validating, txtSitNo20.Validating
        Try
            ...
            Select Case txtSitNo.Name
                ...
                Case txtSitNo15.Name
                    ...
                    txtKinNo = txtKinNO15
                    txtKinName = txtKinName15
                Case txtSitNo16.Name
                    txtSitName = txtSitName16
                    txtKinNo = txtKinNO16
                    txtKinName = txtKinName16
                Case txtSitNo17.Name
                    txtSitName = txtSitName17
                    txtKinNo = txtKinNO17
                    txtKinName = txtKinName17
                Case txtSitNo18.Name
                    txtSitName = txtSitName18
                    txtKinNo = txtKinNO18
                    txtKinName = txtKinName18
                Case txtSitNo19.Name
                    txtSitName = txtSitName19
                    txtKinNo = txtKinNO19
                    txtKinName = txtKinName19
                Case txtSitNo20.Name
                    txtSitName = txtSitName20
                    txtKinNo = txtKinNO20
                    txtKinName = txtKinName20
            End Select
    ...
    Private Sub Kamoku_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) _
            Handles txtKamokuCode1.Validating, txtKamokuCode2.Validating, txtKamokuCode3.Validating, txtKamokuCode4.Validating, txtKamokuCode5.Validating, _
                    ...
                    txtKamokuCode11.Validating, txtKamokuCode12.Validating, txtKamokuCode13.Validating, txtKamokuCode14.Validating, txtKamokuCode15.Validating, _
                    txtKamokuCode16.Validating, txtKamokuCode17.Validating, txtKamokuCode18.Validating, txtKamokuCode19.Validating, txtKamokuCode20.Validating
        Try
            ...
            Select Case txtKamokuCode.Name
                ...
                Case txtKamokuCode16.Name
                    lblKamoku = lblKamoku16
                Case txtKamokuCode17.Name
                    lblKamoku = lblKamoku17
                Case txtKamokuCode18.Name
                    lblKamoku = lblKamoku18
                Case txtKamokuCode19.Name
                    lblKamoku = lblKamoku19
                Case txtKamokuCode20.Name
                    lblKamoku = lblKamoku20
            End Select
    ...
    Private Sub NzCheckString_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) _
            Handles txtKeiyakuName1.Validating, txtKeiyakuName2.Validating, txtKeiyakuName3.Validating, txtKeiyakuName4.Validating, txtKeiyakuName5.Validating, _
                    ...
                    txtKeiyakuName11.Validating, txtKeiyakuName12.Validating, txtKeiyakuName13.Validating, txtKeiyakuName14.Validating, txtKeiyakuName15.Validating, _
                    txtKeiyakuName16.Validating, txtKeiyakuName17.Validating, txtKeiyakuName18.Validating, txtKeiyakuName19.Validating, txtKeiyakuName20.Validating
        Try
    ...
    Private Sub FMT_NzNumberString_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) _
             Handles txtKouza1.Validating, txtKouza2.Validating, txtKouza3.Validating, txtKouza4.Validating, txtKouza5.Validating, _
                    ...
                    txtKouza11.Validating, txtKouza12.Validating, txtKouza13.Validating, txtKouza14.Validating, txtKouza15.Validating, _
                    txtKouza16.Validating, txtKouza17.Validating, txtKouza18.Validating, txtKouza19.Validating, txtKouza20.Validating
        Try
    ...
    Handles txtFuriKin1.Validating, txtFuriKin2.Validating, txtFuriKin3.Validating, txtFuriKin4.Validating, txtFuriKin5.Validating, _
            ...
            txtFuriKin11.Validating, txtFuriKin12.Validating, txtFuriKin13.Validating, txtFuriKin14.Validating, txtFuriKin15.Validating, _
            txtFuriKin16.Validating, txtFuriKin17.Validating, txtFuriKin18.Validating, txtFuriKin19.Validating, txtFuriKin20.Validating
        Try
            ...
            Select Case txtFurikin.Name
                ...
                Case txtFuriKin15.Name
                    No = 15
                Case txtFuriKin16.Name
                    No = 16
                Case txtFuriKin17.Name
                    No = 17
                Case txtFuriKin18.Name
                    No = 18
                Case txtFuriKin19.Name
                    No = 19
                Case txtFuriKin20.Name
                    No = 20
            End Select
            Dim Temp As String = GCom.NzDec(CType(sender, TextBox).Text, "")
        ...
        Catch ex As Exception
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            MainLOG.Write("振替金額入力チェック", "失敗", ex.ToString)
        End Try
    End Sub
#End Region
