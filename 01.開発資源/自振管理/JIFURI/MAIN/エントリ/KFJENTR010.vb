...
Public Class KFJENTR010
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFJENTR010", "依頼書用振替口座登録画面")
    Private Const msgTitle As String = "依頼書用振替口座登録画面(KFJENTR010)"
...
#Region " 変数"
    ...
    Private strKOKYAKU_KNM As String
...
#End Region
#Region " ロード"
    Private Sub KFJENTR010_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '#####################################
            'ログの書込に必要な情報の取得
...
#Region " 関数"
    Private Function fn_Settei() As Boolean
        Try
            ...
            'エントリ画面の自金庫区分
            Dim TMP As String = CASTCommon.GetRSKJIni("FORM", "KFJENTR010_JIKINKOKBN")
            ...
            '--------------------------------
            Dim Msg As String
            Select Case GCom.SetComboBox(cmbKamoku, "Common_科目.TXT", True)
                Case 1  'ファイルなし
                    Msg = String.Format(MSG0025E, "科目", "Common_科目.TXT")
                    MessageBox.Show(Msg, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Msg = "科目設定ファイルなし。ファイル:Common_科目.TXT"
                ...
                Case 2  '異常
                    ...
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(コンボボックス設定)", "失敗", Msg)
            End Select
            Select Case GCom.SetComboBox(cmbKingakuKbn, "KFJENTR010_金額表示区分.TXT", True)
                Case 1  'ファイルなし
                    Msg = String.Format(MSG0025E, "金額表示区分", "KFJENTR010_金額表示区分.TXT")
                    MessageBox.Show(Msg, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Msg = "金額表示区分設定ファイルなし。ファイル:KFJENTR010_金額表示区分.TXT"
                    MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "(コンボボックス設定)", "失敗", Msg)
                Case 2  '異常
    ...
    Function fn_select_ENTMAST() As Boolean
        ...
        'Parameter      :
        'Description    :ENTMASTに該当データが存在するかどうか
        'Return         :True=OK,False=NG
        'Create         :2009/09/25
        'Update         :
        '============================================================================
    ...
    Function fn_ENTDATA_SET() As Boolean
        ...
        Try
            ...
            ENTRY_DATA(6) = lngHYOUJI_NUM 'HYOUJI_SEQ_E
            ENTRY_DATA(7) = GCom.GetComboBox(cmbKamoku).ToString.PadLeft(2, "0") 'KAMOKU_E
            ...
            ENTRY_DATA(12) = CStr(lngKOKYAKU_NUM).PadLeft(10, "0") 'KOKYAKU_NO_E
            ...
            ENTRY_DATA(20) = "1" 'SINKI_CODE_E 
            '======================================
            ENTRY_DATA(21) = lngRECORD_NUM 'RECORD_NO_E  
            ...
            ENTRY_DATA(28) = txtJyuyokaNo.Text.PadLeft(13, " ") 'JYUYOUKA_NO_E 
    ...
    Function fn_insert_ENTMAST() As Boolean
        ...
        Try
            For No As Integer = 1 To 2
                ...
                If No = 1 Then
                    SQL.Append(" INSERT INTO ENTMAST VALUES (")
                Else
                    SQL.Append(" INSERT INTO FUKU_ENTMAST VALUES (")
                End If
    ...
    Function fn_DATA_SET() As Boolean
        ...
        Try
            ...
            UP_DATA(8) = GCom.GetComboBox(cmbKamoku).ToString.PadLeft(2, "0") 'KAMOKU_E
            ...
            '        UP_DATA(13) = "1"
            '    Else
            '        If UP_DATA(12) > strSAVE_FURI_DATE Then       '更新日　>　振替日  過去に振替処理を行っている場合は変更とする
            '            UP_DATA(13) = "2"
            '        Else
            '            UP_DATA(13) = strSAVE_SINKI_CODE
            '        End If
