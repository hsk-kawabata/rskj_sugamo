...
Public Class KFJMAST011
    ...
    Private Const MaxColumns As Integer = 276   '(全161 + 116項目)
    Private Const MaxColumns_Sub As Integer = 116
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFJMAST011", "取引先マスタメンテナンス画面")
    Private Const msgTitle As String = "取引先マスタメンテナンス画面(KFJMAST011)"
    ...
    Private Const ThisModuleName As String = "KFJMAST011.vb"
...
#Region " ロード"
    Private Sub KFJMAST011_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
                    ...
                    Case (TypeOf CTL Is ComboBox)
                        ...
                        With CMB
                            ...
                            Select Case .Name.ToUpper
                                ...
                                Case "HANSOU_KBN_T"
                                    CmbName = "搬送方法"
                                    TxtFileName = "KFJMAST011_搬送方法.TXT"
                                    Ret = GCom.SetComboBox(CMB, "KFJMAST011_搬送方法.TXT", True)
                                Case "SYOUGOU_KBN_T"
                                    CmbName = "照合要否区分"
                                    TxtFileName = "KFJMAST011_照合要否区分.TXT"
                                    Ret = GCom.SetComboBox(CMB, "KFJMAST011_照合要否区分.TXT", True)
                                Case "MAE_SYORI_T"
                                    CmbName = "個別前処理"
                                    TxtFileName = "KFJMAST011_個別前処理.TXT"
                                    Ret = GCom.SetComboBox(CMB, "KFJMAST011_個別前処理.TXT", True)
                                Case "ATO_SYORI_T"
                                    CmbName = "個別後処理"
                                    TxtFileName = "KFJMAST011_個別後処理.TXT"
                                    Ret = GCom.SetComboBox(CMB, "KFJMAST011_個別後処理.TXT", True)
                            End Select
        ...
        Try
            ...
            If DialogResult.OK = MessageBox.Show(MSG, msgTitle, _
                ...
                If Ret = 1 Then
                    ...
                    '==================================================
                    ' 取引先マスタ(固有情報) <TORIMAST_SUB> の削除
                    '==================================================
                    SQL.Length = 0
                    SQL.Append("DELETE FROM TORIMAST_SUB")
                    SQL.Append(" WHERE FSYORI_KBN_TSUB = " & SQ(sTORI(GetIndex("FSYORI_KBN_T"), 3)))
                    SQL.Append(" AND TORIS_CODE_TSUB = " & SQ(sTORI(GetIndex("TORIS_CODE_T"), 3)))
                    SQL.Append(" AND TORIF_CODE_TSUB = " & SQ(sTORI(GetIndex("TORIF_CODE_T"), 3)))
...
#Region " 帳票印刷"
        ...
        Try
            ...
            If GCom.SetDynaset(SQL.ToString, REC) AndAlso REC.Read Then
                ...
                With CreateCSV
                    ...
                    .OutputCsvData(GetComboBoxValue(2, GetIndex("ATO_SYORI_T"), 1), True)           '個別後処理
    ...
    Public Function fn_Print(ByVal PrintNo As Integer) As Boolean
        ...
        Try
            ...
            Select Case PrintNo
                Case 1
                    nRet = ExeRepo.ExecReport("KFJP059.EXE", Param)
                Case 2
...
#Region " 関数"
    ...
    Private Function InsertModule() As Boolean
        ...
        Try
            ...
            '----------------------------------------------------------------
            SQL.Length = 0
            SQL.Append("INSERT INTO TORIMAST_SUB")
            SQL.Append(" ( ")
    ...
    Private Function InsertModuleSaifuri() As Boolean
        ...
        Try
            ...
            Dim Ret As Integer = MainDB.ExecuteNonQuery(SQL)
            If Ret <> 1 Then
                MessageBox.Show(String.Format(MSG0027E, "取引先マスタ(再振)", "登録"), msgTitle, _
                    MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return False
            End If
            ...
            SQL.Append("   " & SQ(sTORI(GetIndex("FSYORI_KBN_T"), 4)))
            SQL.Append(" , " & SQ(sTORI(GetIndex("TORIS_CODE_T"), 4)))
            SQL.Append(" , " & SQ(sTORI(GetIndex("SFURI_FCODE_T"), 4)))
            For Cnt = MaxColumns - MaxColumns_Sub + 1 To MaxColumns Step 1
                If Not TypeOf oTORI(Cnt) Is Label Then
                    SQL.Append(", " & SetColumnData(Cnt))
                End If
            ...
            If Ret = 1 Then
                Return True
            Else
                MessageBox.Show(String.Format(MSG0027E, "取引先マスタ(再振) 個別情報", "登録"), msgTitle, _
                    MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return False
            End If
    ...
    Private Function UpdateModule() As Boolean
        ...
        Try
            ...
            For Cnt = 1 To MaxColumns - MaxColumns_Sub Step 1
    ...
    Private Function UpDATEModuleSaifuri() As Boolean
        ...
        Try
            ...
            Dim Ret As Integer = MainDB.ExecuteNonQuery(SQL)
            If Ret <> 1 Then '更新失敗
                MessageBox.Show(String.Format(MSG0027E, "取引先マスタ(再振)", "更新"), msgTitle, _
                    MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return False
            End If
            ...
            SQL.Append("     FSYORI_KBN_TSUB = " & SetColumnData(GetIndex("FSYORI_KBN_T")))
            SQL.Append(" AND TORIS_CODE_TSUB = " & SetColumnData(GetIndex("TORIS_CODE_T")))
            SQL.Append(" AND TORIF_CODE_TSUB = " & SetColumnData(GetIndex("SFURI_FCODE_T")))

            Ret = MainDB.ExecuteNonQuery(SQL)
            If Ret <> 1 Then '更新失敗
                MessageBox.Show(String.Format(MSG0027E, "取引先マスタ(個別情報) 再振分", "更新"), msgTitle, _
                    MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return False
            End If
    ...
    Private Function DeleteModuleSaifuri(ByVal FCode As String) As Boolean
        ...
        Try
            ...
            If OraReader.DataReader(SQL) = True Then
                ...
                End If
            Else
                MessageBox.Show(String.Format(MSG0027E, "取引先マスタ(再振)", "検索"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                Return False
            End If
            ...
            Dim Ret As Integer = MainDB.ExecuteNonQuery(SQL)
            If Ret <> 1 Then
                MessageBox.Show(String.Format(MSG0027E, "取引先マスタ 再振分", "削除"), msgTitle, _
                               MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return False
            End If
            ...
            SQL.Append(" WHERE FSYORI_KBN_TSUB = " & SQ(sTORI(GetIndex("FSYORI_KBN_T"), 3)))
            SQL.Append(" AND TORIS_CODE_TSUB   = " & SQ(sTORI(GetIndex("TORIS_CODE_T"), 3)))
            SQL.Append(" AND TORIF_CODE_TSUB   = " & SQ(FCode))

            Ret = MainDB.ExecuteNonQuery(SQL)
            If Ret <> 1 Then
                MessageBox.Show(String.Format(MSG0027E, "取引先マスタ(個別情報) 再振分", "削除"), msgTitle, _
                               MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Return False
            End If
...
#End If
    ...
    Private Function CheckExistTorimast(ByVal TorisCode As String, ByVal TorifCode As String) As Integer
        ...
        Try
            SQL.AppendLine("SELECT * FROM TORIMAST_VIEW")
            SQL.AppendLine(" WHERE FSYORI_KBN_T = '1'")
...
#Region " イベント"
    ...
    Private Sub TabIndexSetFocus_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) _
        Handles YUUBIN_NNAME_T.KeyPress, KEKKA_MEISAI_KBN_T.KeyPress, TESUU_TABLE_ID_T.KeyPress, ENC_KEY2_T.KeyPress, FKEKKA_TBL_T.KeyPress, _
                ATO_SYORI_T.KeyPress, TOKKIJIKOU4_T.KeyPress
