...
Public Class KFJMAST030
    ...
    Private Const ThisModuleName As String = "KFJMAST030.vb"
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFJMAST030", "スケジュールメンテナンス画面")
    Private Const msgTitle As String = "スケジュールメンテナンス画面(KFJMAST030)"
    Private MainDB As MyOracle

    '画面起動時イベント
    Private Sub KFJMAST0300G_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    '画面終了時処理
    Private Sub KFJMAST030_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
    ...
    Private Sub CmdInsert_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdInsert.Click
        ...
        Try
            ...
            If CLS.READ_SCHMAST() = True Then
                ...
                ''有効フラグが "1" の時既存進捗を判定する。(再作成ではフラグは常に "1" となる)
                'If CLS.GetColumnValue("YUUKOU_FLG_S", 0) = 1 Then
                ...
                If IntFLG(0) + IntFLG(1) + IntFLG(2) + IntFLG(3) > 0 Then
                    ...
                    'MSG = "該当スケジュールには既に処理フラグが設定されています。" & Space(5)
                    'MSG &= ControlChars.CrLf & "フラグを 0 に設定してから再登録してください。"
                    'MSG &= ControlChars.CrLf & "受付済フラグ =" & ControlChars.Tab & IntFLG(0).ToString
                    'MSG &= ControlChars.CrLf & "登録済フラグ =" & ControlChars.Tab & IntFLG(1).ToString
                    'MSG &= ControlChars.CrLf & "日報フラグ　 =" & ControlChars.Tab & IntFLG(2).ToString
                    'MSG &= ControlChars.CrLf & "中断フラグ　 =" & ControlChars.Tab & IntFLG(3).ToString
        ...
        Finally
            ...
            Application.DoEvents()
            'If Not CTL.Name = "FURI_DATE_S" Then
            '    FURI_DATE_S.Enabled = False
            '    FURI_DATE_S1.Enabled = False
            '    FURI_DATE_S2.Enabled = False
            '    CTL.Focus()
            '    FURI_DATE_S.Enabled = True
            '    FURI_DATE_S1.Enabled = True
            '    FURI_DATE_S2.Enabled = True
            'Else
            CTL.Focus()
            'End If
            Application.DoEvents()
    ...
    Private Sub CmdUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdUpdate.Click
        ...
        Try
            ...
            'SQL &= ", HAISIN_T2FLG_S = '" & GCom.NzInt(Me.HAISIN_T2FLG_S.Text, 0).ToString & "'"
            'SQL &= ", HAISIN_T1FLG_S = '" & GCom.NzInt(Me.HAISIN_T1FLG_S.Text, 0).ToString & "'"
            ...
            'SQL &= ", HENKYAKU_FLG_S = '" & GCom.NzInt(Me.HENKYAKU_FLG_S.Text, 0).ToString & "'"
            ...
            'SQL &= ", YOBI3_S = '" & GCom.NzInt(Me.YOBI3_S.Text, 0).ToString & "'"
            'SQL &= ", TESUU_KIN_S = " & GCom.NzDec(Me.TESUU_KIN_S.Text, 0).ToString
        ...
        Try
            ...
            Else
                ...
                If CLS.GetColumnValue("UKETUKE_FLG_S", 0) > 0 OrElse _
                   ...
                   CLS.GetColumnValue("NIPPO_FLG_S", 0) > 0 Then
            ...
            'Dim BRet As Boolean = CLS.SET_ACTION_TO_YUUKOU_FLG_S(0)
            If BRet Then
    ...
    Private Function CheckEntryText(ByRef CTL As Control, ByRef MSG As String, ByVal SEL As Short) As Boolean
        ...
        Try
            ...
            If SEL = 1 OrElse SEL = 2 Then
                ...
                '各種フラグ値   見直しのためにコメント
                'Dim ObjFLG() As Control = {UKETUKE_FLG_S, SYOUGOU_FLG_S, TOUROKU_FLG_S, _
                '                HAISIN_T2FLG_S, HAISIN_T1FLG_S, HAISIN_FLG_S, SAIFURI_FLG_S, _
                '                SOUSIN_FLG_S, FUNOU_FLG_S, TESUUKEI_FLG_S, TESUUTYO_FLG_S, _
                '                KESSAI_FLG_S, HENKAN_FLG_S, TYUUDAN_FLG_S, TAKOU_FLG_S, _
                '                NIPPO_FLG_S, YOBI3_S, HENKYAKU_FLG_S}

                Dim ObjFLG() As Control = {UKETUKE_FLG_S, TOUROKU_FLG_S, _
                 HAISIN_FLG_S, SAIFURI_FLG_S, _
                SOUSIN_FLG_S, FUNOU_FLG_S, TESUUKEI_FLG_S, TESUUTYO_FLG_S, _
                KESSAI_FLG_S, HENKAN_FLG_S, TYUUDAN_FLG_S, _
                TAKOU_FLG_S, NIPPO_FLG_S}

                Dim StrFLG() As String = {"受付済フラグ", "登録済フラグ", _
                    "自行配信済フラグ", "再振済フラグ", _
                    "送信済フラグ", "不能済フラグ", "手数料計算済フラグ", "手数料徴求済フラグ", _
                    "決済フラグ", "返還済フラグ", "中断フラグ", "他行フラグ", "日報フラグ"}

                'Dim StrFLG() As String = {"受付済フラグ", "照合済フラグ", "登録済フラグ", _
                '    "提携外配信済フラグ", "提携内配信済フラグ", "自行配信済フラグ", "再振済フラグ", _
                '    "送信済フラグ", "不能済フラグ", "手数料計算済フラグ", "手数料徴求済フラグ", _
                '    "決済フラグ", "返還済フラグ", "中断フラグ", "他行フラグ", "日報フラグ", _
                '    "印刷フラグ", "返却送信フラグ"}
    ...
    Private Sub Form_Initialize(ByVal SEL As Boolean)
        Try
            ...
            ''TabControl1／TabPage1／2内配置のコントロール
            'For Index As Integer = 0 To 1 Step 1
            '    For Each CTL As Control In Me.TabControl1.TabPages(Index).Controls
            '        Dim FLG As Boolean = False
            '        If TypeOf CTL Is TextBox Then
            '            FLG = Form_Initialize_Sub(CTL)
            '        ElseIf TypeOf CTL Is Label Then
            ...
            '            Select Case GCom.NzInt(CTL.Tag)
            '                Case 1
            '                    CTL.Text = " -"
            '                Case 2
            '                    CTL.Text = ""
 ...
 _
    ...
    Private Sub SetActionAfter(ByVal SetVisible As Boolean)
        Try
            ...
            '検索条件領域
            'With Me
            '    .ITAKU_KNAME_T.Enabled = SetVisible
            '    .ITAKU_CODE_S.Enabled = SetVisible
            '    .CmdCIFForm.Enabled = SetVisible
            'End With
            ...
            '振替日変更用
            'With Me
            '    .NFURI_DATE_L.Visible = Not SetVisible
            '    .NFURI_DATE_S.Visible = Not SetVisible
            '    .NFURI_DATE_L0.Visible = Not SetVisible
            '    .NFURI_DATE_S1.Visible = Not SetVisible
            '    .NFURI_DATE_L1.Visible = Not SetVisible
            '    .NFURI_DATE_S2.Visible = Not SetVisible
            '    .NFURI_DATE_L2.Visible = Not SetVisible
            'End With
        Catch ex As Exception
            Throw
        End Try
    ...
    ''他行ボタン(他行スケジュールマスタ参照)
    'Private Sub CmdTakoSch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdTakoSch.Click
    '    Try
    '        Call SetButtonEnabled(0)
    '        Application.DoEvents()

    '        If CLS.CHECK_TAKOSCHMAST(0) Then

    '            Dim onForm As New KFJMAST030
    '            With onForm
    '                .SET_TORIS_CODE = TRKEY.TORIS_CODE
    '                .SET_TORIF_CODE = TRKEY.TORIF_CODE
    '                .SET_FURI_DATE = CLS.SCH.FURI_DATE
    '                .ShowDialog()
    '            End With
    '        Else
    '            Dim MSG As String = "他行スケジュールマスタに同一振替日データがありません。" & Space(5)
    '            Call MessageBox.Show(MSG, GCom.GLog.Job1, _
