...
Public Class KFJMAIN121

#Region "クラス定数"
    Private Const msgTitle As String = "送付状入力画面(KFJMAIN121)"
...
#Region "クラス変数"
    Private MainLOG As New CASTCommon.BatchLOG("KFJMAIN121", "送付状入力画面")
...
#Region "イベントハンドラ"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub KFJMAIN121_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '--------------------------------------------------
            'ログの書込に必要な情報の取得
            '--------------------------------------------------
    ...
    Private Sub btnTouroku_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTouroku.Click
        Try
            ...
            If Not RecInf.pnHENKOU_SW = 1 Then
                If Me.Double_Input_Judge_New() = False Then
            ...
            '0:新規登録  1:変更登録(KFJMAIN122より選択データ受け取り表示したもの）
            ...
            Else
                ...
                Select Case TR.BAITAI_CODE
                    ...
                    '2018/09/24 タスク）西野 CHG 広島信金対応　【現行】（OT-№025）------------------------- START
                    ''FD、MT、CMT、学校、WEB伝送、DVD、USB、MO、FDRを許可する
                    'Case "01", "05", "06", "07", "10", "11", "12", "13", "16"
                    'FD、依頼書(端末自振)、MT、CMT、学校、WEB伝送、DVD、USB、MO、FDRを許可する
                    Case "01", "04", "05", "06", "07", "10", "11", "12", "13", "16", "30"
                        '2018/09/24 タスク）西野 CHG 広島信金対応　【現行】（OT-№025）------------------------- END
    ...
    Private Sub cmbKana_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbKana.SelectedIndexChanged
        Try
            ...
            If Not cmbToriName.SelectedItem.ToString = Nothing OrElse String.IsNullOrEmpty(cmbToriName.SelectedItem.ToString) Then
                '取引先コンボボックス設定
                If GCom.SelectItakuName_View(cmbKana.SelectedItem.ToString, cmbToriName, txtTorisCode, txtTorifCode, "1", JYOUKEN) = -1 Then
...
#Region "プライベートメソッド"
    ...
    ''' 送付状新規登録
    ''' </summary>
    ''' <param name="sSonDateWk">振替日</param>
    ''' <returns>True - 正常 , False - 異常</returns>
    ''' <remarks></remarks>
    Private Function Entry_Insert_Section(ByVal sSonDateWk As String) As Boolean
        ...
        Try
            ...
            '取引先マスタ有無確認
            SQL.Length = 0
            SQL.Append("SELECT * FROM TORIMAST_VIEW")
            ...
            If OraReader.DataReader(SQL) = True Then
                While OraReader.EOF = False
                    ...
                    SQL.Append(", ITAKU_KANRI_CODE_ME")        '代表委託者コード
                    ...
                    SQL.Append(",'" & RecInf.psNewFuriDate & "'")                '振替日
        ...
        Try
            ...
            If OraReader.DataReader(SQL) = True Then
                While OraReader.EOF = False
                    ...
                    SQL.Append(" FURI_DATE_ME = " & "'" & sFuri_Date & "',")                                       '振替日
            ...
            Call GCom.SelectedItem(RecInf.ListView, 9, stemp)                                '振替日
    ...
    Public Sub FormInitializa()
        ...
        Try
            ...
            If GCom.SelectItakuName_View("", cmbToriName, txtTorisCode, txtTorifCode, "1", JYOUKEN) = -1 Then
                MessageBox.Show(MSG0230W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
            ...
            Else
                '変更の場合(KFJMAIN122よりデータ受け渡し）送付状入力画面に表示
    ...
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks>同一振替日、同一委託者　でのチェックとする。</remarks>
    Private Function Double_Input_Judge_New() As Boolean
        Dim OraReader As New CASTCommon.MyOracleReader(Me.MainDB)
        Dim SQL As New StringBuilder

        Try
            SQL.Append("SELECT * FROM MEDIA_ENTRY_TBL")
            '画面検索は取引先コードで行うように処理を変更
            ...
            SQL.Append(" AND FSYORI_KBN_ME = '1'")
    ...
    Private Function CheckTorimast() As Boolean
        ...
        Try
            With SQL
                .Append("select SYOUGOU_KBN_T from TORIMAST_VIEW")
    ...
    Private Function SetTuubanInfo() As Boolean
        Try
            ...
            '通番個数取得
            WK = GetRSKJIni("FORM", "KFJMAIN121_MAXTUUBAN").ToUpper
            ...
            For i As Integer = 1 To TUUBAN_CNT
                WK = GetRSKJIni("FORM", "KFJMAIN121_TUUBAN" & i.ToString).ToUpper
                If WK = "" OrElse WK = "ERR" Then
                    MessageBox.Show(String.Format(MSG0001E, "媒体通番" & i.ToString, "FORM", "KFJMAIN121_TUUBAN" & i.ToString), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    MainLOG.Write("設定ファイル取得", "失敗", "項目名:媒体通番" & i.ToString & " 分類:FORM 項目:KFJMAIN121_TUUBAN" & i.ToString)
                    Return False
                Else
                    Dim strBAITAI() As String = WK.Split(":"c)
                    If strBAITAI.Length <> 2 Then
                        MessageBox.Show(String.Format(MSG0035E, "媒体通番" & i.ToString, "FORM", "KFJMAIN121_TUUBAN" & i.ToString), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                        MainLOG.Write("設定ファイル取得", "失敗", "項目名:媒体通番" & i.ToString & " 分類:FORM 項目:KFJMAIN121_TUUBAN" & i.ToString)
                        Return False
                    Else
