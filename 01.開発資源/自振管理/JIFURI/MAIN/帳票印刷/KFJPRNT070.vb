...
Imports System.IO
Imports CASTCommon
Public Class KFJPRNT070
    Private MainLOG As New CASTCommon.BatchLOG("KFJPRNT070", "未処理一覧表(落込)印刷画面")
    Private Const msgTitle As String = "未処理一覧表(落込)印刷画面(KFJPRNT070)"
    ...
    Private strKaisiDate As String
    Private strSyuryoDate As String

#Region " ロード"
    Private Sub KFJPRNT060_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '#####################################
        'ログの書込に必要な情報の取得
    ...
    Private Sub btnAction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAction.Click
        ...
        Try
            ...
            If fn_check_text() = False Then
                Exit Sub
            End If
            strKaisiDate = txtKaisiDateY.Text + txtKaisiDateM.Text + txtKaisiDateD.Text
            strSyuryoDate = txtSyuryoDateY.Text + txtSyuryoDateM.Text + txtSyuryoDateD.Text
            '--------------------------------
            'マスタチェック
            '--------------------------------
            ...
            '印刷前確認メッセージ
            If MessageBox.Show(String.Format(MSG0013I, "未処理一覧表(落込)"), _
            ...
            param = GCom.GetUserID & "," & strKaisiDate & "," & strSyuryoDate

            nRet = ExeRepo.ExecReport("KFJP037.EXE", param)

            '戻り値に対応したメッセージを表示する
            Select Case nRet
                Case 0
                    '印刷後確認メッセージ
                    MessageBox.Show(String.Format(MSG0014I, "未処理一覧表(落込)"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Case -1
                    '印刷対象なしメッセージ
                    MessageBox.Show(MSG0106W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Case Else
                    '印刷失敗メッセージ
                    MessageBox.Show(String.Format(MSG0004E, "未処理一覧表(落込)"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Select
...
#Region " 関数"
    Function fn_check_text() As Boolean
        ...
        '年必須チェック
        If txtSyuryoDateY.Text.Trim = "" Then
            MessageBox.Show(MSG0018W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtSyuryoDateY.Focus()
            Return False
        End If
        '月必須チェック
        If txtSyuryoDateM.Text.Trim = "" Then
            MessageBox.Show(MSG0020W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtSyuryoDateM.Focus()
            Return False
        End If
        '月範囲チェック
        If GCom.NzInt(txtSyuryoDateM.Text.Trim) < 1 OrElse GCom.NzInt(txtSyuryoDateM.Text.Trim) > 12 Then
            MessageBox.Show(MSG0022W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtSyuryoDateM.Focus()
            Return False
        End If
        ...
        '日付必須チェック
        If txtSyuryoDateD.Text.Trim = "" Then
            MessageBox.Show(MSG0023W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtSyuryoDateD.Focus()
            Return False
        End If
        '日付範囲チェック
        If GCom.NzInt(txtSyuryoDateD.Text.Trim) < 1 OrElse GCom.NzInt(txtSyuryoDateD.Text.Trim) > 31 Then
            MessageBox.Show(MSG0025W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtSyuryoDateD.Focus()
            Return False
        End If
        '日付整合性チェック
        Dim WORK_DATE2 As String = txtSyuryoDateY.Text & "/" & txtSyuryoDateM.Text & "/" & txtSyuryoDateD.Text
        ...
        '日付前後チェック
        If Date.Parse(WORK_DATE) > Date.Parse(WORK_DATE2) Then
    ...
    Private Function fn_check_Table() As Boolean
        ...
        Try
            ...
            SQL.Append(" AND FURI_DATE_S BETWEEN " & SQ(strKaisiDate) & " AND " & SQ(strSyuryoDate))
...
#Region " イベント"
    ...
    Private Sub FMT_NzNumberString_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) _
             Handles txtKaisiDateY.Validating, txtKaisiDateM.Validating, txtKaisiDateD.Validating, _
                     txtSyuryoDateY.Validating, txtSyuryoDateM.Validating, txtSyuryoDateD.Validating
