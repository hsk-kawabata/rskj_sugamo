...
#Region " 定数/変数 "
    ...
    ' LOG関連項目
    '--------------------------------
    Private MainLOG As New CASTCommon.BatchLOG("KFCMAIN011", "媒体読込（媒体→ディスク）画面")
    Private Const msgTitle As String = "媒体読込（媒体→ディスク）画面(KFCMAIN011)"
...
#Region " ロード "
    ...
    Private Sub KFCMAIN011_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            ' システム日付とユーザ名を表示
            '--------------------------------
            '2018/02/27 タスク）西野 CHG 標準版修正：広島信金対応（不具合修正）------------------------ START
            Call GCom.SetMonitorTopArea(Label1, Label2, lblUser, lblDate, 200)
            'Call GCom.SetMonitorTopArea(Label1, Label2, lblUser, lblDate)
            '2018/02/27 タスク）西野 CHG 標準版修正：広島信金対応（不具合修正）------------------------ END
            ...
            ' ボタンの初期化
            '--------------------------------
            '2018/03/01 タスク）西野 CHG 標準版修正：広島信金対応（不具合修正）------------------------ START
            Me.btnFileSelect.Enabled = False
            Me.btnRead.Enabled = False
            'Me.btnRead.Enabled = True
            '2018/03/01 タスク）西野 CHG 標準版修正：広島信金対応（不具合修正）------------------------ END
...
#Region " ボタン "
    ...
    Private Sub btnRead_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRead.Click
        ...
        Try
            ...
            '2018/02/27 タスク）西野 ADD 標準版修正：広島信金対応（不具合修正）------------------------ START
            '-------------------------------------
            ' テキストボックスの入力チェック
            '-------------------------------------
            If fn_check_text() = False Then
                Return
            End If
            '2018/02/27 タスク）西野 ADD 標準版修正：広島信金対応（不具合修正）------------------------ END
            ...
            Select Case mArgumentData.INFOToriMast.BAITAI_CODE_T
                Case "01"
                    CopyReturn = clsFUSION.fn_FD_CPYTO_DISK(mArgumentData.INFOToriMast.TORIS_CODE_T & mArgumentData.INFOToriMast.TORIF_CODE_T,
                                                            ReadFileName,
                                                            WorkFileName,
                                                            ReadFMT.RecordLen,
                                                            mArgumentData.INFOToriMast.CODE_KBN_T,
                                                            ReadFMT.FTRANP,
                                                            msgTitle)
                Case Else
                    CopyReturn = clsFUSION.fn_DVD_CPYTO_DISK(mArgumentData.INFOToriMast.TORIS_CODE_T & mArgumentData.INFOToriMast.TORIF_CODE_T,
                                                             ReadFileName,
                                                             WorkFileName,
                                                             ReadFMT.RecordLen,
                                                             mArgumentData.INFOToriMast.CODE_KBN_T,
            ...
            Select Case CopyReturn
                ...
                Case 100
                    '2018/03/05 タスク）西野 DEL 標準版修正：広島信金対応（不具合修正）------------------------ START
                    'clsFUSIONでメッセージ出力しているので、ここではログだけ出力する。
                    'MessageBox.Show(String.Format(MSG0371W, "媒体読込処理", "取引先のファイル名を確認してください。"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    '2018/03/05 タスク）西野 DEL 標準版修正：広島信金対応（不具合修正）------------------------ END
            ...
            '2018/03/02 タスク）西野 ADD 標準版修正：広島信金対応（不具合修正）------------------------ START
            Select Case ReadFMT.ToriData.INFOToriMast.CODE_KBN_T
                Case "2", "3"
                    InfoParam.CODE_KBN = "0"
                    mArgumentData.INFOParameter = InfoParam
                    ReadFMT = CAstFormat.CFormat.GetFormat(mArgumentData.INFOParameter)
                    ReadFMT.ToriData = mArgumentData
            End Select
            '2018/03/02 タスク）西野 ADD 標準版修正：広島信金対応（不具合修正）------------------------ END
                ...
                If DelFileName <> Path.GetFileName(WorkFileName) Then
                    ...
                    If DelFileName.Length >= 32 Then
                        ...
                        If DelFileName.Substring(0, 32) = FileName.Substring(0, 32) Then
                            ...
                            If MessageBox.Show("同一" & FSyoriName & "のデータが存在します。" & vbCrLf &
                                            "既に読み込まれていファイルを削除し、処理を続行しますか？" & vbCrLf & vbCrLf &
                                            "　" & DispName & vbCrLf &
                                ...
                                MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "読込開始", "キャンセル",
                                              "同一" & FSyoriName & "データあり。" &
                                              "　" & DispName &
                                              "　" & FSyoriName & "：" & FileName.Substring(24, 8))
                                Exit Try
            ...
            If InfoParam.CODE_KBN = "0" Then
                'JISの場合はそのままコピー
                '2018/02/27 タスク）西野 ADD 標準版修正：広島信金対応（不具合修正）------------------------ START
                'チェックでＯＫならば、媒体からコピーする（中間ファイルは120バイトになっているため）
                File.Copy(Path.Combine(BaitaiDrive, lblFileName.Text.Trim), Path.Combine(Me.IniInfo.COMMON_BAITAIREAD, FileName))
            ...
            Else
                ...
                If Me.ConvertFileFtranP(GetOrPut, WorkFileName, Path.Combine(Me.IniInfo.COMMON_BAITAIREAD, FileName), Path.Combine(Me.IniInfo.COMMON_FTR, MapFile)) <> 0 Then
                    MessageBox.Show(String.Format(MSG0371W, "媒体読込処理", "FTRANPのコード変換処理に失敗しました。"),
        ...
        Catch ex As IOException
            MessageBox.Show(ex.Message, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
...
#Region " 関数(Function) "
            ...
            If OraReader.DataReader(SQL) = True Then
                ...
                Select Case lblBaitaiCode.Text
                    ...
                    Case Else
                        ...
                        MainLOG.Write(LW.UserID, LW.ToriCode, LW.FuriDate, "委託者情報取得", "成功", "媒体対象外")
    ...
    Private Function fn_check_text() As Boolean
        ...
        Catch ex As Exception
            MessageBox.Show(MSG0006E, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
            MainLOG.Write(LW.UserID, "0000000000-00", "00000000", "(入力チェック処理)", "失敗", ex.Message)
            Return False
...
#Region " 関数(Sub) "
    ...
    Private Sub rdb_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rdbKigyo.CheckedChanged, rdbSofuri.CheckedChanged
        ...
        Try
            ...
            ' ツリービュー初期化
            '--------------------------------
            Me.ItakuTreeView.ITV_FSYORI_KBN = Gyoumu
            Me.ItakuTreeView.TextBoxItakuKName.Text = ""
            Me.ItakuTreeView.TextBoxItakuCode.Text = ""
            Me.ItakuTreeView.CmdCIFForm.PerformClick()
