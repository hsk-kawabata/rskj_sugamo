...
Class ClsPrnInputError
    ...
    Sub New(ByVal EDITION As String)
        ...
        If EDITION = "2" Then
            ...
            ' 定義体名セット
            ReportBaseName = "KFSP035_インプットエラーリスト.rpd"
        Else
            ' CSVファイルセット
            InfoReport.ReportName = "KFSP003"

            ' 定義体名セット
            ReportBaseName = "KFSP003_インプットエラーリスト.rpd"
        End If
    End Sub
    'Sub New()
    '    ' CSVファイルセット
    ...
    Public Sub OutputMemData(ByVal aReadFmt As CAstFormat.CFormat, ByVal aToriComm As CAstBatch.CommData)
        ...
        Try
            For i As Integer = 0 To aReadFmt.InErrorArray.Count - 1
                ...
                Select Case InfoTori.SYUBETU_T                                  '種別
                    Case "21"
                        PutMem("総合振込")
                    Case "11"
                        PutMem("給与振込")
                ...
                If STR_EDITION = "2" Then
                    ...
                    PutMem(CASTCommon.ConvertKamoku2TO1_K(InfoMei.ITAKU_KAMOKU))    ' 依頼人預金種目
                ...
                '---------------------------------------
                PutMem(CStr(i + 1))                                   '通番
                PutMem(inf.KIN)                                       '振込金融機関コード
                '2018/10/07 saitou 広島信金(RSV2標準) UPD （ヘッダ口座情報チェック対応） ------------------ START
                'ヘッダ情報を出力するときは振込金融機関名を参照しない。
                If InfoMei2.KEIYAKU_KIN_KNAME Is Nothing Then
                    PutMem("")
                Else
                    PutMem(InfoMei2.KEIYAKU_KIN_KNAME.Trim)           '振込金融機関名
                End If
                'PutMem(InfoMei2.KEIYAKU_KIN_KNAME.Trim)         '振込金融機関名
                '2018/10/07 saitou 広島信金(RSV2標準) UPD ------------------------------------------------- END
                PutMem(inf.SIT)                                       '振込支店コード
                '2018/10/07 saitou 広島信金(RSV2標準) UPD （ヘッダ口座情報チェック対応） ------------------ START
                'ヘッダ情報を出力するときは振込支店名を参照しない。
                If InfoMei2.KEIYAKU_SIT_KNAME Is Nothing Then
                    PutMem("")
                Else
                    PutMem(InfoMei2.KEIYAKU_SIT_KNAME.Trim)           '振込支店名
                End If
                'PutMem(InfoMei2.KEIYAKU_SIT_KNAME.Trim)         '振込支店名
                '2018/10/07 saitou 広島信金(RSV2標準) UPD ------------------------------------------------- END
    ...
    '            'ヘッダ情報
    '            '---------------------------------------
    '            CSVObject.Output(InfoPara.FURI_DATE)                            '振込日
    ...
    '            Select Case InfoTori.SYUBETU_T                                  '種別
    '                Case "21"
    '                    CSVObject.Output("総合振込")
    '                Case "11"
    '                    CSVObject.Output("給与振込")
    '                Case "12"
    '                    CSVObject.Output("賞与振込")
    '                Case "21"
    '                    CSVObject.Output("口座振替")
    '                Case Else
    '                    CSVObject.Output("総合振込")
    '            End Select
    ...
    '            CSVObject.Output(inf.KIN)                                       '振込金融機関コード
    '            CSVObject.Output(InfoMei2.KEIYAKU_KIN_KNAME.Trim, True)         '振込金融機関名
    '            CSVObject.Output(inf.SIT)                                       '振込支店コード
    '            CSVObject.Output(InfoMei2.KEIYAKU_SIT_KNAME.Trim, True)         '振込支店名
    ...
    '            CSVObject.Output(inf.FURIKIN)                                   '振込金額
