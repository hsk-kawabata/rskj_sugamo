...
Public Class KFGMAST010
#Region " 変数"
    '学校マスタ(２)最大項目数(開始=Zero)

    '*** Astar SAKAI 2008/06/06 
    Private Const MaxColumns As Integer = 102
    '*** END
    ...
    Private Const ThisModuleName As String = "KFGMAST010.vb"
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFGMAST010", "学校マスタメンテナンス画面")
    Private Const msgTitle As String = "学校マスタメンテナンス画面(KFGMAST010)"
...
#End Region
#Region " ロード"
    Private Sub KFGMAST010_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            '#####################################
            'ログの書込に必要な情報の取得
        ...
        Try
            ...
            If SQLCode = 0 Then
                If Ret = 1 Then
                    Return True
                Else
                    Throw New Exception("学校マスタ2に登録できませんでした。(登録件数=" & Ret & ", SQLCode=" & SQLCode & ")")
                End If
            Else
                Throw New Exception("学校マスタ2に登録できませんでした。(登録件数=" & Ret & ", SQLCode=" & SQLCode & ")")
            End If
        ...
        Try
            ...
            For Cnt = 1 To 4 Step 1
                ...
                If SQLCode = 0 Then
                    If Ret = 1 Then
                    Else
                        Throw New Exception("取引先マスタに登録できませんでした。(登録件数=" & Ret & ", SQLCode=" & SQLCode & ")")
                    End If
                Else
                    Throw New Exception("取引先マスタに登録できませんでした。(登録件数=" & Ret & ", SQLCode=" & SQLCode & ")")
                End If
...
#Region " 参照"
    ...
    Public Function CmdSelect_Action() As Boolean
        ...
        Try
            ...
            SQL.Append(" WHERE GAKKOU_CODE_T = " & SQ(Mid(LW.ToriCode, 1, 10)))
...
#Region " イベント"
    ...
    'タブ間のフォーカス移動
    Private Sub TabIndexSetFocus_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles ITAKU_NJYU_T.KeyPress, MEISAI_OUT_T.KeyPress, TESUU_TABLE_ID_T.KeyPress
    ...
    Private Sub FMT_NzNumberString_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) _
        Handles GAKKOU_CODE_T.Validating, ITAKU_CODE_T.Validating, _
                ...
                SYURYOU_DATE_T1.Validating, SYURYOU_DATE_T2.Validating, KEIYAKU_NO_T.Validating, TESUUTYO_KOUZA_T.Validating, HONBU_KOUZA_T.Validating, TUKEKOUZA_T.Validating
        Try
    ...
    Private Sub GetLimitString_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) _
              Handles GAKKOU_NNAME_G.Validating, ITAKU_NJYU_T.Validating, NTEKIYOU_T.Validating, FILE_NAME_T.Validating, YOBI1_T.Validating
        Dim Index As Integer
        With CType(sender, TextBox)
            Select Case .Name
    ...
    End Sub
    'Private Sub GetLimitString_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) _
    '        Handles GAKKOU_NNAME_G.Validating, ITAKU_NJYU_T.Validating, NTEKIYOU_T.Validating, FILE_NAME_T.Validating
    '    Dim Index As Integer
    '    With CType(sender, TextBox)
    '        Select Case .Name
    '            Case "GAKKOU_NNAME_G" : Index = GetIndex("GAKKOU_NNAME_G")
                ...
                If TypeOf oTORI(Index) Is TextBox Then
                    ...
                    Else
                        Select Case sTORI(Index, 1).ToUpper
                            ...
                            Case "YOBI1_T"
                                'WEB伝送ユーザ名つ媒体がWEB伝送(3)の場合は必須チェックを行う
                                If GCom.NzInt(sTORI(GetIndex("BAITAI_CODE_T"), 4)) = 3 Then
                                    MsgIcon = MessageBoxIcon.Warning
                                    Throw New Exception(String.Format(MSG0285W, sTORI(Index, 0)))
                                End If
...
#Region " 帳票印刷"
    Public Function fn_CreateCSV_INSERT() As Boolean
        ...
        'Parameter      :
        'Description    :KFGP006(学校マスタメンテ(登録))印刷用ＣＳＶファイル作成
        'Return         :
        'Create         :2010/03/16
        'Update         :
        '============================================================================
        ...
        Try
            ...
            SQL.Append(" AND GAKKOU_CODE_G = " & SQ(GAKKOU_CODE_T.Text))
            SQL.Append(" ORDER BY GAKUNEN_CODE_G ")
            ...
            If GCom.SetDynaset(SQL.ToString, REC) AndAlso REC.Read Then
                ...
                Do
                    With CreateCSV
                        ...
                        '2014/01/14 saitou 標準版 印紙税対応 UPD -------------------------------------------------->>>>
                        '印紙税の区分を追加
                        '.OutputCsvData(GCom.NzStr(REC.Item("TESUU_C3_T")), True, True)                  '他行3万円以上
                        ...
                        '2017/04/25 タスク）西野 CHG 標準版修正（学校WEB伝送対応）------------------------------------ START
                        .OutputCsvData(Me.InshizeiLabel.Inshizei3, True)                                '印紙税3
                        '.OutputCsvData(Me.InshizeiLabel.Inshizei3, True, True)                          '印紙税3
                        '2017/04/25 タスク）西野 CHG 標準版修正（学校WEB伝送対応）------------------------------------ END
                        '2014/01/14 saitou 標準版 印紙税対応 UPD --------------------------------------------------<<<<
                        '2017/04/25 タスク）西野 ADD 標準版修正（学校WEB伝送対応）------------------------------------ START
                        .OutputCsvData(GCom.NzStr(REC.Item("YOBI1_T")), True, True)                     '予備１(WEB伝送ユーザ名)
                        '2017/04/25 タスク）西野 ADD 標準版修正（学校WEB伝送対応）------------------------------------ END
                    End With
    ...
    Public Function fn_Print(ByVal PrintNo As Integer) As Boolean
        ...
        Try
            ...
            Select Case PrintNo
                Case 1
                    nRet = ExeRepo.ExecReport("KFGP006.EXE", Param)
                Case 2
