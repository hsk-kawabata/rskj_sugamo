...
#End Region
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFGMAST110", "生徒マスタ処理画面")
    Private Const msgTitle As String = "生徒マスタ処理画面(KFGMAST110)"
...
#Region " Form Load "
    Private Sub KFGMAST110_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region " Button Click "
    Private Sub btnImp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImp.Click
        ...
        Try
            ...
            '確認メッセージ
            If MessageBox.Show(G_MSG0013I, msgTitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) <> Windows.Forms.DialogResult.OK Then
                Return
            End If
            ...
            KobetuLogFileName = Path.Combine(STR_LOG_PATH, "SEITO" & txtGAKKOU_CODE.Text & ".log")
            ...
            dlg.FileName = "SEITO" & Trim(txtGAKKOU_CODE.Text)
            ...
            'CSV入力＆マスタ登録
            If PFUNC_IMP_SEITO(dlg.FileName, Lng_Csv_Ok_Cnt, Lng_Csv_Ng_Cnt) = False Then
                MainDB.Rollback()
                ChDir(strDIR)
                btnEnd.Focus()
                Return
    ...
    Private Sub btnExp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnExp.Click
        ...
        Try
            ...
            'CSV出力
            Lng_Csv_Ok_Cnt = PFUNC_EXP_SEITO(dlg.FileName)
...
#Region " Private Function "
    Private Function PFUNC_IMP_SEITO(ByVal pCsvPath As String, ByRef lngOkCnt As Long, ByRef lngErrCnt As Long) As Boolean
        ...
        Try
            ...
            If MainDB.ExecuteNonQuery(sql) < 0 Then
                MainLOG.Write("生徒マスタ削除", "失敗", sql.ToString)
                Return False
            End If
        ...
        For intCnt = 0 To UBound(strSplitValue)
            Select Case intCnt
                Case 0
                    ...
                    sql.Append(" WHERE")
                    sql.Append(" GAKKOU_CODE_G = '" & Trim(strSplitValue(intCnt)).PadLeft(10, "0"c) & "'")
                ...
                Case 3
                    ...
                    Else
                        ...
                        Select Case CLng(strSplitValue(intCnt))
                            Case 1 To 9
                            Case Else
                                pError = pSeqNo & ",学年コード,学年は１～９以外の数値を設定することはできません"
                ...
                Case 20
                    ...
                    Else
                        ...
                        sql.Append(" WHERE")
                        sql.Append(" GAKKOU_CODE_H  ='" & txtGAKKOU_CODE.Text & "'")
                        sql.Append(" AND")
                        sql.Append(" GAKUNEN_CODE_H =" & Trim(strSplitValue(3)))
                        sql.Append(" AND")
                ...
                Case 21  '長子情報追加 2005/03/09
                    ...
                    If strSplitValue(intCnt) <> "" Then
                        '規定外文字チェック 2007/02/15
                        If ConvKanaNGToKanaOK(strSplitValue(intCnt), strRET) = False Then
                            pError = pSeqNo & ",長子フラグ,規定外文字が有ります"
                            Exit Function
                        End If
                        '数値チェック  2007/02/15
                        If IsNumeric(strSplitValue(intCnt)) = False Then

                            pError = pSeqNo & ",長子,長子に数値以外の値が設定されています"
                ...
                Case 22
                    ...
                    If Trim(strSplitValue(21)) = "1" Then
                        ...
                        '入力桁数チェック 2007/02/12
                        If strSplitValue(intCnt).Trim.Length > 4 Then
                            pError = pSeqNo & ",長子入学年度,長子入学年度が４桁を超えて設定されています"
                            Exit Function
                        End If
                ...
                Case 23
                    ...
                    If Trim(strSplitValue(21)) = "1" Then
                        ...
                        '入力チェック追加 2007/02/12
                        If IsNumeric(strSplitValue(intCnt)) = False Then
                            pError = pSeqNo & ",長子通番,数値以外を設定することはできません"
                            Exit Function
                        Else
                            If strSplitValue(intCnt).Trim.Length > 4 Then
                                pError = pSeqNo & ",長子通番,長子通番が４桁を超えて設定されています"
                                Exit Function
                            End If
                            '入力値範囲チェック
                            Select Case CLng(strSplitValue(intCnt))
                                Case 1 To 9999
                                Case Else
                                    pError = pSeqNo & ",長子通番,１～９９９９以外の数値を設定することはできません"
                                    Exit Function
                            End Select
                        End If
                ...
                Case 24
                    ...
                    If Trim(strSplitValue(21)) = "1" Then
                        ...
                        '数値チェック
                        If IsNumeric(strSplitValue(intCnt)) = False Then
                            pError = pSeqNo & ",長子学年,長子学年に数値以外を設定することはできません"
                            Exit Function
                        Else
                            If strSplitValue(intCnt).Trim.Length >= 2 Then
                                pError = pSeqNo & ",長子学年,長子学年が２桁を超えて設定されています"
                                Exit Function
                            End If
                            '入力値範囲チェック
                            Select Case CLng(strSplitValue(intCnt))
                                Case 1 To 9
                                Case Else
                                    pError = pSeqNo & ",長子学年,長子学年は１～９以外の数値を設定することはできません"
                                    Exit Function
                            End Select
                        End If
                        ...
                        If oraReader.DataReader(sql) = False Then
                            'MSG変更 2007/02/12
                            pError = pSeqNo & ",長子学年,長子学年が学校マスタに登録されていません"
                            oraReader.Close()
                            Return False
                        End If
                        oraReader.Close()
                ...
                Case 25
                    ...
                    If Trim(strSplitValue(21)) = "1" Then
                        ...
                        Else
                            If strSplitValue(intCnt).Trim.Length >= 3 Then
                                pError = pSeqNo & ",長子クラス,長子クラスが３桁を超えて設定されています"
                                Exit Function
                            End If

                            If IsNumeric(strSplitValue(intCnt)) = False Then

                                pError = pSeqNo & ",長子クラス,数値以外を設定することはできません"
                            ...
                            Else
                                ...
                                Select Case CLng(strSplitValue(intCnt))
                                    Case 1 To 20
                                    Case Else
                                        pError = pSeqNo & ",長子クラス,１～２０以外の数値を設定することはできません"
                            ...
                            sql.Append(" GAKKOU_CODE_G = '" & Trim(strSplitValue(0)).PadLeft(10, "0"c) & "'")
                            sql.Append(" AND")
                            sql.Append(" GAKUNEN_CODE_G = " & Trim(strSplitValue(24))) '長子の学年をセット
                            sql.Append(" AND (")
                            For intCnt2 = 1 To 20
                                sql.Append(IIf(intCnt2 = 1, "", "or") & " CLASS_CODE1" & Format(intCnt2, "00") & "_G = '" & Trim(strSplitValue(intCnt)).PadLeft(2, "0"c) & "'")
                            Next intCnt2
                            ...
                            If oraReader.DataReader(sql) = False Then
                                pError = pSeqNo & ",長子クラス,学校マスタに存在しないクラスを設定することは出来ません"
                                oraReader.Close()
                                Return False
                            End If
                            oraReader.Close()
                    ...
                    If Trim(strSplitValue(21)) = "1" Then
                        ...
                        Else
                            ...
                            If IsNumeric(strSplitValue(intCnt)) = False Then
                                ...
                                pError = pSeqNo & ",長子生徒番号,数値以外を設定することはできません"
                            ...
                            If strSplitValue(intCnt).Trim.Length > 7 Then
                                ...
                                pError = pSeqNo & ",長子生徒番号,長子生徒番号が７桁を超えて設定されています"
    ...
    Private Function PFUNC_INSERT_STR(ByVal pSeqNo As Long, ByVal pLineValue As String, ByRef pError As String) As Boolean
        ...
        For intTuki = 1 To 12
            ...
            For intCnt = 0 To 26 '2005/06/15修正
                Select Case intCnt
                    ...
                    Case 20
                        '費目ID
                        sql.Append(",'" & Trim(strSplitValue(intCnt)).PadLeft(3, "0"c) & "'")
                    Case 21 '長子フラグ 長子情報追加 2005/03/09
                        '値なしの場合は０固定
                        Select Case Trim(strSplitValue(intCnt))
                            Case "0", "1"
                                sql.Append(",'" & Trim(strSplitValue(intCnt)) & "'")
                    ...
                    Case 22 '長子入学年度
                        '長子フラグ=0の場合は''固定
                        Select Case Trim(strSplitValue(21))
                            Case "1"
                                sql.Append(",'" & Trim(strSplitValue(intCnt)) & "'")
                            Case Else
                                sql.Append(",''")
            ...
            strSql += " WHERE"
            strSql += " GAKKOU_CODE_O ='" & Trim(strSplitValue(0)).PadLeft(10, "0"c) & "'"
            strSql += " AND"
            strSql += " NENDO_O ='" & Trim(strSplitValue(1)) & "'"
            strSql += " AND"
            strSql += " TUUBAN_O =" & Trim(strSplitValue(2))
            strSql += " AND"
    ...
    End Function
    Private Function PFUNC_EXP_SEITO(ByVal pCsvPath As String) As Long
        ...
        Try
            ...
            sql.Append(",TYOUSI_FLG_O, TYOUSI_NENDO_O, TYOUSI_TUUBAN_O, TYOUSI_GAKUNEN_O, TYOUSI_CLASS_O, TYOUSI_SEITONO_O") '長子情報追加 2005/03/09
            ...
            If oraReader.DataReader(sql) = True Then
                ...
                With sbLine
                    ...
                    .Append(",解約区分,進級区分,費目ID")
                    .AppendLine(",長子有無フラグ,長子入学年度,長子通番,長子学年,長子クラス,長子生徒番号")
                End With
                ...
                'strLine += ",長子有無フラグ,長子入学年度,長子通番,長子学年,長子クラス,長子生徒番号" '長子情報追加 2005/03/09
                'strLine += vbCrLf
                '2012/10/06 saitou 大垣信金 カスタマイズ UPD --------------------------------------------------<<<<
                ...
                Do Until oraReader.EOF
                    ...
                    '費目ID
                    sbLine.Append("," & oraReader.GetString("HIMOKU_ID_O"))
                    '長子フラグ
                    sbLine.Append("," & oraReader.GetString("TYOUSI_FLG_O"))
                    '長子入学年度
                    sbLine.Append("," & oraReader.GetString("TYOUSI_NENDO_O"))
                    '長子通番
                    sbLine.Append("," & oraReader.GetString("TYOUSI_TUUBAN_O"))
                    '長子学年
                    sbLine.Append("," & oraReader.GetString("TYOUSI_GAKUNEN_O"))
                    '長子クラス
                    sbLine.Append("," & oraReader.GetString("TYOUSI_CLASS_O"))
                    '長子生徒番号
                    If oraReader.GetString("TYOUSI_SEITONO_O") <> String.Empty Then
                        sbLine.AppendLine("," & oraReader.GetString("TYOUSI_SEITONO_O"))
                    Else
                        sbLine.AppendLine(",0")
                    End If
                    ...
                    ''長子情報追加 2005/03/09
                    ''長子フラグ
                    'If IsDBNull(oraReader.GetString("TYOUSI_FLG_O")) = False Then
                    '    strLine += "," & Trim(CStr(oraReader.GetString("TYOUSI_FLG_O")))
                    'Else
                    '    strLine += ","
                    'End If

                    ''長子入学年度
                    'If IsDBNull(oraReader.GetString("TYOUSI_NENDO_O")) = False Then
                    '    strLine += "," & Trim(CStr(oraReader.GetString("TYOUSI_NENDO_O")))
                    'Else
                    '    strLine += ","
                    'End If
                    ''長子通番
                    'If IsDBNull(oraReader.GetString("TYOUSI_TUUBAN_O")) = False Then
                    '    strLine += "," & Trim(CStr(oraReader.GetString("TYOUSI_TUUBAN_O")))
                    'Else
                    '    strLine += ","
                    'End If
                    ''長子学年
                    'If IsDBNull(oraReader.GetString("TYOUSI_GAKUNEN_O")) = False Then
                    '    strLine += "," & Trim(CStr(oraReader.GetString("TYOUSI_GAKUNEN_O")))
                    'Else
                    '    strLine += ","
                    'End If
                    ''長子クラス
                    'If IsDBNull(oraReader.GetString("TYOUSI_CLASS_O")) = False Then
                    '    strLine += "," & Trim(CStr(oraReader.GetString("TYOUSI_CLASS_O")))
                    'Else
                    '    strLine += ","
                    'End If
                    ''長子生徒番号
                    'If IsDBNull(oraReader.GetString("TYOUSI_SEITONO_O")) = False Then
                    '    If Trim(CStr(oraReader.GetString("TYOUSI_SEITONO_O"))) = "" Then '2005/06/15追加
                    '        strLine += ",0"
                    '    Else
                    '        strLine += "," & Trim(CStr(oraReader.GetString("TYOUSI_SEITONO_O")))
                    '    End If
                    'Else
                    '    strLine += ",0"
                    'End If
