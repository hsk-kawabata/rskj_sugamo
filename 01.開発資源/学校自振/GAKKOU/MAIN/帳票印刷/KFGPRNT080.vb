...
#Region " 共通変数定義 "
    ...
    Private MainLOG As New CASTCommon.BatchLOG("KFGPRNT080", "生徒マスタ登録チェックリスト印刷画面")
    Private Const msgTitle As String = "生徒マスタ登録チェックリスト印刷画面(KFGPRNT080)"
...
#Region " Form Load "
    ...
    'Form Load
    '****************************
    Private Sub KFGPRNT080_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region " Button Click "
    ...
    Private Sub btnPrnt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrnt.Click
        ...
        Try
            ...
            '生徒マスタ存在チェック 2006/10/10
            If fn_select_SEITOMAST() = False Then
                txtGAKKOU_CODE.Focus()
                Exit Sub
            End If


            SQL.Append("SELECT DISTINCT GAKKOU_CODE_O FROM SEITOMASTVIEW ")
            ...
            If OraReader.DataReader(SQL) = False Then
                '2018/06/29 saitou 広島信金(RSV2) MODIFY ------------------------------------- START
                MessageBox.Show(MSG0106W, msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                'MessageBox.Show("対象データが存在しません", msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning)
                '2018/06/29 saitou 広島信金(RSV2) MODIFY ------------------------------------- END
                txtGAKKOU_CODE.Focus()
                Exit Sub
            End If

            '印刷前確認メッセージ
            If MessageBox.Show(String.Format(MSG0013I, "生徒マスタ登録チェックリスト"), _
            ...
            While OraReader.EOF = False
                ...
                Param = GCom.GetUserID & "," & STR学校コード & "," & STR_FURIKAE_DATE(1) & "," & txtSEIKYUTUKI.Text & "," & STR帳票ソート順 & ",1"
                ...
                Select Case nRet
                    ...
                    Case Else
                        '印刷失敗メッセージ
                        MessageBox.Show(String.Format(MSG0004E, "生徒マスタ登録チェックリスト"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                        Return
                End Select
                OraReader.NextRead()
            End While

            MessageBox.Show(String.Format(MSG0014I, "生徒マスタ登録チェックリスト"), msgTitle, MessageBoxButtons.OK, MessageBoxIcon.Information)
...
#Region " Private Function "
    ...
    End Function
    Function fn_select_SEITOMAST() As Boolean
        Dim intRECORD_COUNT As Integer

        fn_select_SEITOMAST = False

        Dim SQL As New StringBuilder
        Dim OraReader As MyOracleReader = Nothing
        Try
            OraReader = New MyOracleReader(MainDB)
            SQL.Append("SELECT COUNT(*) COUNTER FROM SEITOMASTVIEW ")
            SQL.Append(" WHERE TUKI_NO_O = " & SQ(txtSEIKYUTUKI.Text))
            SQL.Append(" AND (KOUSIN_DATE_O = " & SQ(STR_FURIKAE_DATE(1)) & " OR SAKUSEI_DATE_O = " & SQ(STR_FURIKAE_DATE(1)) & ")")
            If txtGAKKOU_CODE.Text.Trim <> "9999999999" Then
                SQL.Append(" AND GAKKOU_CODE_O = " & SQ(txtGAKKOU_CODE.Text))
        ...
        Finally
            If Not OraReader Is Nothing Then OraReader.Close()
        End Try
        fn_select_SEITOMAST = True
    End Function
    '2018/06/29 saitou 広島信金(RSV2) DEL ---------------------------------------- START
    'Private Function PFUC_SQLQuery_生徒マスタ() As String
    ...
    '    SSQL = SSQL & " SEITOMAST.FURIKAE_O, "
    '    SSQL = SSQL & " SEITOMAST.KAMOKU_O, "
    '    SSQL = SSQL & " SEITOMAST.KAIYAKU_FLG_O, "
    ...
    '    SSQL = SSQL & " SEITOMAST.MEIGI_KNAME_O, "
    '    SSQL = SSQL & " SEITOMAST.MEIGI_NNAME_O, "
    '    SSQL = SSQL & " SEITOMAST.TKIN_NO_O, "
    '    SSQL = SSQL & " SEITOMAST.TSIT_NO_O, "
    '    SSQL = SSQL & " TENMAST.KIN_NNAME_N, "
    '    SSQL = SSQL & " TENMAST.SIT_NNAME_N, "
    '    SSQL = SSQL & " SEITOMAST.CLASS_CODE_O, "
    '    SSQL = SSQL & " SEITOMAST.KEIYAKU_DENWA_O, "
    ...
    '    SSQL = SSQL & " SEITOMAST.KOUZA_O, "
    '    SSQL = SSQL & " SEITOMAST.HIMOKU_ID_O, "
    '    SSQL = SSQL & " SEITOMAST.TUKI_NO_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU01_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU02_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU03_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU04_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU05_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU06_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU07_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU08_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU09_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU10_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU11_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU12_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU13_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU14_O, "
    '    SSQL = SSQL & " SEITOMAST.SEIKYU15_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU01_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU01_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU02_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU02_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU03_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU03_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU04_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU04_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU05_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU05_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU06_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU06_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU07_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU07_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU08_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU08_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU09_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU09_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU10_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU10_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU11_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU11_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU12_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU12_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU13_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU13_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU14_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU14_O, "
    '    SSQL = SSQL & " HIMOMAST.HIMOKU_KINGAKU15_H, "
    '    SSQL = SSQL & " SEITOMAST.KINGAKU15_O, "
    '    SSQL = SSQL & " SEITOMAST.SAKUSEI_DATE_O, "
    '    SSQL = SSQL & " SEITOMAST.KOUSIN_DATE_O "
    '    SSQL = SSQL & " FROM   "
    ...
    '    SSQL = SSQL & " ((SEITOMAST.TKIN_NO_O=TENMAST.KIN_NO_N (+)) AND "
    '    SSQL = SSQL & " (SEITOMAST.TSIT_NO_O=TENMAST.SIT_NO_N (+))) AND "
    '    SSQL = SSQL & " ((((SEITOMAST.GAKKOU_CODE_O=HIMOMAST.GAKKOU_CODE_H (+)) AND "
    '    SSQL = SSQL & " (SEITOMAST.GAKUNEN_CODE_O=HIMOMAST.GAKUNEN_CODE_H (+))) AND "
    '    SSQL = SSQL & " (SEITOMAST.HIMOKU_ID_O=HIMOMAST.HIMOKU_ID_H (+))) AND "
    '    SSQL = SSQL & " (SEITOMAST.TUKI_NO_O=HIMOMAST.TUKI_NO_H (+))) AND "
    ...
    '    End If
    '    '作成日
    '    SSQL = SSQL & "(SEITOMAST.SAKUSEI_DATE_O ='" & STR_FURIKAE_DATE(1) & "'"
    '    '更新日
    '    SSQL = SSQL & " OR SEITOMAST.KOUSIN_DATE_O ='" & STR_FURIKAE_DATE(1) & "')"
    '    '請求月
    '    SSQL = SSQL & " AND SEITOMAST.TUKI_NO_O ='" & txtSEIKYUTUKI.Text & "'"
    ...
    '            SSQL = SSQL & " SEITOMAST.TUUBAN_O         ASC"
    '    End Select
    '    PFUC_SQLQuery_生徒マスタ = SSQL

    '    'Debug.WriteLine("SSQL=" & SSQL)

    'End Function
    '2018/06/29 saitou 広島信金(RSV2) DEL ---------------------------------------- END
#End Region
